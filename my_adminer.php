...
print_fieldset($t,$ge,$Ai=false,$cf=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($cf)."return !toggle('fieldset-$t');\">$ge</a></legend><div id='fieldset-$t'".($Ai?"":" class='hidden'").">\n";}function
...
header("Set-Cookie: $C=".urlencode($Y).($je?"; expires=".gmdate("D, d M Y H:i:s",time()+$je)." GMT":"")."; path=".preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]).($ba?"; secure":"")."; HttpOnly; SameSite=lax");}function
...
as$s=>$lb){$oc=$Wb[$lb];if(!isset($oc))$oc=$Gi.$Gi[0];$J.=$oc;if($s)$Wb[]=$Gi.$oc[0];$Gi=$oc;}return$J;}function
...
as$y=>$ag){if(isset($ae[$y])){$ca=$y;break;}$y=preg_replace('~-.*~','',$y);if(!isset($ua[$y])&&isset($ae[$y])){$ca=$y;break;}}}$Sh=$_SESSION["translations"];if($_SESSION["translations_version"]!=1391795524){$Sh=array();$_SESSION["translations_version"]=1391795524;}function
...
set_schema($Dg){global$g,$ci,$ih;$J=$g->query("SET search_path TO ".idf_escape($Dg));foreach(types()as$U){if(!isset($ci[$U])){$ci[$U]=0;$ih[lang(23)][]=$U;}}return$J;}function
...
false;}libxml_use_internal_errors(true);$Hi=simplexml_load_string($Oc);if(!$Hi){$n=libxml_get_last_error();$g->error=$n->message;return
false;}if($Hi->Errors){$n=$Hi->Errors->Error;$g->error="$n->Message ($n->Code)";return
false;}$g->error='';$xh=$va."Result";return($Hi->$xh?$Hi->$xh:true);}function
sdb_request_all($va,$xh,$F=array(),$Fh=0){$J=array();$fh=($Fh?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$Hi=sdb_request($va,$F);if(!$Hi)break;foreach($Hi->$xh
as$oc)$J[]=$oc;if($z&&count($J)>=$z){$_GET["next"]=$Hi->NextToken;break;}if($Fh&&microtime(true)-$fh>$Fh)return
false;$F['NextToken']=$Hi->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($Hi->NextToken);return$J;}$x="simpledb";$ff=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$ed=array();$jd=array("count");$lc=array(array("json"));}$dc["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Of=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
...
as$Ki){foreach($c
as$Sd=>$ab){$rb=substr($ab,7,strpos($ab,"` ")-6);$rb=substr($rb,1,strlen($rb)-2);if($Ki===$rb)unset($c[$Sd]);}}if($Ge){foreach($Ge
as$y=>$Ki){$eh=substr($Ki,7,strpos($Ki,"` ")-6);foreach($c
as$Sd=>$ab){$Ga=substr($ab,strpos($ab,"NULL")+4,strlen($ab));$rb=substr($ab,strpos($ab,"CHANGE")+7,strpos($ab,"` ")-6);if($eh===$rb){unset($c[$Sd]);$c[$Sd]=$Ki.$Ga;unset($Ge[$y]);}}}foreach($Ge
as$Ki)$c[]=$Ki;}return($c||$Cf?queries("ALTER TABLE ".table($R)."\n".implode(",\n",$c).$Cf):true);}function
alter_indexes($R,$c){foreach($c
...
long2str($W,$Di){$Bg='';foreach($W
as$X)$Bg.=pack('V',$X);if($Di)return
substr($Bg,0,end($W));return$Bg;}function
str2long($Bg,$Di){$W=array_values(unpack('V*',str_pad($Bg,4*ceil(strlen($Bg)/4),"\0")));if($Di)$W[]=strlen($Bg);return$W;}function
xxtea_mx($Ji,$Ii,$mh,$Pd){return
int32((($Ji>>5&0x7FFFFFF)^$Ii<<2)+(($Ii>>3&0x1FFFFFFF)^$Ji<<4))^int32(($mh^$Ii)+($Pd^$Ji));}function
encrypt_string($hh,$y){if($hh=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($hh,true);$Je=count($W)-1;$Ji=$W[$Je];$Ii=$W[0];$ag=floor(6+52/($Je+1));$mh=0;while($ag-->0){$mh=int32($mh+0x9E3779B9);$kc=$mh>>2&3;for($wf=0;$wf<$Je;$wf++){$Ii=$W[$wf+1];$Ie=xxtea_mx($Ji,$Ii,$mh,$y[$wf&3^$kc]);$Ji=int32($W[$wf]+$Ie);$W[$wf]=$Ji;}$Ii=$W[0];$Ie=xxtea_mx($Ji,$Ii,$mh,$y[$wf&3^$kc]);$Ji=int32($W[$Je]+$Ie);$W[$Je]=$Ji;}return
long2str($W,false);}function
decrypt_string($hh,$y){if($hh=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($hh,false);$Je=count($W)-1;$Ji=$W[$Je];$Ii=$W[0];$ag=floor(6+52/($Je+1));$mh=int32($ag*0x9E3779B9);while($mh){$kc=$mh>>2&3;for($wf=$Je;$wf>0;$wf--){$Ji=$W[$wf-1];$Ie=xxtea_mx($Ji,$Ii,$mh,$y[$wf&3^$kc]);$Ii=int32($W[$wf]-$Ie);$W[$wf]=$Ii;}$Ji=$W[$Je];$Ie=xxtea_mx($Ji,$Ii,$mh,$y[$wf&3^$kc]);$Ii=int32($W[0]-$Ie);$W[0]=$Ii;$mh=int32($mh-0x9E3779B9);}return
...
auth_error($n){global$b,$nd;$Qg=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Qg]||$_GET[$Qg])&&!$nd)$n=lang(81);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(82,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Qg]&&$_GET[$Qg]&&ini_bool("session.use_only_cookies"))$n=lang(83);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(84),lang(85,implode(", ",$Of)),false);page_footer("auth");exit;}$g=connect();}$cc=new
Min_Driver($g);if(!is_object($g)||($le=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?h($g):(is_string($le)?$le:lang(86))));if($La&&$_POST["token"])$_POST["token"]=$Nh;$n='';if($_POST){if(!verify_token()){$Bd="max_input_vars";$we=ini_get($Bd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$we||$X<$we)){$Bd=$y;$we=$X;}}}$n=(!$_POST["token"]&&$we?lang(87,"'$Bd'"):lang(79).' '.lang(88));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(89,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(90);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
edit_type($y,$o,$qb,$Yc=array(),$_i){global$ih,$ci,$ji,$af;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),' aria-labelledby="label-type">';if($U&&!isset($ci[$U])&&!isset($Yc[$U]))array_unshift($ih,$U);if($Yc)$ih[lang(91)]=$Yc;echo
optionlist($ih,$U),'</select>
<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';if($o["is_virtual"])textarea(h($y)."[virtual1]",$_i,$L=2,$sb=40);else{echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(92).')'.optionlist($qb,$o["collation"]).'</select>',($ji?"<select name='".h($y)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($ji,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(93).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Yc?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(94).")".optionlist(explode("|",$af),$o["on_delete"])."</select> ":" ");}}function
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="',lang(48),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default">',lang(49),(support("comment")?"<td id='label-comment'".($yb?"":" class='hidden'").">".lang(47):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".lang(99)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($yb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".lang(99)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.3.0' alt='^' title='".lang(100)."' onclick='return !editingMoveRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.3.0' alt='v' title='".lang(101)."' onclick='return !editingMoveRow(this, 0);'>&nbsp;":""),($of==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".lang(102)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
as$yh)echo"\n<div class='references' title='".h($zh)."' id='refd$de-".($s++)."' style='left: $ee"."em; top: ".$R["fields"][$yh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$ee)."em;'></div></div>";}}echo"\n</div>\n";}foreach($Dg
...
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(!isset($o["privileges"][$ki?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($ki?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$fi=unique_array($_GET["where"],$w);$dg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(158),$cc->delete($a,$dg,!$fi));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($ki){if(!$O)redirect($A);queries_redirect($A,lang(159),$cc->update($a,$O,$dg,!$fi));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$cc->insert($a,$O);$ce=($I?last_id():0);queries_redirect($A,lang(160,($ce?" $ce":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($uc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".lang(165).")")+$uc,$K["Engine"])."</select>":""),' ',($qb&&!preg_match("~sqlite|mssql~",$x)?html_select("Collation",array(""=>"(".lang(92).")")+$qb,$K["Collation"]):""),' <input type="submit" value="',lang(14),'">
';}echo'
...
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}if(support("partitioning")){$Bf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(167),$K["partition_by"]);echo'<p>
...
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$kh=false;$be=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($be),lang(177),$kh);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(178));}}page_header(DB!=""?lang(64):lang(106),$n,array(),h(DB));$qb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$qb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$fd){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$fd,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$af),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$af),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
<noscript><p><input type="submit" name="add" value="',lang(191),'"></noscript>
';}if($C!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Nh,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$rf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$rf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ha=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ye=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$rf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ha,$A,$ye);else{$_h=$C."_adminer_".uniqid();drop_create("DROP $rf ".table($a),"CREATE $U ".table($C).$Ha,"DROP $U ".table($C),"CREATE $U ".table($_h).$Ha,"DROP $U ".table($_h),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$ye,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($rf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<input type="submit" value="',lang(14),'">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Nh,'">
</form>
...
<tr><th>',lang(203),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Fd,$K["INTERVAL_FIELD"]),'<tr><th>',lang(109),'<td>',html_select("STATUS",$gh,$K["STATUS"]),'<tr><th>',lang(47),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",lang(204)),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="',lang(14),'">
';if($aa!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Nh,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$yg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$_h="$K[name]_adminer_".uniqid();drop_create("DROP $yg ".idf_escape($da),create_routine($yg,$K),"DROP $yg ".idf_escape($K["name"]),create_routine($yg,array("name"=>$_h)+$K),"DROP $yg ".idf_escape($_h),substr(ME,0,-1),lang(205),lang(206),lang(207),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(208):lang(209)).": ".h($da):(isset($_GET["function"])?lang(210):lang(211))),$n);if(!$_POST&&$da!=""){$K=routine($da,$yg);$K["name"]=$da;}$qb=get_vals("SHOW CHARACTER SET");sort($qb);$zg=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(174),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
...
<input type="submit" value="',lang(14),'">
';if($da!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Nh,'">
</form>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Xh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Xh["Timing"])&&in_array($_POST["Event"],$Xh["Event"])&&in_array($_POST["Type"],$Xh["Type"])){$Ze=" ON ".table($a);$ec="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$Ze:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($ec,$A,lang(222));else{if($C!="")queries($ec);queries_redirect($A,($C!=""?lang(223):lang(224)),queries(create_trigger($Ze,$_POST)));if($C!="")queries(create_trigger($Ze,$K+array("Type"=>reset($Xh["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(225).": ".h($C):lang(226)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>',lang(227),'<td>',html_select("Timing",$Xh["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>',lang(228),'<td>',html_select("Event",$Xh["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Xh["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>',lang(46),'<td>',html_select("Type",$Xh["Type"],$K["Type"]),'</table>
...
as$X){if($X[1]!="USAGE")$gd["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$gd["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Xe=$B[1];}}if($_POST&&!$n){$Ye=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Ye",ME."privileges=",lang(229));else{$Ne=q($_POST["user"])."@".q($_POST["host"]);$Ef=$_POST["pass"];if($Ef!=''&&!$_POST["hashed"]){$Ef=$g->result("SELECT PASSWORD(".q($Ef).")");$n=!$Ef;}$Ib=false;if(!$n){if($Ye!=$Ne){$Ib=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Ne IDENTIFIED BY PASSWORD ".q($Ef));$n=!$Ib;}elseif($Ef!=$Xe)queries("SET PASSWORD FOR $Ne = ".q($Ef));}if(!$n){$vg=array();foreach($Le
...
as$Se=>$fd){echo'<th>'.($Se!="*.*"?"<input name='objects[$s]' value='".h($Se)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(123),"Columns"=>lang(45),"Procedures"=>lang(233),)as$Db=>$Ub){foreach((array)$Wf[$Db]as$Vf=>$xb){echo"<tr".odd()."><td".($Ub?">$Ub<td":" colspan='2'").' lang="en" title="'.h($xb).'">'.h($Vf);$s=0;foreach($gd
as$Se=>$fd){$C="'grants[$s][".h(strtoupper($Vf))."]'";$Y=$fd[strtoupper($Vf)];if($Db=="Server Admin"&&$Se!=(isset($gd["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(234)."<option value='0'".($Y=="0"?" selected":"").">".lang(235)."</select>";else
...
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Nh,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Wd=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$Wd++;}queries_redirect(ME."processlist=",lang(236,$Wd),$Wd||!$_POST["kill"]);}page_header(lang(107),$n);echo'
<form action="" method="post">
...
as$y=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){if($o["is_virtual"])continue;$e[$y]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$Dh=$b->selectLengthProcess();}$wg+=$o["privileges"];}list($M,$hd)=$b->selectColumnsProcess($e,$w);$Jd=count($hd)<count($M);$Z=$b->selectSearchProcess($p,$w);$kf=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$dd=($M?implode(", ",$M):"*".($Ue?", $Ue":"")).convert_fields($e,$p,$M)."\nFROM ".table($a);$id=($hd&&$Jd?"\nGROUP BY ".implode(", ",$hd):"").($kf?"\nORDER BY ".implode(", ",$kf):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$gi=>$K){$Ha=convert_field($p[key($K)]);$M=array($Ha?$Ha:idf_escape(key($K)));$Z[]=where_check($gi,$p);$J=$cc->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$Fi=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$gb=array();foreach($_POST["check"]as$db)$gb[]=where_check($db,$p);$Fi[]="((".implode(") OR (",$gb)."))";}$Fi=($Fi?"\nWHERE ".implode(" AND ",$Fi):"");$Rf=$ii=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Rf=array_flip($v["columns"]);$ii=($M?$Rf:array());break;}}foreach((array)$ii
as$y=>$X){if(in_array(idf_escape($y),$M))unset($ii[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$ii===array())$H="SELECT $dd$Fi$id";else{$ei=array();foreach($_POST["check"]as$X)$ei[]="(SELECT".limit($dd,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$id,1).")";$H=implode(" UNION ALL ",$ei);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Yc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($e
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($ii===array()&&is_array($_POST["check"]))||$Jd){$I=($_POST["delete"]?$cc->delete($a,$Fi):($_POST["clone"]?queries("INSERT $H$Fi"):$cc->update($a,$O,$Fi)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Ei="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$cc->delete($a,$Ei,1):($_POST["clone"]?queries("INSERT".limit1($H,$Ei)):$cc->update($a,$O,$Ei)));if(!$I)break;$za+=$g->affected_rows;}}}$ye=lang(242,$za);if($_POST["clone"]&&$I&&$za==1){$ce=last_id();if($ce)$ye=lang(160," $ce");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$ye,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(243);else{$I=true;$za=0;foreach($_POST["val"]as$gi=>$K){$O=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$I=$cc->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($gi,$p),!($Jd||$ii===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(242,$za),$I);}}elseif(!is_string($Oc=get_file("csv_file",true)))$n=upload_error($Oc);elseif(!preg_match('~~u',$Oc))$n=lang(244);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$sb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Oc,$qe);$za=count($qe[0]);$cc->begin();$Kg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($qe[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Kg]*)$Kg~",$X.$Kg,$re);if(!$y&&!array_diff($re[1],$sb)){$sb=$re[1];$za--;}else{$O=array();foreach($re[1]as$s=>$nb)$O[idf_escape($sb[$s])]=($nb==""&&$p[$sb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$nb))));$L[]=$O;}}$I=(!$L||$cc->insertUpdate($a,$L,$Rf));if($I)$I=$cc->commit();queries_redirect(remove_from_uri("page"),lang(245,$za),$I);$cc->rollback();}}}$qh=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(50).": $qh",$n);$O=null;if(isset($wg["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Yc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e&&support("table"))echo"<p class='error'>".lang(246).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$w);$b->selectOrderPrint($kf,$e,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($Dh);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$bd=$g->result(count_rows($a,$Z,$Jd,$hd));$E=floor(max(0,$bd-1)/$z);}$Hg=$M;if(!$Hg){$Hg[]="*";if($Ue)$Hg[]=$Ue;}$Eb=convert_fields($e,$p,$M);if($Eb)$Hg[]=substr($Eb,2);$I=$cc->select($a,$Hg,$Z,$hd,$kf,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$I->seek($z*$E);$rc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$hd&&$Jd&&$x=="sql")$bd=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Qa=$b->backwardKeys($a,$qh);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$hd&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(247)."</a>");$Ke=array();$ed=array();reset($M);$fg=1;foreach($L[0]as$y=>$X){if($y!=$Ue){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$y];$C=($o?$b->fieldName($o,$fg):($X["fun"]?"*":$y));if($C!=""){$fg++;$Ke[$y]=$C;$d=idf_escape($y);$sd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Ub="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($sd.($kf[0]==$d||$kf[0]==$y||(!$kf&&$Jd&&$hd[0]==$d)?$Ub:'')).'">';echo
...
as$y=>$X)json_row("sum-$y",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$m=>$X){json_row("tables-$m",$X);json_row("size-$m",db_size($m));}json_row("");}exit;}else{$wh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($wh&&!$n&&!$_POST["search"]){$I=true;$ye="";if($x=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ye=lang(256);}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ye=lang(257);}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ye=lang(258);}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ye=lang(259);}elseif($x!="sql"){$I=($x=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ye=lang(260);}elseif(!$_POST["tables"])$ye=lang(9);elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ye.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ye,$I);}page_header(($_GET["ns"]==""?lang(35).": ".h(DB):lang(73).": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".lang(261)."</h3>\n";$vh=tables_list();if(!$vh)echo"<p class='message'>".lang(9)."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".lang(262)." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".lang(53)."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}$ac=doc_link(array('sql'=>'show-table-status.html'));echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);" class="jsonly">','<th>'.lang(123),'<td>'.lang(263).doc_link(array('sql'=>'storage-engines.html')),'<td>'.lang(114).doc_link(array('sql'=>'charset-mysql.html')),'<td>'.lang(264).$ac,'<td>'.lang(265).$ac,'<td>'.lang(266).$ac,'<td>'.lang(48).doc_link(array('sql'=>'example-auto-increment.html')),'<td>'.lang(267).$ac,(support("comment")?'<td>'.lang(47).$ac:''),"</thead>\n";$T=0;foreach($vh
as$C=>$U){$yi=($U!==null&&!preg_match('~table~i',$U));$t=h("Table-".$C);echo'<tr'.odd().'><td>'.checkbox(($yi?"views[]":"tables[]"),$C,in_array($C,$wh,true),"","formUncheck('check-all');","",$t),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($C)."' title='".lang(40)."' id='$t'>".h($C).'</a>':h($C));if($yi){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.lang(41).'">'.(preg_match('~materialized~i',$U)?lang(121):lang(122)).'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.lang(39).'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",lang(42)),"Index_length"=>array("indexes",lang(125)),"Data_free"=>array("edit",lang(43)),"Auto_increment"=>array("auto_increment=1&create",lang(42)),"Rows"=>array("select",lang(39)),)as$y=>$_){$t=" id='$y-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$y=="Rows"||(support("indexes")&&$y!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$t title='$_[1]'>?</a>":"<span$t>?</span>"):"<td id='$y-".h($C)."'>&nbsp;");}$T++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".lang(238,count($vh)),"<td>".nbsp($x=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$y)echo"<td align='right' id='sum-$y'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$si="<input type='submit' value='".lang(268)."'".on_help("'VACUUM'")."> ";$gf="<input type='submit' name='optimize' value='".lang(269)."'".on_help($x=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".lang(118)." <span id='selected'></span></legend><div>".($x=="sqlite"?$si:($x=="pgsql"?$si.$gf:($x=="sql"?"<input type='submit' value='".lang(270)."'".on_help("'ANALYZE TABLE'")."> ".$gf."<input type='submit' name='check' value='".lang(271)."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".lang(272)."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".lang(273)."'".confirm().on_help($x=="sqlite"?"'DELETE'":"'TRUNCATE".($x=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".lang(119)."'".confirm().on_help("'DROP TABLE'").">\n";$l=(support("scheme")?$b->schemas():$b->databases());if(count($l)!=1&&$x!="sqlite"){$m=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".lang(274).": ",($l?html_select("target",$l,$m):'<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".lang(275)."'>",(support("copy")?" <input type='submit' name='copy' value='".lang(276)."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $T);":"")."\">\n";echo"<input type='hidden' name='token' value='$Nh'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.lang(71)."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.lang(195)."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".lang(135)."</h3>\n";$_g=routines();if($_g){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.lang(174).'<td>'.lang(46).'<td>'.lang(212)."<td>&nbsp;</thead>\n";odd('');foreach($_g
