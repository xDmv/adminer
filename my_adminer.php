...
remove_slashes($Rf,$Lc=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($Rf)){foreach($X
as$Kd=>$W){unset($Rf[$y][$Kd]);if(is_array($W)){$Rf[$y][stripslashes($Kd)]=$W;$Rf[]=&$Rf[$y][stripslashes($Kd)];}else$Rf[$y][stripslashes($Kd)]=($Lc?$W:stripslashes($W));}}}}function
bracket_escape($u,$Na=false){static$Kh=array(':'=>':1',']'=>':2','['=>':3','"'=>':4');return
strtr($u,($Na?array_flip($Kh):$Kh));}function
...
nl_br($Q){return
str_replace("\n","<br>",$Q);}function
checkbox($C,$Y,$db,$Rd="",$We="",$ib="",$Sd=""){$J="<input type='checkbox' name='$C' value='".h($Y)."'".($db?" checked":"").($Sd?" aria-labelledby='$Sd'":"").($We?' onclick="'.h($We).'"':'').">";return($Rd!=""||$ib?"<label".($ib?" class='$ib'":"").">$J".h($Rd)."</label>":$J);}function
optionlist($cf,$Dg=null,$ii=false){$J="";foreach($cf
as$Kd=>$W){$df=array($Kd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Kd).'">';$df=$W;}foreach($df
as$y=>$X)$J.='<option'.($ii||is_string($y)?' value="'.h($y).'"':'').(($ii||is_string($y)?(string)$y:$X)===$Dg?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$cf,$Y="",$Ve=true,$Sd=""){if($Ve)return"<select name='".h($C)."'".(is_string($Ve)?' onchange="'.h($Ve).'"':"").($Sd?" aria-labelledby='$Sd'":"").">".optionlist($cf,$Y)."</select>";$J="";foreach($cf
as$y=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($y)."'".($y==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ja,$cf,$Y="",$Df=""){return($cf?"<select$Ja><option value=''>$Df".optionlist($cf,$Y,true)."</select>":"<input$Ja size='10' value='".h($Y)."' placeholder='$Df'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($t,$ae,$ui=false,$We=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($We)."return !toggle('fieldset-$t');\">$ae</a></legend><div id='fieldset-$t'".($ui?"":" class='hidden'").">\n";}function
...
set_password($pi,$N,$V,$G){$_SESSION["pwds"][$pi][$N][$V]=($_COOKIE["adminer_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
...
get_key_vals($H,$h=null,$_h=0){global$g;if(!is_object($h))$h=$g;$J=array();$h->timeout=$_h;$I=$h->query($H);$h->timeout=0;if(is_object($I)){while($K=$I->fetch_row())$J[$K[0]]=$K[1];}return$J;}function
...
where($bb,$p);}function
where_link($s,$d,$Y,$Ye="="){return"&where%5B$s%5D%5Bcol%5D=".urlencode($d)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y!==null?$Ye:"IS NULL"))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);}function
...
auth_url($pi,$N,$V,$m=null){global$Yb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($Yb))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($pi!="server"||$N!=""?urlencode($pi)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
...
query_redirect($H,$A,$se,$cg=true,$xc=true,$Ec=false,$zh=""){global$g,$n,$b;if($xc){$Zg=microtime(true);$Ec=!$g->query($H);$zh=format_time($Zg);}$Wg="";if($H)$Wg=$b->messageQuery($H,$zh);if($Ec){$n=error().$Wg;return
false;}if($cg)redirect($A,$se.$Wg);return
true;}function
queries($H){global$g;static$Wf=array();static$Zg;if(!$Zg)$Zg=microtime(true);if($H===null)return
array(implode("\n",$Wf),format_time($Zg));$Wf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$g->query($H);}function
apply_queries($H,$T,$tc='table'){foreach($T
as$R){if(!queries("$H ".$tc($R)))return
false;}return
true;}function
queries_redirect($A,$se,$cg){list($Wf,$zh)=queries(null);return
query_redirect($Wf,$A,$se,$cg,false,!$cg,$zh);}function
format_time($Zg){return
lang(1,max(0,microtime(true)-$Zg));}function
remove_from_uri($rf=""){return
substr(preg_replace("~(?<=[?&])($rf".(SID?"":"|".session_name()).")=[^&]*&~",'',"$_SERVER[REQUEST_URI]&"),0,-1);}function
...
null;foreach($Jc
as$y=>$X)$Jc[$y]=(array)$X;$J='';foreach($Jc["error"]as$y=>$n){if($n)return$n;$C=$Jc["name"][$y];$Gh=$Jc["tmp_name"][$y];$xb=file_get_contents($Lb&&preg_match('~\\.gz$~',$C)?"compress.zlib://$Gh":$Gh);if($Lb){$Zg=substr($xb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Zg,$ig))$xb=iconv("utf-16","utf-8",$xb);elseif($Zg=="\xEF\xBB\xBF")$xb=substr($xb,3);$J.=$xb."\n\n";}else$J.=$xb;}return$J;}function
upload_error($n){$pe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($pe?" ".lang(3,$pe):""):lang(4));}function
repeat_pattern($Bf,$be){return
str_repeat("$Bf{0,65535}",$be/65535)."$Bf{0,".($be%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($Q,$be=80,$fh=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{10FFFF}]",$be).")($)?)u",$Q,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$be).")($)?)",$Q,$B);return
h($B[1]).$fh.(isset($B[2])?"":"<i>...</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Rf,$qd=array()){while(list($y,$X)=each($Rf)){if(!in_array($y,$qd)){if(is_array($X)){foreach($X
as$Kd=>$W)$Rf[$y."[$Kd]"]=$W;}else
...
input($o,$Y,$r){global$g,$Vh,$b,$x;$C=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$r){$Ea=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$Ea[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$Ea);$r="json";}$lg=($x=="mssql"&&$o["auto_increment"]);if($lg&&!$_POST["save"])$r=null;$Zc=(isset($_GET["select"])||$lg?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ja=" name='fields[$C]'";if($o["type"]=="enum")echo
nbsp($Zc[""])."<td>".$b->editInput($_GET["edit"],$o,$Ja,$Y);else{$Mc=0;foreach($Zc
as$y=>$X){if($y===""||!$X)break;$Mc++;}$Ve=($Mc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Mc > f.selectedIndex) f.selectedIndex = $Mc;\" onkeyup='keyupChange.call(this);'":"");$Ja.=$Ve;$hd=(in_array($r,$Zc)||isset($Zc[$r]));echo(count($Zc)>1?"<select name='function[$C]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($Zc,$r===null||$hd?$r:"")."</select>":nbsp(reset($Zc))).'<td>';$zd=$b->editInput($_GET["edit"],$o,$Ja,$Y);if($zd!="")echo$zd;elseif(preg_match('~bool~',$o["type"]))echo"<input type='hidden'$Ja value='0'>"."<input type='checkbox'".(in_array(strtolower($Y),array('1','t','true','y','yes','on'))?" checked='checked'":"")."$Ja value='1'>";elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$ke);foreach($ke[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$db=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($db?' checked':'')."$Ve>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$C'$Ve>";elseif(($xh=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($xh&&$x!="sqlite")$Ja.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Ja.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Ja>".h($Y).'</textarea>';}elseif($r=="json"||preg_match('~^jsonb?$~',$o["type"]))echo"<textarea$Ja cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$re=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$B)?((preg_match("~binary~",$o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($Vh[$o["type"]]?$Vh[$o["type"]]+($o["unsigned"]?0:1):0));if($x=='sql'&&$g->server_info>=5.6&&preg_match('~time~',$o["type"]))$re+=7;echo"<input".((!$hd||$r==="")&&preg_match('~(?<!o)int~',$o["type"])&&!preg_match('~\[\]~',$o["full_type"])?" type='number'":"")." value='".h($Y)."'".($re?" data-maxlength='$re'":"").(preg_match('~char|binary~',$o["type"])&&$re>20?" size='40'":"")."$Ja>";}}}function
...
dump_headers($od,$Be=false){global$b;$J=$b->dumpHeaders($od,$Be);$pf=$_POST["output"];if($pf!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($od).".$J".($pf!="file"&&!preg_match('~[^0-9a-z]~',$pf)?".$pf":""));session_write_close();ob_flush();flush();return$J;}function
...
select_value($X,$_,$o,$yh){global$b,$ba;if(is_array($X)){$J="";foreach($X
as$Kd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Kd):"")."<td>".select_value($W,$_,$o,$yh);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="mailto:$X";if($Tf=is_url($X))$_=(($Tf=="http"&&$ba)||preg_match('~WebKit|Firefox~i',$_SERVER["HTTP_USER_AGENT"])?$X:"https://www.adminer.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$o);if($J!==null){if($J==="")$J="&nbsp;";elseif(!is_utf8($J))$J="\0";elseif($yh!=""&&is_shortable($o))$J=shorten_utf8($J,max(0,+$yh));else$J=h($J);}return$b->selectVal($J,$_,$o,$X);}function
is_mail($kc){$Ha='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Wb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$Bf="$Ha+(\\.$Ha+)*@($Wb?\\.)+$Wb";return
is_string($kc)&&preg_match("(^$Bf(,\\s*$Bf)*\$)i",$kc);}function
...
slow_query($H){global$b,$Hh;$m=$b->database();$_h=$b->queryTimeout();if(support("kill")&&is_object($h=connect())&&($m==""||$h->select_db($m))){$Pd=$h->result(connection_id());echo'<script type="text/javascript">
var timeout = setTimeout(function () {
	ajax(\'',js_escape(ME),'script=kill\', function () {
	}, \'token=',$Hh,'&kill=',$Pd,'\');
}, ',1000*$_h,');
</script>
';}else$h=null;ob_flush();flush();$J=@get_key_vals($H,$h,$_h);if($h){echo"<script type='text/javascript'>clearTimeout(timeout);</script>\n";ob_flush();flush();}return
array_keys($J);}function
get_token(){$Zf=rand(1,1e6);return($Zf^$_SESSION["token"]).":$Zf";}function
verify_token(){list($Hh,$Zf)=explode(":",$_POST["token"]);return($Zf^$_SESSION["token"])==$Hh;}function
lzw_decompress($Ra){$Sb=256;$Sa=8;$kb=array();$ng=0;$og=0;for($s=0;$s<strlen($Ra);$s++){$ng=($ng<<8)+ord($Ra[$s]);$og+=8;if($og>=$Sa){$og-=$Sa;$kb[]=$ng>>$og;$ng&=(1<<$og)-1;$Sb++;if($Sb>>$Sa)$Sa++;}}$Rb=range("\0","\xFF");$J="";foreach($kb
as$s=>$jb){$jc=$Rb[$jb];if(!isset($jc))$jc=$zi.$zi[0];$J.=$jc;if($s)$Rb[]=$zi.$jc[0];$zi=$jc;}return$J;}function
on_help($qb,$Ng=0){return" onmouseover='helpMouseover(this, event, ".h($qb).", $Ng);' onmouseout='helpMouseout(this, event);'";}function
edit_form($a,$p,$K,$di){global$b,$x,$Hh,$n;$kh=$b->tableName(table_status1($a,true));page_header(($di?lang(10):lang(11)),$n,array("select"=>array($a,$kh)),$kh);if($K===false)echo"<p class='error'>".lang(12)."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$p)echo"<p class='error'>".lang(13)."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($p
as$C=>$o){echo"<tr><th>".$b->fieldName($o);$Mb=$_GET["set"][bracket_escape($C)];if($Mb===null){$Mb=$o["default"];if($o["type"]=="bit"&&preg_match("~^b'([01]*)'\$~",$Mb,$ig))$Mb=$ig[1];}$Y=($K!==null?($K[$C]!=""&&$x=="sql"&&preg_match("~enum|set~",$o["type"])?(is_array($K[$C])?array_sum($K[$C]):+$K[$C]):$K[$C]):(!$di&&$o["auto_increment"]?"":(isset($_GET["select"])?false:$Mb)));if(!$_POST["save"]&&is_string($Y))$Y=$b->editVal($Y,$o);$r=($_POST["save"]?(string)$_POST["function"][$C]:($di&&$o["on_update"]=="CURRENT_TIMESTAMP"?"now":($Y===false?null:($Y!==null?'':'NULL'))));if(preg_match("~time~",$o["type"])&&$Y=="CURRENT_TIMESTAMP"){$Y="";$r="now";}input($o,$Y,$r);echo"\n";}if(!support("table"))echo"<tr>"."<th><input name='field_keys[]' onkeyup='keyupChange.call(this);' onchange='fieldChange(this);' value=''>"."<td class='function'>".html_select("field_funs[]",$b->editFunctions(array("null"=>isset($_GET["select"]))))."<td><input name='field_vals[]'>"."\n";echo"</table>\n";}echo"<p>\n";if($p){echo"<input type='submit' value='".lang(14)."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($di?lang(15)."' onclick='return !ajaxForm(this.form, \"".lang(16).'...", this)':lang(17))."' title='Ctrl+Shift+Enter'>\n";}echo($di?"<input type='submit' name='delete' value='".lang(18)."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$Hh,'">
</form>
';}global$b,$g,$Yb,$gc,$qc,$n,$Zc,$ed,$ba,$yd,$x,$ca,$Ud,$Ue,$Cf,$ch,$id,$Hh,$Mh,$Vh,$ci,$ia;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$ba=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);session_cache_limiter("");if(!defined("SID")){session_name("adminer_sid");$F=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$ba);if(version_compare(PHP_VERSION,'5.2.0')>=0)$F[]=true;call_user_func_array('session_set_cookie_params',$F);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Lc);if(get_magic_quotes_runtime())set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);$Ud=array('en'=>'English','ar'=>'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bg'=>'Ð‘ÑŠÐ»Ð³Ð°Ñ€ÑÐºÐ¸','bn'=>'à¦¬à¦¾à¦‚à¦²à¦¾','bs'=>'Bosanski','ca'=>'CatalÃ ','cs'=>'ÄŒeÅ¡tina','da'=>'Dansk','de'=>'Deutsch','el'=>'Î•Î»Î»Î·Î½Î¹ÎºÎ¬','es'=>'EspaÃ±ol','et'=>'Eesti','fa'=>'ÙØ§Ø±Ø³ÛŒ','fi'=>'Suomi','fr'=>'FranÃ§ais','gl'=>'Galego','hu'=>'Magyar','id'=>'Bahasa Indonesia','it'=>'Italiano','ja'=>'æ—¥æœ¬èªž','ko'=>'í•œêµ­ì–´','lt'=>'LietuviÅ³','nl'=>'Nederlands','no'=>'Norsk','pl'=>'Polski','pt'=>'PortuguÃªs','pt-br'=>'PortuguÃªs (Brazil)','ro'=>'Limba RomÃ¢nÄƒ','ru'=>'Ð ÑƒÑÑÐºÐ¸Ð¹ ÑÐ·Ñ‹Ðº','sk'=>'SlovenÄina','sl'=>'Slovenski','sr'=>'Ð¡Ñ€Ð¿ÑÐºÐ¸','ta'=>'à®¤â€Œà®®à®¿à®´à¯','th'=>'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr'=>'TÃ¼rkÃ§e','uk'=>'Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°','vi'=>'Tiáº¿ng Viá»‡t','zh'=>'ç®€ä½“ä¸­æ–‡','zh-tw'=>'ç¹é«”ä¸­æ–‡',);function
get_lang(){global$ca;return$ca;}function
lang($u,$Le=null){if(is_string($u)){$Ff=array_search($u,get_translations("en"));if($Ff!==false)$u=$Ff;}global$ca,$Mh;$Lh=($Mh[$u]?$Mh[$u]:$u);if(is_array($Lh)){$Ff=($Le==1?0:($ca=='cs'||$ca=='sk'?($Le&&$Le<5?1:2):($ca=='fr'?(!$Le?0:1):($ca=='pl'?($Le%10>1&&$Le%10<5&&$Le/10%10!=1?1:2):($ca=='sl'?($Le%100==1?0:($Le%100==2?1:($Le%100==3||$Le%100==4?2:3))):($ca=='lt'?($Le%10==1&&$Le%100!=11?0:($Le%10>1&&$Le/10%10!=1?1:2)):($ca=='bs'||$ca=='ru'||$ca=='sr'||$ca=='uk'?($Le%10==1&&$Le%100!=11?0:($Le%10>1&&$Le%10<5&&$Le/10%10!=1?1:2)):1)))))));$Lh=$Lh[$Ff];}$Ea=func_get_args();array_shift($Ea);$Uc=str_replace("%d","%s",$Lh);if($Uc!=$Lh)$Ea[0]=format_number($Le);return
...
select($R,$M,$Z,$cd,$ef=array(),$z=1,$E=0,$Nf=false){global$b,$x;$Ed=(count($cd)<count($M));$H=$b->selectQueryBuild($M,$Z,$cd,$ef,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$cd&&$Ed&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($cd&&$Ed?"\nGROUP BY ".implode(", ",$cd):"").($ef?"\nORDER BY ".implode(", ",$ef):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");$Zg=microtime(true);$J=$this->_conn->query($H);if($Nf)echo$b->selectQuery($H,format_time($Zg));return$J;}function
delete($R,$Xf,$z=0){$H="FROM ".table($R);return
queries("DELETE".($z?limit1($H,$Xf):" $H$Xf"));}function
update($R,$O,$Xf,$z=0,$Fg="\n"){$ni=array();foreach($O
as$y=>$X)$ni[]="$y = $X";$H=table($R)." SET$Fg".implode(",$Fg",$ni);return
queries("UPDATE".($z?limit1($H,$Xf):" $H$Xf"));}function
insert($R,$O){return
queries("INSERT INTO ".table($R).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":" DEFAULT VALUES"));}function
insertUpdate($R,$L,$Lf){return
false;}function
begin(){return
...
rollback(){return
queries("ROLLBACK");}}$Yb["sqlite"]="SQLite 3";$Yb["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$If=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($Kc){$this->_link=new
SQLite3($Kc);$qi=$this->_link->version();$this->server_info=$qi["versionString"];}function
...
query($H,$Wh=false){$ye=($Wh?"unbufferedQuery":"query");$I=@$this->_link->$ye($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
fetch_field(){$C=$this->_result->fieldName($this->_offset++);$Bf='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($Bf\\.)?$Bf\$~",$C,$B)){$R=($B[3]!=""?$B[3]:idf_unescape($B[2]));$C=($B[5]!=""?$B[5]:idf_unescape($B[4]));}return(object)array("name"=>$C,"orgname"=>$C,"orgtable"=>$R,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
extends
Min_SQL{function
insertUpdate($R,$L,$Lf){$ni=array();foreach($L
...
array();}function
limit($H,$Z,$z,$D=0,$Fg=" "){return" $H$Z".($z!==null?$Fg."LIMIT $z".($D?" OFFSET $D":""):"");}function
...
fields($R){global$g;$J=array();$Lf="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Mb=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Mb,$B)?str_replace("''","'",$B[1]):($Mb=="NULL"?null:$Mb)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Lf!="")$J[$Lf]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Lf=$C;}}$Wg=$g->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Wg,$ke,PREG_SET_ORDER);foreach($ke
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$Wg=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Wg,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$ke,PREG_SET_ORDER);foreach($ke
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($R)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$Yg=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$h);foreach(get_rows("PRAGMA index_list(".table($R).")",$h)as$K){$C=$K["name"];$v=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$v["lengths"]=array();$v["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$h)as$wg){$v["columns"][]=$wg["name"];$v["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$Yg[$C],$ig)){preg_match_all('/("[^"]*+")+( DESC)?/',$ig[2],$ke);foreach($ke[2]as$y=>$X){if($X)$v["descs"][$y]='1';}}if(!$J[""]||$v["type"]!="UNIQUE"||$v["columns"]!=$J[""]["columns"]||$v["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$v;}return$J;}function
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){$hi=($R==""||$Rc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$hi=true;break;}}$c=array();$nf=array();foreach($p
as$o){if($o[1]){$c[]=($hi?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$nf[$o[0]]=$o[1][0];}}if(!$hi){foreach($c
as$X){if(!queries("ALTER TABLE ".table($R)." $X"))return
false;}if($R!=$C&&!queries("ALTER TABLE ".table($R)." RENAME TO ".table($C)))return
false;}elseif(!recreate_table($R,$C,$c,$nf,$Rc))return
false;if($La)queries("UPDATE sqlite_sequence SET seq = $La WHERE name = ".q($C));return
true;}function
recreate_table($R,$C,$p,$nf,$Rc,$w=array()){if($R!=""){if(!$p){foreach(fields($R)as$y=>$o){$p[]=process_field($o,$o);$nf[$y]=idf_escape($y);}}$Mf=false;foreach($p
as$o){if($o[6])$Mf=true;}$bc=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$bc[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$Nd=>$v){$e=array();foreach($v["columns"]as$y=>$d){if(!$nf[$d])continue
2;$e[]=$nf[$d].($v["descs"][$y]?" DESC":"");}if(!$bc[$Nd]){if($v["type"]!="PRIMARY"||!$Mf)$w[]=array($v["type"],$Nd,$e);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$Rc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($R)as$Nd=>$q){foreach($q["source"]as$y=>$d){if(!$nf[$d])continue
2;$q["source"][$y]=idf_unescape($nf[$d]);}if(!isset($Rc[" $Nd"]))$Rc[]=" ".format_foreign_key($q);}queries("BEGIN");}foreach($p
as$y=>$o)$p[$y]="  ".implode($o);$p=array_merge($p,array_filter($Rc));if(!queries("CREATE TABLE ".table($R!=""?"adminer_$C":$C)." (\n".implode(",\n",$p)."\n)"))return
false;if($R!=""){if($nf&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ",$nf).") SELECT ".implode(", ",array_map('idf_escape',array_keys($nf)))." FROM ".table($R)))return
false;$Sh=array();foreach(triggers($R)as$Qh=>$Ah){$Ph=trigger($Qh);$Sh[]="CREATE TRIGGER ".idf_escape($Qh)." ".implode(" ",$Ah)." ON ".table($C)."\n$Ph[Statement]";}if(!queries("DROP TABLE ".table($R)))return
...
move_tables($T,$si,$sh){return
false;}function
trigger($C){global$g;if($C=="")return
array("Statement"=>"BEGIN\n\t;\nEND");$u='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$Rh=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|",$Rh["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$g->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$Ne=$B[3];return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($Ne?" OF":""),"Of"=>($Ne[0]=='`'||$Ne[0]=='"'?idf_unescape($Ne):$Ne),"Trigger"=>$C,"Statement"=>$B[4],);}function
triggers($R){$J=array();$Rh=trigger_options();foreach(get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($R))as$K){preg_match('~^CREATE\\s+TRIGGER\\s*(?:[^`"\\s]+|`[^`]*`|"[^"]*")+\\s*('.implode("|",$Rh["Timing"]).')\\s*(.*)\\s+ON\\b~iU',$K["sql"],$B);$J[$K["name"]]=array($B[1],$B[2]);}return$J;}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$bf){list($y,$X)=explode("=",$bf,2);$J[$y]=$X;}return$J;}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$Gc);}$x="sqlite";$Vh=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$ch=array_keys($Vh);$ci=array();$Ze=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$Zc=array("hex","length","lower","round","unixepoch","upper");$ed=array("avg","count","count distinct","group_concat","max","min","sum");$gc=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$Yb["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$If=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
extends
Min_SQL{function
insertUpdate($R,$L,$Lf){global$g;foreach($L
as$O){$di=array();$Z=array();foreach($O
as$y=>$X){$di[]="$y = $X";if(isset($Lf[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$di)." WHERE ".implode(" AND ",$Z))&&$g->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
...
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$lh=$h->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($R));$e=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $lh AND attnum > 0",$h);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $lh AND ci.oid = i.indexrelid",$h)as$K){$jg=$K["relname"];$J[$jg]["type"]=($K["indispartial"]?"INDEX":($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX")));$J[$jg]["columns"]=array();foreach(explode(" ",$K["indkey"])as$ud)$J[$jg]["columns"][]=$e[$ud];$J[$jg]["descs"]=array();foreach(explode(" ",$K["indoption"])as$vd)$J[$jg]["descs"][]=($vd&1?'1':null);$J[$jg]["lengths"]=array();}return$J;}function
foreign_keys($R){global$Ue;$J=array();foreach(get_rows("SELECT conname, condeferrable::int AS deferrable, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
WHERE conrelid = (SELECT pc.oid FROM pg_class AS pc INNER JOIN pg_namespace AS pn ON (pn.oid = pc.relnamespace) WHERE pc.relname = ".q($R)." AND pn.nspname = current_schema())
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$B)){$K['source']=array_map('trim',explode(',',$B[1]));if(preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~',$B[2],$je)){$K['ns']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$je[2]));$K['table']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$je[4]));}$K['target']=array_map('trim',explode(',',$B[3]));$K['on_delete']=(preg_match("~ON DELETE ($Ue)~",$B[4],$je)?$je[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($Ue)~",$B[4],$je)?$je[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){$c=array();$Wf=array();foreach($p
as$o){$d=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $d";else{$mi=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($d!=$X[0])$Wf[]="ALTER TABLE ".table($R)." RENAME $d TO $X[0]";$c[]="ALTER $d TYPE$X[1]";if(!$X[6]){$c[]="ALTER $d ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $d ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$mi!="")$Wf[]="COMMENT ON COLUMN ".table($R).".$X[0] IS ".($mi!=""?substr($mi,9):"''");}}$c=array_merge($c,$Rc);if($R=="")array_unshift($Wf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Wf,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""&&$R!=$C)$Wf[]="ALTER TABLE ".table($R)." RENAME TO ".table($C);if($R!=""||$sb!="")$Wf[]="COMMENT ON TABLE ".table($C)." IS ".q($sb);if($La!=""){}foreach($Wf
as$H){if(!queries($H))return
false;}return
true;}function
alter_indexes($R,$c){$i=array();$Zb=array();$Wf=array();foreach($c
as$X){if($X[0]!="INDEX")$i[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$Zb[]=idf_escape($X[1]);else$Wf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R)." (".implode(", ",$X[2]).")";}if($i)array_unshift($Wf,"ALTER TABLE ".table($R).implode(",",$i));if($Zb)array_unshift($Wf,"DROP INDEX ".implode(", ",$Zb));foreach($Wf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($T){return
...
false;}return
true;}function
move_tables($T,$si,$sh){foreach(array_merge($T,$si)as$R){$P=table_status($R);if(!queries("ALTER ".strtoupper($P["Engine"])." ".table($R)." SET SCHEMA ".idf_escape($sh)))return
false;}return
true;}function
...
set_schema($zg){global$g,$Vh,$ch;$J=$g->query("SET search_path TO ".idf_escape($zg));foreach(types()as$U){if(!isset($Vh[$U])){$Vh[$U]=0;$ch[lang(23)][]=$U;}}return$J;}function
create_sql($R,$La){global$g;$J='';$pg=array();$Hg=array();$P=table_status($R);$p=fields($R);$w=indexes($R);ksort($w);$Pc=foreign_keys($R);ksort($Pc);$Sh=triggers($R);if(!$P||empty($p))return
false;$J="CREATE TABLE ".idf_escape($P['nspname']).".".idf_escape($P['Name'])." (\n    ";foreach($p
as$Ic=>$o){$uf=idf_escape($o['field']).' '.$o['full_type'].(is_null($o['default'])?"":" DEFAULT $o[default]").($o['attnotnull']?"":" NOT NULL");$pg[]=$uf;if(preg_match('~nextval\(\'([^\']+)\'\)~',$o['default'],$ke)){$Gg=$ke[1];$Vg=reset(get_rows("SELECT * FROM $Gg"));$Hg[]="CREATE SEQUENCE $Gg INCREMENT $Vg[increment_by] MINVALUE $Vg[min_value] MAXVALUE $Vg[max_value] START ".($La?$Vg['last_value']:1)." CACHE $Vg[cache_value];";}}if(!empty($Hg))$J=implode("\n\n",$Hg)."\n\n$J";foreach($w
as$sd=>$v){switch($v['type']){case'UNIQUE':$pg[]="CONSTRAINT ".idf_escape($sd)." UNIQUE (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;case'PRIMARY':$pg[]="CONSTRAINT ".idf_escape($sd)." PRIMARY KEY (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;}}foreach($Pc
as$Oc=>$Nc)$pg[]="CONSTRAINT ".idf_escape($Oc)." $Nc[definition] ".($Nc['deferrable']?'DEFERRABLE':'NOT DEFERRABLE');$J.=implode(",\n    ",$pg)."\n) WITH (oids = ".($P['Oid']?'true':'false').");";foreach($w
...
as$Ic=>$o){if($o['comment'])$J.="\n\nCOMMENT ON COLUMN ".idf_escape($P['nspname']).".".idf_escape($P['Name']).".".idf_escape($Ic)." IS ".q($o['comment']).";";}foreach($Sh
as$Oh=>$Nh){$Ph=trigger($Oh,$P['Name']);$J.="\n\nCREATE TRIGGER ".idf_escape($Ph['Trigger'])." $Ph[Timing] $Ph[Events] ON ".idf_escape($P["nspname"]).".".idf_escape($P['Name'])." $Ph[Type] $Ph[Statement];";}return
rtrim($J,';');}function
trigger_sql($R,$dh){$J="";return
false;}function
...
select_db($k){return
true;}function
query($H,$Wh=false){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$n=oci_error($this->_link);$this->errno=$n["code"];$this->error=$n["message"];return
...
limit($H,$Z,$z,$D=0,$Fg=" "){return($D?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $H$Z) t WHERE rownum <= ".($z+$D).") WHERE rnum > $D":($z!==null?" * FROM (SELECT $H$Z) WHERE rownum <= ".($z+$D):" $H$Z"));}function
limit1($H,$Z){return" $H$Z";}function
...
found_rows($S,$Z){}function
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){$c=$Zb=array();foreach($p
...
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Gc);}$x="oracle";$Vh=array();$ch=array();foreach(array(lang(24)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(25)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(26)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$Vh+=$X;$ch[$y]=array_keys($X);}$ci=array();$Ze=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Zc=array("length","lower","round","upper");$ed=array("avg","count","count distinct","max","min","sum");$gc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Yb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$If=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
extends
Min_SQL{function
insertUpdate($R,$L,$Lf){foreach($L
as$O){$di=array();$Z=array();foreach($O
as$y=>$X){$di[]="$y = $X";if(isset($Lf[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$di)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
limit($H,$Z,$z,$D=0,$Fg=" "){return($z!==null?" TOP (".($z+$D).")":"")." $H$Z";}function
limit1($H,$Z){return
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){$c=array();foreach($p
...
move_tables($T,$si,$sh){return
apply_queries("ALTER SCHEMA ".idf_escape($sh)." TRANSFER",array_merge($T,$si));}function
...
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|indexes|scheme|sql|table|trigger|view|view_trigger)$~',$Gc);}$x="mssql";$Vh=array();$ch=array();foreach(array(lang(24)=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),lang(25)=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),lang(26)=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),lang(27)=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$y=>$X){$Vh+=$X;$ch[$y]=array_keys($X);}$ci=array();$Ze=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$Zc=array("len","lower","round","upper");$ed=array("avg","count","count distinct","max","min","sum");$gc=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$Yb['firebird']='Firebird (alpha)';if(isset($_GET["firebird"])){$If=array("interbase");define("DRIVER","firebird");if(extension_loaded("interbase")){class
Min_DB{var$extension="Firebird",$server_info,$affected_rows,$errno,$error,$_link,$_result;function
connect($N,$V,$G){$this->_link=ibase_connect($N,$V,$G);if($this->_link){$fi=explode(':',$N);$this->service_link=ibase_service_attach($fi[0],$V,$G);$this->server_info=ibase_server_info($this->service_link,IBASE_SVC_SERVER_VERSION);}else{$this->errno=ibase_errcode();$this->error=ibase_errmsg();}return(bool)$this->_link;}function
quote($Q){return"'".str_replace("'","''",$Q)."'";}function
select_db($k){return($k=="domain");}function
query($H,$Wh=false){$I=ibase_query($H,$this->_link);if(!$I){$this->errno=ibase_errcode();$this->error=ibase_errmsg();return
...
limit($H,$Z,$z,$D=0,$Fg=" "){$J='';$J.=($z!==null?$Fg."FIRST $z".($D?" SKIP $D":""):"");$J.=" $H$Z";return$J;}function
limit1($H,$Z){return
limit($H,$Z,1);}function
...
true;}function
support($Gc){return
preg_match("~^(columns|sql|status|table)$~",$Gc);}$x="firebird";$Ze=array("=");$Zc=array();$ed=array();$gc=array();}$Yb["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$If=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
Min_DB{var$extension="SimpleXML",$server_info='2009-04-15',$error,$timeout,$next,$affected_rows,$_result;function
select_db($k){return($k=="domain");}function
query($H,$Wh=false){$F=array('SelectExpression'=>$H,'ConsistentRead'=>'true');if($this->next)$F['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$F,$this->timeout);if($I===false)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$H)){$gh=0;foreach($I
as$Id)$gh+=$Id->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$gh,))));}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result(){return$this->_result;}function
...
Min_Driver
extends
Min_SQL{public$Lf="itemName()";function
_chunkRequest($pd,$va,$F,$zc=array()){global$g;foreach(array_chunk($pd,25)as$gb){$sf=$F;foreach($gb
as$s=>$t){$sf["Item.$s.ItemName"]=$t;foreach($zc
as$y=>$X)$sf["Item.$s.$y"]=$X;}if(!sdb_request($va,$sf))return
false;}$g->affected_rows=count($pd);return
true;}function
_extractIds($R,$Xf,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$Xf,$ke))$J=array_map('idf_unescape',$ke[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$Xf.($z?" LIMIT 1":"")))as$Id)$J[]=$Id->Name;}return$J;}function
select($R,$M,$Z,$cd,$ef=array(),$z=1,$E=0,$Nf=false){global$g;$g->next=$_GET["next"];$J=parent::select($R,$M,$Z,$cd,$ef,$z,$E,$Nf);$g->next=0;return$J;}function
delete($R,$Xf,$z=0){return$this->_chunkRequest($this->_extractIds($R,$Xf,$z),'BatchDeleteAttributes',array('DomainName'=>$R));}function
update($R,$O,$Xf,$z=0,$Fg="\n"){$Nb=array();$_d=array();$s=0;$pd=$this->_extractIds($R,$Xf,$z);$t=idf_unescape($O["`itemName()`"]);unset($O["`itemName()`"]);foreach($O
...
idf_escape($u);}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
limit($H,$Z,$z,$D=0,$Fg=" "){return" $H$Z".($z!==null?$Fg."LIMIT $z":"");}function
unconvert_field($o,$J){return$J;}function
fk_support($S){}function
engines(){return
array();}function
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($T){foreach($T
...
last_id(){}function
hmac($Ba,$Gb,$y,$bg=false){$Ua=64;if(strlen($y)>$Ua)$y=pack("H*",$Ba($y));$y=str_pad($y,$Ua,"\0");$Ld=$y^str_repeat("\x36",$Ua);$Md=$y^str_repeat("\x5C",$Ua);$J=$Ba($Md.pack("H*",$Ba($Ld.$Gb)));if($bg)$J=pack("H*",$J);return$J;}function
sdb_request($va,$F=array()){global$b,$g;list($md,$F['AWSAccessKeyId'],$Bg)=$b->credentials();$F['Action']=$va;$F['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$F['Version']='2009-04-15';$F['SignatureVersion']=2;$F['SignatureMethod']='HmacSHA1';ksort($F);$H='';foreach($F
as$y=>$X)$H.='&'.rawurlencode($y).'='.rawurlencode($X);$H=str_replace('%7E','~',substr($H,1));$H.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$md)."\n/\n$H",$Bg,true)));@ini_set('track_errors',1);$Jc=@file_get_contents((preg_match('~^https?://~',$md)?$md:"http://$md"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$H,'ignore_errors'=>1,))));if(!$Jc){$g->error=$php_errormsg;return
false;}libxml_use_internal_errors(true);$_i=simplexml_load_string($Jc);if(!$_i){$n=libxml_get_last_error();$g->error=$n->message;return
false;}if($_i->Errors){$n=$_i->Errors->Error;$g->error="$n->Message ($n->Code)";return
false;}$g->error='';$rh=$va."Result";return($_i->$rh?$_i->$rh:true);}function
sdb_request_all($va,$rh,$F=array(),$_h=0){$J=array();$Zg=($_h?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$_i=sdb_request($va,$F);if(!$_i)break;foreach($_i->$rh
as$jc)$J[]=$jc;if($z&&count($J)>=$z){$_GET["next"]=$_i->NextToken;break;}if($_h&&microtime(true)-$Zg>$_h)return
false;$F['NextToken']=$_i->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($_i->NextToken);return$J;}$x="simpledb";$Ze=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Zc=array();$ed=array("count");$gc=array(array("json"));}$Yb["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$If=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$error,$last_id,$_link,$_db;function
connect($N,$V,$G){global$b;$m=$b->database();$cf=array();if($V!=""){$cf["username"]=$V;$cf["password"]=$G;}if($m!="")$cf["db"]=$m;try{$this->_link=@new
MongoClient("mongodb://$N",$cf);return
...
Min_Driver
extends
Min_SQL{public$Lf="_id";function
select($R,$M,$Z,$cd,$ef=array(),$z=1,$E=0,$Nf=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Sg=array();foreach($ef
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Bb);$Sg[$X]=($Bb?-1:1);}return
new
Min_Result($this->_conn->_db->selectCollection($R)->find(array(),$M)->sort($Sg)->limit(+$z)->skip($E*$z));}function
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf){global$g;if($R==""){$g->_db->createCollection($C);return
true;}}function
...
support($Gc){return
preg_match("~database|indexes~",$Gc);}$x="mongo";$Ze=array("=");$Zc=array();$ed=array();$gc=array(array("json"));}$Yb["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$If=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($_f,$xb=array(),$ye='GET'){@ini_set('track_errors',1);$Jc=@file_get_contents($this->_url.'/'.ltrim($_f,'/'),false,stream_context_create(array('http'=>array('method'=>$ye,'content'=>json_encode($xb),'ignore_errors'=>1,))));if(!$Jc){$this->error=$php_errormsg;return$Jc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Jc;return
...
query($_f,$xb=array(),$ye='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($_f,'/'),$xb,$ye);}function
...
extends
Min_SQL{function
select($R,$M,$Z,$cd,$ef=array(),$z=1,$E=0,$Nf=false){global$b;$Gb=array();$H="$R/_search";if($M!=array("*"))$Gb["fields"]=$M;if($ef){$Sg=array();foreach($ef
as$lb){$lb=preg_replace('~ DESC$~','',$lb,1,$Bb);$Sg[]=($Bb?array($lb=>"desc"):$lb);}$Gb["sort"]=$Sg;}if($z){$Gb["size"]=+$z;if($E)$Gb["from"]=($E*$z);}foreach($Z
as$X){list($lb,$Xe,$X)=explode(" ",$X,3);if($lb=="_id")$Gb["query"]["ids"]["values"][]=$X;elseif($lb.$X!=""){$vh=array("term"=>array(($lb!=""?$lb:"_all")=>$X));if($Xe=="=")$Gb["query"]["filtered"]["filter"]["and"][]=$vh;else$Gb["query"]["filtered"]["query"]["bool"]["must"][]=$vh;}}if($Gb["query"]&&!$Gb["query"]["filtered"]["query"]&&!$Gb["query"]["ids"])$Gb["query"]["filtered"]["query"]=array("match_all"=>array());$Zg=microtime(true);$Ag=$this->_conn->query($H,$Gb);if($Nf)echo$b->selectQuery("$H: ".print_r($Gb,true),format_time($Zg));if(!$Ag)return
false;$J=array();foreach($Ag['hits']['hits']as$ld){$K=array();if($M==array("*"))$K["_id"]=$ld["_id"];$p=$ld['_source'];if($M!=array("*")){$p=array();foreach($M
...
table_status($C="",$Fc=false){global$g;$Ag=$g->query("_search?search_type=count",array("facets"=>array("count_by_type"=>array("terms"=>array("field"=>"_type",)))),"POST");$J=array();if($Ag){foreach($Ag["facets"]["count_by_type"]["terms"]as$R){$J[$R["term"]]=array("Name"=>$R["term"],"Engine"=>"table","Rows"=>$R["count"],);if($C!=""&&$C==$R["term"])return$J[$C];}}return$J;}function
error(){global$g;return
h($g->error);}function
information_schema(){}function
is_view($S){}function
...
as$R)$J=$J&&$g->query(urlencode($R),array(),'DELETE');return$J;}$x="elastic";$Ze=array("=","query");$Zc=array();$ed=array();$gc=array(array("json"));}$Yb=array("server"=>"MySQL")+$Yb;if(!defined("DRIVER")){$If=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N="",$V="",$G="",$k=null,$Ef=null,$Rg=null){mysqli_report(MYSQLI_REPORT_OFF);list($md,$Ef)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$md:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),$k,(is_numeric($Ef)?$Ef:ini_get("mysqli.default_port")),(!is_numeric($Ef)?$Ef:$Rg));return$J;}function
...
insertUpdate($R,$L,$Lf){$e=array_keys(reset($L));$Jf="INSERT INTO ".table($R)." (".implode(", ",$e).") VALUES\n";$ni=array();foreach($e
as$y)$ni[$y]="$y = VALUES($y)";$fh="\nON DUPLICATE KEY UPDATE ".implode(", ",$ni);$ni=array();$be=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($ni&&(strlen($Jf)+$be+strlen($Y)+strlen($fh)>1e6)){if(!queries($Jf.implode(",\n",$ni).$fh))return
false;$ni=array();$be=0;}$ni[]=$Y;$be+=strlen($Y)+2;}return
queries($Jf.implode(",\n",$ni).$fh);}}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
table($u){return
idf_escape($u);}function
connect(){global$b,$Vh,$ch;$g=new
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2])){$g->set_charset(charset($g));$g->query("SET sql_quote_show_create = 1, autocommit = 1");if(version_compare($g->server_info,'5.7.8')>=0){$ch[lang(26)][]="json";$Vh["json"]=4294967295;}return$g;}$J=$g->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($xg=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$xg;return$J;}function
...
foreign_keys($R){global$g,$Ue;static$Bf='`(?:[^`]|``)+`';$J=array();$Cb=$g->result("SHOW CREATE TABLE ".table($R),1);if($Cb){preg_match_all("~CONSTRAINT ($Bf) FOREIGN KEY ?\\(((?:$Bf,? ?)+)\\) REFERENCES ($Bf)(?:\\.($Bf))? \\(((?:$Bf,? ?)+)\\)(?: ON DELETE ($Ue))?(?: ON UPDATE ($Ue))?~",$Cb,$ke,PREG_SET_ORDER);foreach($ke
as$B){preg_match_all("~$Bf~",$B[2],$Tg);preg_match_all("~$Bf~",$B[5],$sh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Tg[0]),"target"=>array_map('idf_unescape',$sh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$xf,$Ae){$c=array();foreach($p
...
queries("CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)$P$xf");if($R!=$C)$c[]="RENAME TO ".table($C);if($P)$c[]=ltrim($P);if($Ae){$Xg="";foreach($Ae
as$Ci)$Xg=$Xg."\n Modify ".$Ci.",";$Xg=substr($Xg,0,strlen($Xg)-1);queries("ALTER TABLE ".table($R).$Xg);}return($c||$xf?queries("ALTER TABLE ".table($R)."\n".implode(",\n",$c).$xf):true);}function
alter_indexes($R,$c){foreach($c
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$si,$sh){$kg=array();foreach(array_merge($T,$si)as$R)$kg[]=table($R)." TO ".idf_escape($sh).".".table($R);return
queries("RENAME TABLE ".implode(", ",$kg));}function
copy_tables($T,$si,$sh){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($T
as$R){$C=($sh==DB?table("copy_$R"):idf_escape($sh).".".table($R));if(!queries("\nDROP TABLE IF EXISTS $C")||!queries("CREATE TABLE $C LIKE ".table($R))||!queries("INSERT INTO $C SELECT * FROM ".table($R)))return
false;}foreach($si
as$R){$C=($sh==DB?table("copy_$R"):idf_escape($sh).".".table($R));$ri=view($R);if(!queries("DROP VIEW IF EXISTS $C")||!queries("CREATE VIEW $C AS $ri[select]"))return
false;}return
true;}function
trigger($C){if($C=="")return
...
routine($C,$U){global$g,$qc,$yd,$Vh;$Ca=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Uh="((".implode("|",array_merge(array_keys($Vh),$Ca)).")\\b(?:\\s*\\(((?:[^'\")]|$qc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$Bf="\\s*(".($U=="FUNCTION"?"":$yd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Uh";$i=$g->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$Bf\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Uh\\s+":"")."(.*)~is",$i,$B);$p=array();preg_match_all("~$Bf\\s*,?~is",$B[1],$ke,PREG_SET_ORDER);foreach($ke
as$rf){$C=str_replace("``","`",$rf[2]).$rf[3];$p[]=array("field"=>$C,"type"=>strtolower($rf[5]),"length"=>preg_replace_callback("~$qc~s",'normalize_enum',$rf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$rf[8] $rf[7]"))),"null"=>1,"full_type"=>$rf[4],"inout"=>strtoupper($rf[1]),"collation"=>strtolower($rf[9]),);}if($U!="FUNCTION")return
...
use_sql($k){return"USE ".idf_escape($k);}function
trigger_sql($R,$dh){$J="";foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($R,"%_\\")),null,"-- ")as$K)$J.="\n".($dh=='CREATE+ALTER'?"DROP TRIGGER IF EXISTS ".idf_escape($K["Trigger"]).";;\n":"")."CREATE TRIGGER ".idf_escape($K["Trigger"])." $K[Timing] $K[Event] ON ".table($K["Table"])." FOR EACH ROW\n$K[Statement];;\n";return$J;}function
show_variables(){return
...
selectLinks($jh,$O=""){echo'<p class="links">';$ee=array("select"=>lang(39));if(support("table")||support("indexes"))$ee["table"]=lang(40);if(support("table")){if(is_view($jh))$ee["view"]=lang(41);else$ee["create"]=lang(42);}if($O!==null)$ee["edit"]=lang(43);foreach($ee
as$y=>$X)echo" <a href='".h(ME)."$y=".urlencode($jh["Name"]).($y=="edit"?$O:"")."'".bold(isset($_GET[$y])).">$X</a>";echo"\n";}function
foreignKeys($R){return
foreign_keys($R);}function
backwardKeys($R,$ih){return
array();}function
backwardKeysPrint($Oa,$K){}function
selectQuery($H,$zh){global$x;return"<p><code class='jush-$x'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($zh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>":"")."</p>";}function
sqlCommandQuery($H){return
shorten_utf8(trim($H),1000);}function
rowDescription($R){return"";}function
rowDescriptions($L,$Sc){return$L;}function
selectLink($X,$o){}function
selectVal($X,$_,$o,$mf){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$o["type"])&&!preg_match("~var~",$o["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$o["type"])&&!is_utf8($X))$J="<i>".lang(44,strlen($mf))."</i>";if(preg_match('~json~',$o["type"]))$J="<code class='jush-js'>$J</code>";return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
...
as$C=>$v){ksort($v["columns"]);$Nf=array();foreach($v["columns"]as$y=>$X)$Nf[]="<i>".h($X)."</i>".($v["lengths"][$y]?"(".$v["lengths"][$y].")":"").($v["descs"][$y]?" DESC":"");echo"<tr title='".h($C)."'><th>$v[type]<td>".implode(", ",$Nf)."\n";}echo"</table>\n";}function
...
selectOrderPrint($ef,$e,$w){print_fieldset("sort",lang(55),$ef);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'",$e,$X),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),lang(56))."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'",$e),checkbox("desc[$s]",1,false,lang(56))."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($yh){if($yh!==null){echo"<fieldset><legend>".lang(58)."</legend><div>","<input type='number' name='text_length' class='size' value='".h($yh)."'>","</div></fieldset>\n";}}function
...
false;}function
selectQueryBuild($M,$Z,$cd,$ef,$z,$E){return"";}function
messageQuery($H,$zh){global$x;restart_session();$jd=&get_session("queries");$t="sql-".count($jd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$jd[$_GET["db"]][]=array($H,time(),$zh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$t' onclick=\"return !toggle('$t');\">".lang(61)."</a>"."<div id='$t' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($H,1000).'</code></pre>'.($zh?" <span class='time'>($zh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($jd[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';}function
editFunctions($o){global$gc;$J=($o["null"]?"NULL/":"");foreach($gc
as$y=>$Zc){if(!$y||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($Zc
as$Bf=>$X){if(!$Bf||preg_match("~$Bf~",$o["type"]))$J.="/$X";}if($y&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$J.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J=lang(48);return
explode("/",$J);}function
editInput($R,$o,$Ja,$Y){if($o["is_virtual"])return"<label $Ja>".h($Y).'</label>';else{if($o["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ja value='-1' checked><i>".lang(8)."</i></label> ":"").($o["null"]?"<label><input type='radio'$Ja value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ja,$o,$Y,0);}}function
...
dumpTable($R,$dh,$Hd=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($dh)dump_csv(array_keys(fields($R)));}else{if($Hd==2){$p=array();foreach(fields($R)as$C=>$o)$p[]=idf_escape($C)." $o[full_type]";$i="CREATE TABLE ".table($R)." (".implode(", ",$p).")";}else$i=create_sql($R,$_POST["auto_increment"]);set_utf8mb4($i);if($dh&&$i){if($dh=="DROP+CREATE"||$Hd==1)echo"DROP ".($Hd==2?"VIEW":"TABLE")." IF EXISTS ".table($R).";\n";if($Hd==1)$i=remove_definer($i);echo"$i;\n\n";}}}function
dumpData($R,$dh,$H){global$g,$x;$me=($x=="sqlite"?0:1048576);if($dh){if($_POST["format"]=="sql"){if($dh=="TRUNCATE+INSERT")echo
truncate_sql($R).";\n";$p=fields($R);}$I=$g->query($H,1);if($I){$_d="";$Xa="";$Od=array();$fh="";$Hc=($R!=''?'fetch_assoc':'fetch_row');while($K=$I->$Hc()){if(!$Od){$ni=array();foreach($K
as$X){$o=$I->fetch_field();if($p[$o->name]['is_virtual']){$vi[]=idf_escape($o->name);continue;}$Od[]=$o->name;$y=idf_escape($o->name);$ni[]="$y = VALUES($y)";}$fh=($dh=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$ni):"").";\n";}if($_POST["format"]!="sql"){if($dh=="table"){dump_csv($Od);$dh="INSERT";}dump_csv($K);}else{if(!$_d)$_d="INSERT INTO ".table($R)." (".implode(", ",array_map('idf_escape',$Od)).") VALUES";foreach($K
as$y=>$X){if(in_array("`".$y."`",(array)$vi)){unset($K[$y]);continue;}$o=$p[$y];$K[$y]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$xg=($me?"\n":" ")."(".implode(",\t",$K).")";if(!$Xa)$Xa=$_d.$xg;elseif(strlen($Xa)+4+strlen($xg)+strlen($fh)<$me)$Xa.=",$xg";else{echo$Xa.$fh;$Xa=$_d.$xg;}}}if($Xa)echo$Xa.$fh;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$g->error)."\n";}}function
dumpFilename($od){return
friendly_url($od!=""?$od:(SERVER!=""?SERVER:"localhost"));}function
dumpHeaders($od,$Be=false){$pf=$_POST["output"];$Bc=(preg_match('~sql~',$_POST["format"])?"sql":($Be?"tar":"csv"));header("Content-Type: ".($pf=="gz"?"application/x-gzip":($Bc=="tar"?"application/x-tar":($Bc=="sql"||$pf!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($pf=="gz")ob_start('ob_gzencode',1e6);return$Bc;}function
...
page_header($Bh,$n="",$Wa=array(),$Ch=""){global$ca,$ia,$b,$Yb,$x;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$Dh=$Bh.($Ch!=""?": $Ch":"");$Eh=strip_tags($Dh.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
as$y=>$X){$Pb=(is_array($X)?$X[1]:h($X));if($Pb!="")echo"<a href='".h(ME."$y=").urlencode(is_array($X)?$X[0]:$X)."'>$Pb</a> &raquo; ";}}echo"$Bh\n";}}echo"<h2>$Dh</h2>\n","<div id='ajaxstatus' class='jsonly hidden'></div>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$ei=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$we=$_SESSION["messages"][$ei];if($we){echo"<div class='message'>".implode("</div>\n<div class='message'>",$we)."</div>\n";unset($_SESSION["messages"][$ei]);}if($n)echo"<div class='error'>$n</div>\n";}function
page_footer($_e=""){global$b,$Hh;echo'</div>
...
long2str($W,$wi){$xg='';foreach($W
as$X)$xg.=pack('V',$X);if($wi)return
substr($xg,0,end($W));return$xg;}function
str2long($xg,$wi){$W=array_values(unpack('V*',str_pad($xg,4*ceil(strlen($xg)/4),"\0")));if($wi)$W[]=strlen($xg);return$W;}function
xxtea_mx($Bi,$Ai,$gh,$Kd){return
int32((($Bi>>5&0x7FFFFFF)^$Ai<<2)+(($Ai>>3&0x1FFFFFFF)^$Bi<<4))^int32(($gh^$Ai)+($Kd^$Bi));}function
encrypt_string($bh,$y){if($bh=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($bh,true);$De=count($W)-1;$Bi=$W[$De];$Ai=$W[0];$Uf=floor(6+52/($De+1));$gh=0;while($Uf-->0){$gh=int32($gh+0x9E3779B9);$fc=$gh>>2&3;for($qf=0;$qf<$De;$qf++){$Ai=$W[$qf+1];$Ce=xxtea_mx($Bi,$Ai,$gh,$y[$qf&3^$fc]);$Bi=int32($W[$qf]+$Ce);$W[$qf]=$Bi;}$Ai=$W[0];$Ce=xxtea_mx($Bi,$Ai,$gh,$y[$qf&3^$fc]);$Bi=int32($W[$De]+$Ce);$W[$De]=$Bi;}return
long2str($W,false);}function
decrypt_string($bh,$y){if($bh=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($bh,false);$De=count($W)-1;$Bi=$W[$De];$Ai=$W[0];$Uf=floor(6+52/($De+1));$gh=int32($Uf*0x9E3779B9);while($gh){$fc=$gh>>2&3;for($qf=$De;$qf>0;$qf--){$Bi=$W[$qf-1];$Ce=xxtea_mx($Bi,$Ai,$gh,$y[$qf&3^$fc]);$Ai=int32($W[$qf]-$Ce);$W[$qf]=$Ai;}$Bi=$W[$De];$Ce=xxtea_mx($Bi,$Ai,$gh,$y[$qf&3^$fc]);$Ai=int32($W[0]-$Ce);$W[0]=$Ai;$gh=int32($gh-0x9E3779B9);}return
long2str($W,true);}$g='';$id=$_SESSION["token"];if(!$id)$_SESSION["token"]=rand(1,1e6);$Hh=get_token();$Cf=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$Cf[$y]=$X;}}function
add_invalid_login(){global$b;$Kc=get_temp_dir()."/adminer.invalid";$Xc=@fopen($Kc,"r+");if(!$Xc){$Xc=@fopen($Kc,"w");if(!$Xc)return;}flock($Xc,LOCK_EX);$Cd=unserialize(stream_get_contents($Xc));$zh=time();if($Cd){foreach($Cd
as$Dd=>$X){if($X[0]<$zh)unset($Cd[$Dd]);}}$Bd=&$Cd[$b->bruteForceKey()];if(!$Bd)$Bd=array($zh+30*60,0);$Bd[1]++;$Ig=serialize($Cd);rewind($Xc);fwrite($Xc,$Ig);ftruncate($Xc,strlen($Ig));flock($Xc,LOCK_UN);fclose($Xc);}$Ka=$_POST["auth"];if($Ka){$Cd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Bd=$Cd[$b->bruteForceKey()];$Je=($Bd[1]>30?$Bd[0]-time():0);if($Je>0)auth_error(lang(78,ceil($Je/60)));session_regenerate_id();$pi=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$G=(string)$Ka["password"];$m=$Ka["db"];set_password($pi,$N,$V,$G);$_SESSION["db"][$pi][$N][$V][$m]=true;if($Ka["permanent"]){$y=base64_encode($pi)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$Of=$b->permanentLogin(true);$Cf[$y]="$y:".base64_encode($Of?encrypt_string($G,$Of):"");cookie("adminer_permanent",implode(" ",$Cf));}if(count($_POST)==1||DRIVER!=$pi||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($pi,$N,$V,$m));}elseif($_POST["logout"]){if($id&&!verify_token()){page_header(lang(77),lang(79));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(80));}}elseif($Cf&&!$_SESSION["pwds"]){session_regenerate_id();$Of=$b->permanentLogin();foreach($Cf
as$y=>$X){list(,$hb)=explode(":",$X);list($pi,$N,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($pi,$N,$V,decrypt_string(base64_decode($hb),$Of));$_SESSION["db"][$pi][$N][$V][$m]=true;}}function
unset_permanent(){global$Cf;foreach($Cf
as$y=>$X){list($pi,$N,$V,$m)=array_map('base64_decode',explode("-",$y));if($pi==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($Cf[$y]);}cookie("adminer_permanent",implode(" ",$Cf));}function
auth_error($n){global$b,$id;$Lg=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Lg]||$_GET[$Lg])&&!$id)$n=lang(81);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(82,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Lg]&&$_GET[$Lg]&&ini_bool("session.use_only_cookies"))$n=lang(83);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(84),lang(85,implode(", ",$If)),false);page_footer("auth");exit;}$g=connect();}$Xb=new
Min_Driver($g);if(!is_object($g)||($fe=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?h($g):(is_string($fe)?$fe:lang(86))));if($Ka&&$_POST["token"])$_POST["token"]=$Hh;$n='';if($_POST){if(!verify_token()){$xd="max_input_vars";$qe=ini_get($xd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$qe||$X<$qe)){$xd=$y;$qe=$X;}}}$n=(!$_POST["token"]&&$qe?lang(87,"'$xd'"):lang(79).' '.lang(88));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(89,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(90);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
select($I,$h=null,$hf=array(),$z=0){global$x;$ee=array();$w=array();$e=array();$Ta=array();$Vh=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Jd=0;$Jd<count($K);$Jd++){$o=$I->fetch_field();$C=$o->name;$gf=$o->orgtable;$ff=$o->orgname;$J[$o->table]=$gf;if($hf&&$x=="sql")$ee[$Jd]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($gf!=""){if(!isset($w[$gf])){$w[$gf]=array();foreach(indexes($gf,$h)as$v){if($v["type"]=="PRIMARY"){$w[$gf]=array_flip($v["columns"]);break;}}$e[$gf]=$w[$gf];}if(isset($e[$gf][$ff])){unset($e[$gf][$ff]);$w[$gf][$ff]=$Jd;$ee[$Jd]=$gf;}}if($o->charsetnr==63)$Ta[$Jd]=true;$Vh[$Jd]=$o->type;echo"<th".($gf!=""||$o->name!=$ff?" title='".h(($gf!=""?"$gf.":"").$ff)."'":"").">".h($C).($hf?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$y=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ta[$y]&&!is_utf8($X))$X="<i>".lang(44,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Vh[$y]==254)$X="<code>$X</code>";}if(isset($ee[$y])&&!$e[$ee[$y]]){if($hf&&$x=="sql"){$R=$K[array_search("table=",$ee)];$_=$ee[$y].urlencode($hf[$R]!=""?$hf[$R]:$R);}else{$_="edit=".urlencode($ee[$y]);foreach($w[$ee[$y]]as$lb=>$Jd)$_.="&where".urlencode("[".bracket_escape($lb)."]")."=".urlencode($K[$Jd]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
referencable_primary($Eg){$J=array();foreach(table_status('',true)as$kh=>$R){if($kh!=$Eg&&fk_support($R)){foreach(fields($kh)as$o){if($o["primary"]){if($J[$kh]){unset($J[$kh]);break;}$J[$kh]=$o;}}}}return$J;}function
...
edit_type($y,$o,$ob,$Tc=array(),$ti){global$ch,$Vh,$ci,$Ue;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),' aria-labelledby="label-type">';if($U&&!isset($Vh[$U])&&!isset($Tc[$U]))array_unshift($ch,$U);if($Tc)$ch[lang(91)]=$Tc;echo
optionlist($ch,$U),'</select>
<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';if($o["is_virtual"])textarea(h($y)."[virtual1]",$ti,$L=2,$pb=40);else{echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(92).')'.optionlist($ob,$o["collation"]).'</select>',($ci?"<select name='".h($y)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($ci,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(93).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Tc?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(94).")".optionlist(explode("|",$Ue),$o["on_delete"])."</select> ":" ");}}function
...
process_type($o,$mb="COLLATE"){global$ci;return" $o[type]".process_length($o["length"]).(preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&in_array($o["unsigned"],$ci)?" $o[unsigned]":"").(preg_match('~char|text|enum|set~',$o["type"])&&$o["collation"]?" $mb ".q($o["collation"]):"");}function
process_field($o,$Th){global$x;$Mb=$o["default"];return
array(idf_escape(trim($o["field"])),process_type($Th),($o["null"]?" NULL":" NOT NULL"),(isset($Mb)?" DEFAULT ".((preg_match('~time~',$o["type"])&&preg_match('~^CURRENT_TIMESTAMP$~i',$Mb))||($x=="sqlite"&&preg_match('~^CURRENT_(TIME|TIMESTAMP|DATE)$~i',$Mb))||($o["type"]=="bit"&&preg_match("~^([0-9]+|b'[0-1]+')\$~",$Mb))||($x=="pgsql"&&preg_match("~^[a-z]+\\(('[^']*')+\\)\$~",$Mb))?$Mb:q($Mb)):""),(preg_match('~timestamp|datetime~',$o["type"])&&$o["on_update"]?" ON UPDATE $o[on_update]":""),(support("comment")&&$o["comment"]!=""?" COMMENT ".q($o["comment"]):""),($o["auto_increment"]?auto_increment():null),);}function
type_class($U){foreach(array('char'=>'text','date'=>'time|year','binary'=>'blob','enum'=>'set',)as$y=>$X){if(preg_match("~$y|$X~",$U))return" class='$y'";}}function
edit_fields($p,$ob,$U="TABLE",$Tc=array(),$tb=false,$Mg){global$g,$yd;$p=array_values($p);echo'<thead><tr class="wrap">
...
';foreach($p
as$s=>$o){$s++;$if=$o[($_POST?"orig":"field")];$Tb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$if=="");echo'<tr',($Tb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$yd),$o["inout"]):""),'<th>';if($Tb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($if),'">
';$Vf=substr($Mg[substr($s,-2,1)-1],stripos($Mg[substr($s,-2,1)-1],"as")+3);$Uf=substr($Vf,0,strpos($Vf,"VIRTUAL"));edit_type("fields[$s]",$o,$ob,$Tc,$Uf);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }" aria-labelledby="label-ai"></label><td><?php
echo
...
grant($ad,$Qf,$e,$Te){if(!$Qf)return
true;if($Qf==array("ALL PRIVILEGES","GRANT OPTION"))return($ad=="GRANT"?queries("$ad ALL PRIVILEGES$Te WITH GRANT OPTION"):queries("$ad ALL PRIVILEGES$Te")&&queries("$ad GRANT OPTION$Te"));return
queries("$ad ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~','\\1',implode("$e, ",$Qf).$e).$Te);}function
drop_create($Zb,$i,$ac,$wh,$cc,$A,$ve,$te,$ue,$Qe,$Ge){if($_POST["drop"])query_redirect($Zb,$A,$ve);elseif($Qe=="")query_redirect($i,$A,$ue);elseif($Qe!=$Ge){$Db=queries($i);queries_redirect($A,$te,$Db&&queries($Zb));if($Db)queries($ac);}else
queries_redirect($A,$te,queries($wh)&&queries($cc)&&queries($Zb)&&queries($i));}function
create_trigger($Te,$K){global$x;$Ah=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($x=="mssql"?$Te.$Ah:$Ah.$Te).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
create_routine($tg,$K){global$yd;$O=array();$p=(array)$K["fields"];ksort($p);foreach($p
as$o){if($o["field"]!="")$O[]=(preg_match("~^($yd)\$~",$o["inout"])?"$o[inout] ":"").idf_escape($o["field"]).process_type($o,"CHARACTER SET");}return"CREATE $tg ".idf_escape(trim($K["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($K["returns"],"CHARACTER SET"):"").($K["language"]?" LANGUAGE $K[language]":"").rtrim("\n$K[definition]",";").";";}function
remove_definer($H){return
preg_replace('~^([A-Z =]+) DEFINER=`'.preg_replace('~@(.*)~','`@`(%|\\1)',logged_user()).'`~','\\1',$H);}function
format_foreign_key($q){global$Ue;return" FOREIGN KEY (".implode(", ",array_map('idf_escape',$q["source"])).") REFERENCES ".table($q["table"])." (".implode(", ",array_map('idf_escape',$q["target"])).")".(preg_match("~^($Ue)\$~",$q["on_delete"])?" ON DELETE $q[on_delete]":"").(preg_match("~^($Ue)\$~",$q["on_update"])?" ON UPDATE $q[on_update]":"");}function
tar_file($Kc,$Fh){$J=pack("a100a8a8a8a12a12",$Kc,644,0,0,decoct($Fh->size),decoct(time()));$fb=8*32;for($s=0;$s<strlen($J);$s++)$fb+=ord($J[$s]);$J.=sprintf("%06o",$fb)."\0 ";echo$J,str_repeat("\0",512-strlen($J));$Fh->send();echo
str_repeat("\0",511-($Fh->size+511)%512);}function
ini_bytes($xd){$X=ini_get($xd);switch(strtolower(substr($X,-1))){case'g':$X*=1024;case'm':$X*=1024;case'k':$X*=1024;}return$X;}function
doc_link($Af){global$x,$g;$gi=array('sql'=>"http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/",'sqlite'=>"http://www.sqlite.org/",'pgsql'=>"http://www.postgresql.org/docs/".substr($g->server_info,0,3)."/static/",'mssql'=>"http://msdn.microsoft.com/library/",'oracle'=>"http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/",);return($Af[$x]?"<a href='$gi[$x]$Af[$x]' target='_blank' rel='noreferrer'><sup>?</sup></a>":"");}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".lang(128)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(131)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(67),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$mh=array();$nh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$ke,PREG_SET_ORDER);foreach($ke
as$s=>$B){$mh[$B[1]]=array($B[2],$B[3]);$nh[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$Ih=0;$Qa=-1;$zg=array();$gg=array();$Zd=array();foreach(table_status('',true)as$R=>$S){if(is_view($S))continue;$Ff=0;$zg[$R]["fields"]=array();foreach(fields($R)as$C=>$o){$Ff+=1.25;$o["pos"]=$Ff;$zg[$R]["fields"][$C]=$o;}$zg[$R]["pos"]=($mh[$R]?$mh[$R]:array($Ih,0));foreach($b->foreignKeys($R)as$X){if(!$X["db"]){$Xd=$Qa;if($mh[$R][1]||$mh[$X["table"]][1])$Xd=min(floatval($mh[$R][1]),floatval($mh[$X["table"]][1]))-1;else$Qa-=.1;while($Zd[(string)$Xd])$Xd-=.0001;$zg[$R]["references"][$X["table"]][(string)$Xd]=array($X["source"],$X["target"]);$gg[$X["table"]][$R][(string)$Xd]=$X["target"];$Zd[(string)$Xd]=true;}}$Ih=max($Ih,$zg[$R]["pos"][0]+2.5+$Ff);}echo'<div id="schema" style="height: ',$Ih,'em;" onselectstart="return false;">
<script type="text/javascript">
var tablePos = {',implode(",",$nh)."\n",'};
...
';foreach($zg
as$C=>$R){echo"<div class='table' style='top: ".$R["pos"][0]."em; left: ".$R["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($C).'"><b>'.h($C)."</b></a>";foreach($R["fields"]as$o){$X='<span'.type_class($o["type"]).' title="'.h($o["full_type"].($o["null"]?" NULL":'')).'">'.h($o["field"]).'</span>';echo"<br>".($o["primary"]?"<i>$X</i>":$X);}foreach((array)$R["references"]as$th=>$hg){foreach($hg
as$Xd=>$dg){$Yd=$Xd-$mh[$C][1];$s=0;foreach($dg[0]as$Tg)echo"\n<div class='references' title='".h($th)."' id='refs$Xd-".($s++)."' style='left: $Yd"."em; top: ".$R["fields"][$Tg]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$Yd)."em;'></div></div>";}}foreach((array)$gg[$C]as$th=>$hg){foreach($hg
as$Xd=>$e){$Yd=$Xd-$mh[$C][1];$s=0;foreach($e
as$sh)echo"\n<div class='references' title='".h($th)."' id='refd$Xd-".($s++)."' style='left: $Yd"."em; top: ".$R["fields"][$sh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Yd)."em;'></div></div>";}}echo"\n</div>\n";}foreach($zg
as$C=>$R){foreach((array)$R["references"]as$th=>$hg){foreach($hg
as$Xd=>$dg){$ze=$Ih;$oe=-10;foreach($dg[0]as$y=>$Tg){$Gf=$R["pos"][0]+$R["fields"][$Tg]["pos"];$Hf=$zg[$th]["pos"][0]+$zg[$th]["fields"][$dg[1][$y]]["pos"];$ze=min($ze,$Gf,$Hf);$oe=max($oe,$Gf,$Hf);}echo"<div class='references' id='refl$Xd' style='left: $Xd"."em; top: $ze"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($oe-$ze)."em;'></div></div>\n";}}}echo'</div>
...
":"")."
";$g->query("SET time_zone = '+00:00';");}}$dh=$_POST["db_style"];$l=array(DB);if(DB==""){$l=$_POST["databases"];if(is_string($l))$l=explode("\n",rtrim(str_replace("\r","",$l),"\n"));}foreach((array)$l
as$m){$b->dumpDatabase($m);if($g->select_db($m)){if($Fd&&preg_match('~CREATE~',$dh)&&($i=$g->result("SHOW CREATE DATABASE ".idf_escape($m),1))){set_utf8mb4($i);if($dh=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($m).";\n";echo"$i;\n";}if($Fd){if($dh)echo
use_sql($m).";\n\n";$of="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$tg){foreach(get_rows("SHOW $tg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE $tg ".idf_escape($K["Name"]),2));set_utf8mb4($i);$of.=($dh!='DROP+CREATE'?"DROP $tg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($i);$of.=($dh!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}if($of)echo"DELIMITER ;;\n\n$of"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$si=array();foreach(table_status('',true)as$C=>$S){$R=(DB==""||in_array($C,(array)$_POST["tables"]));$Gb=(DB==""||in_array($C,(array)$_POST["data"]));if($R||$Gb){if($Bc=="tar"){$Fh=new
TmpFile;ob_start(array($Fh,'write'),1e5);}$b->dumpTable($C,($R?$_POST["table_style"]:""),(is_view($S)?2:0));if(is_view($S))$si[]=$C;elseif($Gb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Fd&&$_POST["triggers"]&&$R&&($Sh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Sh\nDELIMITER ;\n";if($Bc=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$Fh);}elseif($Fd)echo"\n";}}foreach($si
as$ri)$b->dumpTable($ri,$_POST["table_style"],1);if($Bc=="tar")echo
...
<table cellspacing="0">
';$Jb=array('','USE','DROP+CREATE','CREATE');$oh=array('','DROP+CREATE','CREATE');$Hb=array('','TRUNCATE+INSERT','INSERT');if($x=="sql")$Hb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".lang(133)."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".lang(134)."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($x=="sqlite"?"":"<tr><th>".lang(35)."<td>".html_select('db_style',$Jb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],lang(135)):"").(support("event")?checkbox("events",1,$K["events"],lang(136)):"")),"<tr><th>".lang(115)."<td>".html_select('table_style',$oh,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],lang(48)).(support("trigger")?checkbox("triggers",1,$K["triggers"],lang(130)):""),"<tr><th>".lang(137)."<td>".html_select('data_style',$Hb,$K["data_style"]),'</table>
<p><input type="submit" value="',lang(70),'">
<input type="hidden" name="token" value="',$Hh,'">

<table cellspacing="0">
';$Kf=array();if(DB!=""){$db=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$db onclick='formCheck(this, /^tables\\[/);'>".lang(115)."</label>","<th style='text-align: right;'><label class='block'>".lang(137)."<input type='checkbox' id='check-data'$db onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$si="";$ph=tables_list();foreach($ph
as$C=>$U){$Jf=preg_replace('~_.*~','',$C);$db=($a==""||$a==(substr($a,-1)=="%"?"$Jf%":$C));$Nf="<tr><td>".checkbox("tables[]",$C,$db,$C,"checkboxClick(event, this); formUncheck('check-tables');","block");if($U!==null&&!preg_match('~table~i',$U))$si.="$Nf\n";else
echo"$Nf<td align='right'><label class='block'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$db,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Kf[$Jf]++;}echo$si;if($ph)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$Jf=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$Jf%",$m,"formUncheck('check-databases');","block")."\n";$Kf[$Jf]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Mc=true;foreach($Kf
as$y=>$X){if($y!=""&&$X>1){echo($Mc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Mc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(68));echo'<p class="links"><a href="'.h(ME).'user=">'.lang(138)."</a>";$I=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ad=$I;if(!$I)$I=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ad?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$ad||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$kd=&get_session("queries");$jd=&$kd[DB];if(!$n&&$_POST["clear"]){$jd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(69):lang(61)),$n);if(!$n&&$_POST){$Xc=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$Xc=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($Xc?fread($Xc,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Uf=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$jd||reset(end($jd))!=$Uf){restart_session();$jd[]=array($Uf,time());set_session("queries",$kd);stop_session();}}$Ug="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Ob=";";$D=0;$nc=true;$h=connect();if(is_object($h)&&DB!="")$h->select_db(DB);$rb=0;$sc=array();$tf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\\*|-- |$'.($x=="pgsql"?'|\\$[^$]*\\$':'');$Jh=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$ec=$b->dumpFormat();unset($ec["sql"]);while($H!=""){if(!$D&&preg_match("~^$Ug*+DELIMITER\\s+(\\S+)~i",$H,$B)){$Ob=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Ob)."\\s*|$tf)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Vc,$Ff)=$B[0];if(!$Vc&&$Xc&&!feof($Xc))$H.=fread($Xc,1e5);else{if(!$Vc&&rtrim($H)=="")break;$D=$Ff+strlen($Vc);if($Vc&&rtrim($Vc)!=$Ob){while(preg_match('('.($Vc=='/*'?'\\*/':($Vc=='['?']':(preg_match('~^-- |^#~',$Vc)?"\n":preg_quote($Vc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$xg=$B[0][0];if(!$xg&&$Xc&&!feof($Xc))$H.=fread($Xc,1e5);else{$D=$B[0][1]+strlen($xg);if($xg[0]!="\\")break;}}}else{$nc=false;$Uf=substr($H,0,$Ff);$rb++;$Nf="<pre id='sql-$rb'><code class='jush-$x'>".$b->sqlCommandQuery($Uf)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$Ug*+ATTACH\\b~i",$Uf,$B)){echo$Nf,"<p class='error'>".lang(139)."\n";$sc[]=" <a href='#sql-$rb'>$rb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$Nf;ob_flush();flush();}$Zg=microtime(true);if($g->multi_query($Uf)&&is_object($h)&&preg_match("~^$Ug*+USE\\b~i",$Uf))$h->query($Uf);do{$I=$g->store_result();$zh=" <span class='time'>(".format_time($Zg).")</span>".(strlen($Uf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Uf))."'>".lang(10)."</a>":"");if($g->error){echo($_POST["only_errors"]?$Nf:""),"<p class='error'>".lang(140).($g->errno?" ($g->errno)":"").": ".error()."\n";$sc[]=" <a href='#sql-$rb'>$rb</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$hf=select($I,$h,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Ke=$I->num_rows;echo"<p>".($Ke?($z&&$Ke>$z?lang(141,$z):"").lang(142,$Ke):""),$zh;$t="export-$rb";$Ac=", <a href='#$t' onclick=\"return !toggle('$t');\">".lang(70)."</a><span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$ec,$xa["format"])."<input type='hidden' name='query' value='".h($Uf)."'>"." <input type='submit' name='export' value='".lang(70)."'><input type='hidden' name='token' value='$Hh'></span>\n";if($h&&preg_match("~^($Ug|\\()*+SELECT\\b~i",$Uf)&&($_c=explain($h,$Uf))){$t="explain-$rb";echo", <a href='#$t' onclick=\"return !toggle('$t');\">EXPLAIN</a>$Ac","<div id='$t' class='hidden'>\n";select($_c,$h,$hf);echo"</div>\n";}else
echo$Ac;echo"</form>\n";}}else{if(preg_match("~^$Ug*+(CREATE|DROP|ALTER)$Ug++(DATABASE|SCHEMA)\\b~i",$Uf)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($g->info)."'>".lang(143,$g->affected_rows)."$zh\n";}$Zg=microtime(true);}while($g->next_result());}$H=substr($H,$D);$D=0;}}}}if($nc)echo"<p class='message'>".lang(144)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(145,$rb-count($sc))," <span class='time'>(".format_time($Jh).")</span>\n";}elseif($sc&&$rb>1)echo"<p class='error'>".lang(140).": ".implode("",$sc)."\n";}else
echo"<p class='error'>".upload_error($H)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$xc="<input type='submit' value='".lang(146)."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$Uf=$_GET["sql"];if($_POST)$Uf=$_POST["query"];elseif($_GET["history"]=="all")$Uf=$jd;elseif($_GET["history"]!="")$Uf=$jd[$_GET["history"]][0];echo"<p>";textarea("query",$Uf,20);echo($_POST?"":"<script type='text/javascript'>focus(document.getElementsByTagName('textarea')[0]);</script>\n"),"<p>$xc\n",lang(147).": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".lang(148)."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$xc":lang(149)),"</div></fieldset>\n","<fieldset><legend>".lang(150)."</legend><div>",lang(151,"<code>adminer.sql".(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.lang(152).'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(153))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(154))."\n","<input type='hidden' name='token' value='$Hh'>\n";if(!isset($_GET["import"])&&$jd){print_fieldset("history",lang(155),$_GET["history"]!="");for($X=end($jd);$X;$X=prev($jd)){$y=key($jd);list($Uf,$zh,$ic)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$zh)."'>".@date("H:i:s",$zh)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Uf)))),80,"</code>").($ic?" <span class='time'>($ic)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(156)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(157)."</a>\n","</div></fieldset>\n";}echo'</form>
';}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$di=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(!isset($o["privileges"][$di?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($di?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$Yh=unique_array($_GET["where"],$w);$Yf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(158),$Xb->delete($a,$Yf,!$Yh));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($di){if(!$O)redirect($A);queries_redirect($A,lang(159),$Xb->update($a,$O,$Yf,!$Yh));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$Xb->insert($a,$O);$Wd=($I?last_id():0);queries_redirect($A,lang(160,($Wd?" $Wd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(isset($o["privileges"]["select"])){$Ga=convert_field($o);if($_POST["clone"]&&$o["auto_increment"])$Ga="''";if($x=="sql"&&preg_match("~enum|set~",$o["type"]))$Ga="1*".idf_escape($C);$M[]=($Ga?"$Ga AS ":"").idf_escape($C);}}$K=array();if(!support("table"))$M=array("*");if($M){$I=$Xb->select($a,$M,array($Z),$M,array(),(isset($_GET["select"])?2:1));$K=$I->fetch_assoc();if(!$K)$K=false;if(isset($_GET["select"])&&(!$K||$I->fetch_assoc()))$K=null;}}if(!support("table")&&!$p){if(!$Z){$I=$Xb->select($a,array("*"),$Z,array("*"));$K=($I?$I->fetch_assoc():false);if(!$K)$K=array($Xb->primary=>"");}if($K){foreach($K
as$y=>$X){if(!$Z)$K[$y]=null;$p[$y]=array("field"=>$y,"null"=>($y!=$Xb->primary),"auto_increment"=>($y==$Xb->primary));}}}edit_form($a,$p,$K,$di);}elseif(isset($_GET["create"])){$a=$_GET["create"];$vf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$y)$vf[$y]=$y;$fg=referencable_primary($a);$Tc=array();foreach($fg
as$kh=>$o)$Tc[str_replace("`","``",$kh)."`".str_replace("`","``",$o["field"])]=$kh;$kf=array();$S=array();if($a!=""){$kf=fields($a);$S=table_status($a);if(!$S)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(161),drop_tables(array($a)));else{$p=array();$Da=array();$hi=false;$Rc=array();$jf=reset($kf);$Aa=" FIRST";$Ae=array();foreach($K["fields"]as$y=>$o){$q=$Tc[$o["type"]];$Th=($q!==null?$fg[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($y==$K["auto_increment_col"])$o["auto_increment"]=true;$Sf=process_field($o,$Th);$Da[]=array($o["orig"],$Sf,$Aa);if($Sf!=process_field($jf,$jf)){$p[]=array($o["orig"],$Sf,$Aa);if($o["orig"]!=""||$Aa)$hi=true;}if($q!==null)$Rc[idf_escape($o["field"])]=($a!=""&&$x!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Tc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Th["field"]),'on_delete'=>$o["on_delete"],));$Aa=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$hi=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$jf=next($kf);if(!$jf)$Aa="";}if($o["virtual1"]){if(!$o["length"])$Ae[]="`".$o["field"]."` ".$o["type"]." as ".$o["virtual1"];else$Ae[]="`".$o["field"]."` ".$o["type"]."(".$o["length"].") as ".$o["virtual1"];}}$xf="";if($vf[$K["partition_by"]]){$yf=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$y=>$X){$Y=$K["partition_values"][$y];$yf[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$xf.="\nPARTITION BY $K[partition_by]($K[partition])".($yf?" (".implode(",",$yf)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$S["Create_options"]))$xf.="\nREMOVE PARTITIONING";$se=lang(162);if($a==""){cookie("adminer_engine",$K["Engine"]);$se=lang(163);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$se,alter_table($a,$C,($x=="sqlite"&&($hi||$Rc)?$Da:$p),$Rc,($K["Comment"]!=$S["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$S["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$S["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$xf,$Ae));}}page_header(($a!=""?lang(42):lang(71)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Vh["int"])?"int":(isset($Vh["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$S;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($kf
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$Yc="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Yc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$yf=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Yc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$yf[""]="";$K["partition_names"]=array_keys($yf);$K["partition_values"]=array_values($yf);}}}$ob=collations();$pc=engines();foreach($pc
...
';$tb=($_POST?$_POST["comments"]:$K["Comment"]!="");if(!$_POST&&!$tb){foreach($K["fields"]as$o){if($o["comment"]!=""){$tb=true;break;}}}$Mg=explode(",",create_sql($K["name"],""));edit_fields($K["fields"],$ob,"TABLE",$Tc,$tb,$Mg);echo'</table>
<p>
...
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}if(support("partitioning")){$wf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(167),$K["partition_by"]);echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')",1).">".optionlist(array(""=>"")+$vf,$K["partition_by"])."</select>",'(<input name="partition" value="',h($K["partition"]),'">)
',lang(168),': <input type="number" name="partitions" class="size',($wf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
';if($Lf){echo"<tr><td>PRIMARY<td>";foreach($Lf["columns"]as$y=>$d){echo
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(182).": ".h($da),$n);$tg=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$rd=array();$of=array();foreach($tg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$of[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$rd[]=$s;}if(!$n&&$_POST){$Ya=array();foreach($tg["fields"]as$y=>$o){if(in_array($y,$rd)){$X=process_input($o);if($X===false)$X="''";if(isset($of[$y]))$g->query("SET @".idf_escape($o["field"])." = $X");}$Ya[]=(isset($of[$y])?"@".idf_escape($o["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Ya).")";echo"<p><code class='jush-$x'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n";if(!$g->multi_query($H))echo"<p class='error'>".error()."\n";else{$h=connect();if(is_object($h))$h->select_db(DB);do{$I=$g->store_result();if(is_object($I))select($I,$h);else
...
';$Jd=0;foreach($K["source"]as$y=>$X){echo"<tr>","<td>".html_select("source[".(+$y)."]",array(-1=>"")+$Tg,$X,($Jd==count($K["source"])-1?"foreignAddRow(this);":1),"label-source"),"<td>".html_select("target[".(+$y)."]",$sh,$K["target"][$y],1,"label-target");$Jd++;}echo'</table>
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$Ue),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$Ue),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
<noscript><p><input type="submit" name="add" value="',lang(191),'"></noscript>
';}if($C!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Hh,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$lf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$lf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ga=" AS\n$K[select]";$A=ME."table=".urlencode($C);$se=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$lf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$se);else{$uh=$C."_adminer_".uniqid();drop_create("DROP $lf ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($uh).$Ga,"DROP $U ".table($uh),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$se,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($lf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>',lang(203),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ad,$K["INTERVAL_FIELD"]),'<tr><th>',lang(109),'<td>',html_select("STATUS",$ah,$K["STATUS"]),'<tr><th>',lang(47),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",lang(204)),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="',lang(14),'">
';if($aa!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Hh,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$tg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$uh="$K[name]_adminer_".uniqid();drop_create("DROP $tg ".idf_escape($da),create_routine($tg,$K),"DROP $tg ".idf_escape($K["name"]),create_routine($tg,array("name"=>$uh)+$K),"DROP $tg ".idf_escape($uh),substr(ME,0,-1),lang(205),lang(206),lang(207),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(208):lang(209)).": ".h($da):(isset($_GET["function"])?lang(210):lang(211))),$n);if(!$_POST&&$da!=""){$K=routine($da,$tg);$K["name"]=$da;}$ob=get_vals("SHOW CHARACTER SET");sort($ob);$ug=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(174),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
...
<input type="submit" value="',lang(14),'">
';if($da!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Hh,'">
</form>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Rh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Rh["Timing"])&&in_array($_POST["Event"],$Rh["Event"])&&in_array($_POST["Type"],$Rh["Type"])){$Te=" ON ".table($a);$Zb="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$Te:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Zb,$A,lang(222));else{if($C!="")queries($Zb);queries_redirect($A,($C!=""?lang(223):lang(224)),queries(create_trigger($Te,$_POST)));if($C!="")queries(create_trigger($Te,$K+array("Type"=>reset($Rh["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(225).": ".h($C):lang(226)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>',lang(227),'<td>',html_select("Timing",$Rh["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>',lang(228),'<td>',html_select("Event",$Rh["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Rh["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>',lang(46),'<td>',html_select("Type",$Rh["Type"],$K["Type"]),'</table>
...
';}elseif(isset($_GET["user"])){$ha=$_GET["user"];$Qf=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$K){foreach(explode(",",($K["Privilege"]=="Grant option"?"":$K["Context"]))as$zb)$Qf[$zb][$K["Privilege"]]=$K["Comment"];}$Qf["Server Admin"]+=$Qf["File access on server"];$Qf["Databases"]["Create routine"]=$Qf["Procedures"]["Create routine"];unset($Qf["Procedures"]["Create routine"]);$Qf["Columns"]=array();foreach(array("Select","Insert","Update","References")as$X)$Qf["Columns"][$X]=$Qf["Tables"][$X];unset($Qf["Server Admin"]["Usage"]);foreach($Qf["Tables"]as$y=>$X)unset($Qf["Databases"][$y]);$Fe=array();if($_POST){foreach($_POST["objects"]as$y=>$X)$Fe[$X]=(array)$Fe[$X]+(array)$_POST["grants"][$y];}$bd=array();$Re="";if(isset($_GET["host"])&&($I=$g->query("SHOW GRANTS FOR ".q($ha)."@".q($_GET["host"])))){while($K=$I->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$K[0],$B)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$B[1],$ke,PREG_SET_ORDER)){foreach($ke
as$X){if($X[1]!="USAGE")$bd["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$bd["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Re=$B[1];}}if($_POST&&!$n){$Se=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Se",ME."privileges=",lang(229));else{$He=q($_POST["user"])."@".q($_POST["host"]);$zf=$_POST["pass"];if($zf!=''&&!$_POST["hashed"]){$zf=$g->result("SELECT PASSWORD(".q($zf).")");$n=!$zf;}$Db=false;if(!$n){if($Se!=$He){$Db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $He IDENTIFIED BY PASSWORD ".q($zf));$n=!$Db;}elseif($zf!=$Re)queries("SET PASSWORD FOR $He = ".q($zf));}if(!$n){$qg=array();foreach($Fe
as$Me=>$ad){if(isset($_GET["grant"]))$ad=array_filter($ad);$ad=array_keys($ad);if(isset($_GET["grant"]))$qg=array_diff(array_keys(array_filter($Fe[$Me],'strlen')),$ad);elseif($Se==$He){$Pe=array_keys((array)$bd[$Me]);$qg=array_diff($Pe,$ad);$ad=array_diff($ad,$Pe);unset($bd[$Me]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Me,$B)&&(!grant("REVOKE",$qg,$B[2]," ON $B[1] FROM $He")||!grant("GRANT",$ad,$B[2]," ON $B[1] TO $He"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($Se!=$He)queries("DROP USER $Se");elseif(!isset($_GET["grant"])){foreach($bd
as$Me=>$qg){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Me,$B))grant("REVOKE",array_keys($qg),$B[2]," ON $B[1] FROM $He");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?lang(230):lang(231)),!$n);if($Db)$g->query("DROP USER $He");}}page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(138)),$n,array("privileges"=>array('',lang(68))));if($_POST){$K=$_POST;$bd=$Fe;}else{$K=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Re;if($Re!="")$K["hashed"]=true;$bd[(DB==""||$bd?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
as$Me=>$ad){echo'<th>'.($Me!="*.*"?"<input name='objects[$s]' value='".h($Me)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(123),"Columns"=>lang(45),"Procedures"=>lang(233),)as$zb=>$Pb){foreach((array)$Qf[$zb]as$Pf=>$sb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($sb).'">'.h($Pf);$s=0;foreach($bd
as$Me=>$ad){$C="'grants[$s][".h(strtoupper($Pf))."]'";$Y=$ad[strtoupper($Pf)];if($zb=="Server Admin"&&$Me!=(isset($bd["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(234)."<option value='0'".($Y=="0"?" selected":"").">".lang(235)."</select>";else
echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Pf=="All privileges"?" id='grants-$s-all'":($Pf=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Hh,'">
</form>
...
';}elseif(isset($_GET["replication"])){page_header(lang(110));echo"<h3>".lang(240).doc_link(array("sql"=>"show-master-status.html"))."</h3>\n";$ie=replication_status("MASTER");if(!$ie)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($ie[0]as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}$Qg=replication_status("SLAVE");if($Qg){echo"<h3>".lang(241).doc_link(array("sql"=>"show-slave-status.html"))."</h3>\n";foreach($Qg[0]as$Pg){echo"<table cellspacing='0'>\n";foreach($Pg
as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}}}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$p=fields($a);$Tc=column_foreign_keys($a);$Oe="";if($S["Oid"]){$Oe=($x=="sqlite"?"rowid":"oid");$w[]=array("type"=>"PRIMARY","columns"=>array($Oe));}parse_str($_COOKIE["adminer_import"],$ya);$rg=array();$e=array();$yh=null;foreach($p
as$y=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){if($o["is_virtual"])continue;$e[$y]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$yh=$b->selectLengthProcess();}$rg+=$o["privileges"];}list($M,$cd)=$b->selectColumnsProcess($e,$w);$Ed=count($cd)<count($M);$Z=$b->selectSearchProcess($p,$w);$ef=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$Yc=($M?implode(", ",$M):"*".($Oe?", $Oe":"")).convert_fields($e,$p,$M)."\nFROM ".table($a);$dd=($cd&&$Ed?"\nGROUP BY ".implode(", ",$cd):"").($ef?"\nORDER BY ".implode(", ",$ef):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Zh=>$K){$Ga=convert_field($p[key($K)]);$M=array($Ga?$Ga:idf_escape(key($K)));$Z[]=where_check($Zh,$p);$J=$Xb->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$yi=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$eb=array();foreach($_POST["check"]as$bb)$eb[]=where_check($bb,$p);$yi[]="((".implode(") OR (",$eb)."))";}$yi=($yi?"\nWHERE ".implode(" AND ",$yi):"");$Lf=$bi=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Lf=array_flip($v["columns"]);$bi=($M?$Lf:array());break;}}foreach((array)$bi
as$y=>$X){if(in_array(idf_escape($y),$M))unset($bi[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$bi===array())$H="SELECT $Yc$yi$dd";else{$Xh=array();foreach($_POST["check"]as$X)$Xh[]="(SELECT".limit($Yc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$dd,1).")";$H=implode(" UNION ALL ",$Xh);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Tc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($e
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($bi===array()&&is_array($_POST["check"]))||$Ed){$I=($_POST["delete"]?$Xb->delete($a,$yi):($_POST["clone"]?queries("INSERT $H$yi"):$Xb->update($a,$O,$yi)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$xi="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$Xb->delete($a,$xi,1):($_POST["clone"]?queries("INSERT".limit1($H,$xi)):$Xb->update($a,$O,$xi)));if(!$I)break;$za+=$g->affected_rows;}}}$se=lang(242,$za);if($_POST["clone"]&&$I&&$za==1){$Wd=last_id();if($Wd)$se=lang(160," $Wd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$se,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(243);else{$I=true;$za=0;foreach($_POST["val"]as$Zh=>$K){$O=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$I=$Xb->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Zh,$p),!($Ed||$bi===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(242,$za),$I);}}elseif(!is_string($Jc=get_file("csv_file",true)))$n=upload_error($Jc);elseif(!preg_match('~~u',$Jc))$n=lang(244);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$pb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Jc,$ke);$za=count($ke[0]);$Xb->begin();$Fg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($ke[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Fg]*)$Fg~",$X.$Fg,$le);if(!$y&&!array_diff($le[1],$pb)){$pb=$le[1];$za--;}else{$O=array();foreach($le[1]as$s=>$lb)$O[idf_escape($pb[$s])]=($lb==""&&$p[$pb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$lb))));$L[]=$O;}}$I=(!$L||$Xb->insertUpdate($a,$L,$Lf));if($I)$I=$Xb->commit();queries_redirect(remove_from_uri("page"),lang(245,$za),$I);$Xb->rollback();}}}$kh=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(50).": $kh",$n);$O=null;if(isset($rg["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Tc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e&&support("table"))echo"<p class='error'>".lang(246).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$w);$b->selectOrderPrint($ef,$e,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($yh);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Wc=$g->result(count_rows($a,$Z,$Ed,$cd));$E=floor(max(0,$Wc-1)/$z);}$Cg=$M;if(!$Cg){$Cg[]="*";if($Oe)$Cg[]=$Oe;}$_b=convert_fields($e,$p,$M);if($_b)$Cg[]=substr($_b,2);$I=$Xb->select($a,$Cg,$Z,$cd,$ef,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$I->seek($z*$E);$mc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$cd&&$Ed&&$x=="sql")$Wc=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Pa=$b->backwardKeys($a,$kh);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$cd&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(247)."</a>");$Ee=array();$Zc=array();reset($M);$ag=1;foreach($L[0]as$y=>$X){if($y!=$Oe){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$y];$C=($o?$b->fieldName($o,$ag):($X["fun"]?"*":$y));if($C!=""){$ag++;$Ee[$y]=$C;$d=idf_escape($y);$nd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Pb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($nd.($ef[0]==$d||$ef[0]==$y||(!$ef&&$Ed&&$cd[0]==$d)?$Pb:'')).'">';echo
...
as$K){foreach($K
as$y=>$X)$ce[$y]=max($ce[$y],min(40,strlen(utf8_decode($X))));}}echo($Pa?"<th>".lang(248):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Tc)as$De=>$K){$Yh=unique_array($L[$De],$w);if(!$Yh){$Yh=array();foreach($L[$De]as$y=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$y))$Yh[$y]=$X;}}$Zh="";foreach($Yh
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x=='sql'&&preg_match("~^utf8_~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$Zh.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$cd&&$M?"":"<td>".checkbox("check[]",substr($Zh,1),in_array(substr($Zh,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Ed||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Zh)."'>".lang(249)."</a>"));foreach($K
as$y=>$X){if(isset($Ee[$y])){$o=$p[$y];if($X!=""&&(!isset($mc[$y])||$mc[$y]!=""))$mc[$y]=(is_mail($X)?$Ee[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$Zh;if(!$_&&$X!==null){foreach((array)$Tc[$y]as$q){if(count($Tc[$y])==1||end($q["source"])==$y){$_="";foreach($q["source"]as$s=>$Tg)$_.=where_link($s,$q["target"][$s],$L[$De][$Tg]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if($q["ns"])$_=preg_replace('~([?&]ns=)[^&]+~','\\1'.urlencode($q["ns"]),$_);if(count($q["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Yh))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($Yh
as$Kd=>$W)$_.=where_link($s++,$Kd,$W);}$X=select_value($X,$_,$o,$yh);$t=h("val[$Zh][".bracket_escape($y)."]");$Y=$_POST["val"][$Zh][bracket_escape($y)];$hc=!is_array($K[$y])&&is_utf8($X)&&$L[$De][$y]==$K[$y]&&!$Zc[$y];$xh=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$hc)||$Y!==null){$fd=h($Y!==null?$Y:$K[$y]);echo"<td>".($xh?"<textarea name='$t' cols='30' rows='".(substr_count($K[$y],"\n")+1)."'>$fd</textarea>":"<input name='$t' value='$fd' size='$ce[$y]'>");}else{$ge=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($ge?2:($xh?1:0)).($hc?"":", '".h(lang(250))."'").");\">$X";}}}if($Pa)echo"<td>";$b->backwardKeysPrint($Pa,$L[$De]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$wc=true;if($_GET["page"]!="last"){if(!+$z)$Wc=count($L);elseif($x!="sql"||!$Ed){$Wc=($Ed?false:found_rows($S,$Z));if($Wc<max(1e4,2*($E+1)*$z))$Wc=reset(slow_query(count_rows($a,$Z,$Ed,$cd)));else$wc=false;}}if(+$z&&($Wc===false||$Wc>$z||$E)){echo"<p class='pages'>";$ne=($Wc===false?$E+(count($L)>=$z?2:1):floor(($Wc-1)/$z));if($x!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(251)."', '".($E+1)."'), event); return false;\">".lang(251)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($ne,$E+5);$s++)echo
