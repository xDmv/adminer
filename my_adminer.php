...
remove_slashes($Vf,$Oc=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($Vf)){foreach($X
as$Nd=>$W){unset($Vf[$y][$Nd]);if(is_array($W)){$Vf[$y][stripslashes($Nd)]=$W;$Vf[]=&$Vf[$y][stripslashes($Nd)];}else$Vf[$y][stripslashes($Nd)]=($Oc?$W:stripslashes($W));}}}}function
...
nl_br($Q){return
str_replace("\n","<br>",$Q);}function
checkbox($C,$Y,$fb,$Vd="",$af="",$kb="",$Wd=""){$J="<input type='checkbox' name='$C' value='".h($Y)."'".($fb?" checked":"").($Wd?" aria-labelledby='$Wd'":"").($af?' onclick="'.h($af).'"':'').">";return($Vd!=""||$kb?"<label".($kb?" class='$kb'":"").">$J".h($Vd)."</label>":$J);}function
optionlist($gf,$Gg=null,$ni=false){$J="";foreach($gf
as$Nd=>$W){$hf=array($Nd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Nd).'">';$hf=$W;}foreach($hf
as$y=>$X)$J.='<option'.($ni||is_string($y)?' value="'.h($y).'"':'').(($ni||is_string($y)?(string)$y:$X)===$Gg?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$gf,$Y="",$Ze=true,$Wd=""){if($Ze)return"<select name='".h($C)."'".(is_string($Ze)?' onchange="'.h($Ze).'"':"").($Wd?" aria-labelledby='$Wd'":"").">".optionlist($gf,$Y)."</select>";$J="";foreach($gf
as$y=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($y)."'".($y==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ka,$gf,$Y="",$Hf=""){return($gf?"<select$Ka><option value=''>$Hf".optionlist($gf,$Y,true)."</select>":"<input$Ka size='10' value='".h($Y)."' placeholder='$Hf'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($t,$ee,$zi=false,$af=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($af)."return !toggle('fieldset-$t');\">$ee</a></legend><div id='fieldset-$t'".($zi?"":" class='hidden'").">\n";}function
...
where($db,$p);}function
where_link($s,$d,$Y,$cf="="){return"&where%5B$s%5D%5Bcol%5D=".urlencode($d)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y!==null?$cf:"IS NULL"))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);}function
...
cookie($C,$Y,$he=2592000){global$ba;return
header("Set-Cookie: $C=".urlencode($Y).($he?"; expires=".gmdate("D, d M Y H:i:s",time()+$he)." GMT":"")."; path=".preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]).($ba?"; secure":"")."; HttpOnly; SameSite=lax");}function
restart_session(){if(!ini_bool("session.use_cookies"))session_start();}function
stop_session(){if(!ini_bool("session.use_cookies"))session_write_close();}function&get_session($y){return$_SESSION[$y][DRIVER][SERVER][$_GET["username"]];}function
set_session($y,$X){$_SESSION[$y][DRIVER][SERVER][$_GET["username"]]=$X;}function
auth_url($ui,$N,$V,$m=null){global$bc;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($bc))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($ui!="server"||$N!=""?urlencode($ui)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
is_ajax(){return($_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest");}function
redirect($A,$we=null){if($we!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$we;}if($A!==null){if($A=="")$A=".";header("Location: $A");exit;}}function
query_redirect($H,$A,$we,$fg=true,$_c=true,$Hc=false,$Ch=""){global$g,$n,$b;if($_c){$dh=microtime(true);$Hc=!$g->query($H);$Ch=format_time($dh);}$ah="";if($H)$ah=$b->messageQuery($H,$Ch);if($Hc){$n=error().$ah;return
false;}if($fg)redirect($A,$we.$ah);return
true;}function
...
false;}return
true;}function
queries_redirect($A,$we,$fg){list($Zf,$Ch)=queries(null);return
query_redirect($Zf,$A,$we,$fg,false,!$fg,$Ch);}function
format_time($dh){return
lang(1,max(0,microtime(true)-$dh));}function
remove_from_uri($vf=""){return
substr(preg_replace("~(?<=[?&])($vf".(SID?"":"|".session_name()).")=[^&]*&~",'',"$_SERVER[REQUEST_URI]&"),0,-1);}function
...
upload_error($n){$te=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($te?" ".lang(3,$te):""):lang(4));}function
repeat_pattern($Ff,$fe){return
str_repeat("$Ff{0,65535}",$fe/65535)."$Ff{0,".($fe%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($Q,$fe=80,$jh=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{10FFFF}]",$fe).")($)?)u",$Q,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$fe).")($)?)",$Q,$B);return
h($B[1]).$jh.(isset($B[2])?"":"<i>...</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Vf,$td=array()){while(list($y,$X)=each($Vf)){if(!in_array($y,$td)){if(is_array($X)){foreach($X
as$Nd=>$W)$Vf[$y."[$Nd]"]=$W;}else
...
enum_input($U,$Ka,$o,$Y,$qc=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$oe);$J=($qc!==null?"<label><input type='$U'$Ka value='$qc'".((is_array($Y)?in_array($qc,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($oe[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$fb=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Ka value='".($s+1)."'".($fb?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$J;}function
input($o,$Y,$r){global$g,$ai,$b,$x;$C=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$r){$Ea=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$Ea[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$Ea);$r="json";}$og=($x=="mssql"&&$o["auto_increment"]);if($og&&!$_POST["save"])$r=null;$cd=(isset($_GET["select"])||$og?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ka=" name='fields[$C]'";if($o["type"]=="enum")echo
nbsp($cd[""])."<td>".$b->editInput($_GET["edit"],$o,$Ka,$Y);else{$Pc=0;foreach($cd
as$y=>$X){if($y===""||!$X)break;$Pc++;}$Ze=($Pc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Pc > f.selectedIndex) f.selectedIndex = $Pc;\" onkeyup='keyupChange.call(this);'":"");$Ka.=$Ze;$kd=(in_array($r,$cd)||isset($cd[$r]));echo(count($cd)>1?"<select name='function[$C]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($cd,$r===null||$kd?$r:"")."</select>":nbsp(reset($cd))).'<td>';$Bd=$b->editInput($_GET["edit"],$o,$Ka,$Y);if($Bd!="")echo$Bd;elseif(preg_match('~bool~',$o["type"]))echo"<input type='hidden'$Ka value='0'>"."<input type='checkbox'".(in_array(strtolower($Y),array('1','t','true','y','yes','on'))?" checked='checked'":"")."$Ka value='1'>";elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$oe);foreach($oe[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$fb=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($fb?' checked':'')."$Ze>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$C'$Ze>";elseif(($Ah=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($Ah&&$x!="sqlite")$Ka.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Ka.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Ka>".h($Y).'</textarea>';}elseif($r=="json"||preg_match('~^jsonb?$~',$o["type"]))echo"<textarea$Ka cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$ve=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$B)?((preg_match("~binary~",$o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($ai[$o["type"]]?$ai[$o["type"]]+($o["unsigned"]?0:1):0));if($x=='sql'&&$g->server_info>=5.6&&preg_match('~time~',$o["type"]))$ve+=7;echo"<input".((!$kd||$r==="")&&preg_match('~(?<!o)int~',$o["type"])&&!preg_match('~\[\]~',$o["full_type"])?" type='number'":"")." value='".h($Y)."'".($ve?" data-maxlength='$ve'":"").(preg_match('~char|binary~',$o["type"])&&$ve>20?" size='40'":"")."$Ka>";}}}function
...
dump_headers($rd,$Fe=false){global$b;$J=$b->dumpHeaders($rd,$Fe);$tf=$_POST["output"];if($tf!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($rd).".$J".($tf!="file"&&!preg_match('~[^0-9a-z]~',$tf)?".$tf":""));session_write_close();ob_flush();flush();return$J;}function
...
as$Nd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Nd):"")."<td>".select_value($W,$_,$o,$Bh);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="mailto:$X";if($Xf=is_url($X))$_=(($Xf=="http"&&$ba)||preg_match('~WebKit|Firefox~i',$_SERVER["HTTP_USER_AGENT"])?$X:"https://www.adminer.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$o);if($J!==null){if($J==="")$J="&nbsp;";elseif(!is_utf8($J))$J="\0";elseif($Bh!=""&&is_shortable($o))$J=shorten_utf8($J,max(0,+$Bh));else$J=h($J);}return$b->selectVal($J,$_,$o,$X);}function
is_mail($nc){$Ia='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Zb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$Ff="$Ia+(\\.$Ia+)*@($Zb?\\.)+$Zb";return
is_string($nc)&&preg_match("(^$Ff(,\\s*$Ff)*\$)i",$nc);}function
...
slow_query($H){global$b,$Lh;$m=$b->database();$Dh=$b->queryTimeout();if(support("kill")&&is_object($h=connect())&&($m==""||$h->select_db($m))){$Td=$h->result(connection_id());echo'<script type="text/javascript">
...
as$s=>$lb){$mc=$Ub[$lb];if(!isset($mc))$mc=$Di.$Di[0];$J.=$mc;if($s)$Ub[]=$Di.$mc[0];$Di=$mc;}return$J;}function
on_help($tb,$Rg=0){return" onmouseover='helpMouseover(this, event, ".h($tb).", $Rg);' onmouseout='helpMouseout(this, event);'";}function
...
select($R,$M,$Z,$fd,$if=array(),$z=1,$E=0,$Rf=false){global$b,$x;$Hd=(count($fd)<count($M));$H=$b->selectQueryBuild($M,$Z,$fd,$if,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$fd&&$Hd&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($fd&&$Hd?"\nGROUP BY ".implode(", ",$fd):"").($if?"\nORDER BY ".implode(", ",$if):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");$dh=microtime(true);$J=$this->_conn->query($H);if($Rf)echo$b->selectQuery($H,format_time($dh));return$J;}function
delete($R,$ag,$z=0){$H="FROM ".table($R);return
queries("DELETE".($z?limit1($H,$ag):" $H$ag"));}function
update($R,$O,$ag,$z=0,$Ig="\n"){$si=array();foreach($O
as$y=>$X)$si[]="$y = $X";$H=table($R)." SET$Ig".implode(",$Ig",$si);return
queries("UPDATE".($z?limit1($H,$ag):" $H$ag"));}function
insert($R,$O){return
queries("INSERT INTO ".table($R).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":" DEFAULT VALUES"));}function
insertUpdate($R,$L,$Pf){return
false;}function
begin(){return
...
rollback(){return
queries("ROLLBACK");}}$bc["sqlite"]="SQLite 3";$bc["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$Mf=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($Nc){$this->_link=new
SQLite3($Nc);$vi=$this->_link->version();$this->server_info=$vi["versionString"];}function
...
query($H,$bi=false){$Be=($bi?"unbufferedQuery":"query");$I=@$this->_link->$Be($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
fetch_field(){$C=$this->_result->fieldName($this->_offset++);$Ff='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($Ff\\.)?$Ff\$~",$C,$B)){$R=($B[3]!=""?$B[3]:idf_unescape($B[2]));$C=($B[5]!=""?$B[5]:idf_unescape($B[4]));}return(object)array("name"=>$C,"orgname"=>$C,"orgtable"=>$R,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
extends
Min_SQL{function
insertUpdate($R,$L,$Pf){$si=array();foreach($L
...
fields($R){global$g;$J=array();$Pf="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Pb=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Pb,$B)?str_replace("''","'",$B[1]):($Pb=="NULL"?null:$Pb)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Pf!="")$J[$Pf]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Pf=$C;}}$ah=$g->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$ah,$oe,PREG_SET_ORDER);foreach($oe
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$ah=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$ah,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$oe,PREG_SET_ORDER);foreach($oe
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($R)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$bh=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$h);foreach(get_rows("PRAGMA index_list(".table($R).")",$h)as$K){$C=$K["name"];$v=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$v["lengths"]=array();$v["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$h)as$zg){$v["columns"][]=$zg["name"];$v["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$bh[$C],$lg)){preg_match_all('/("[^"]*+")+( DESC)?/',$lg[2],$oe);foreach($oe[2]as$y=>$X){if($X)$v["descs"][$y]='1';}}if(!$J[""]||$v["type"]!="UNIQUE"||$v["columns"]!=$J[""]["columns"]||$v["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$v;}return$J;}function
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){$mi=($R==""||$Uc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$mi=true;break;}}$c=array();$rf=array();foreach($p
as$o){if($o[1]){$c[]=($mi?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$rf[$o[0]]=$o[1][0];}}if(!$mi){foreach($c
as$X){if(!queries("ALTER TABLE ".table($R)." $X"))return
false;}if($R!=$C&&!queries("ALTER TABLE ".table($R)." RENAME TO ".table($C)))return
false;}elseif(!recreate_table($R,$C,$c,$rf,$Uc))return
false;if($Ma)queries("UPDATE sqlite_sequence SET seq = $Ma WHERE name = ".q($C));return
true;}function
recreate_table($R,$C,$p,$rf,$Uc,$w=array()){if($R!=""){if(!$p){foreach(fields($R)as$y=>$o){$p[]=process_field($o,$o);$rf[$y]=idf_escape($y);}}$Qf=false;foreach($p
as$o){if($o[6])$Qf=true;}$ec=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$ec[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$Rd=>$v){$e=array();foreach($v["columns"]as$y=>$d){if(!$rf[$d])continue
2;$e[]=$rf[$d].($v["descs"][$y]?" DESC":"");}if(!$ec[$Rd]){if($v["type"]!="PRIMARY"||!$Qf)$w[]=array($v["type"],$Rd,$e);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$Uc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($R)as$Rd=>$q){foreach($q["source"]as$y=>$d){if(!$rf[$d])continue
2;$q["source"][$y]=idf_unescape($rf[$d]);}if(!isset($Uc[" $Rd"]))$Uc[]=" ".format_foreign_key($q);}queries("BEGIN");}foreach($p
as$y=>$o)$p[$y]="  ".implode($o);$p=array_merge($p,array_filter($Uc));if(!queries("CREATE TABLE ".table($R!=""?"adminer_$C":$C)." (\n".implode(",\n",$p)."\n)"))return
false;if($R!=""){if($rf&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ",$rf).") SELECT ".implode(", ",array_map('idf_escape',array_keys($rf)))." FROM ".table($R)))return
...
array("Statement"=>"BEGIN\n\t;\nEND");$u='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$Vh=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|",$Vh["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$g->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$Re=$B[3];return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($Re?" OF":""),"Of"=>($Re[0]=='`'||$Re[0]=='"'?idf_unescape($Re):$Re),"Trigger"=>$C,"Statement"=>$B[4],);}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$ff){list($y,$X)=explode("=",$ff,2);$J[$y]=$X;}return$J;}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Jc){return
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$Jc);}$x="sqlite";$ai=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$gh=array_keys($ai);$hi=array();$df=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$cd=array("hex","length","lower","round","unixepoch","upper");$hd=array("avg","count","count distinct","group_concat","max","min","sum");$jc=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$bc["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$Mf=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
extends
Min_SQL{function
insertUpdate($R,$L,$Pf){global$g;foreach($L
as$O){$ii=array();$Z=array();foreach($O
as$y=>$X){$ii[]="$y = $X";if(isset($Pf[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$ii)." WHERE ".implode(" AND ",$Z))&&$g->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
...
AND a.attnum > 0
ORDER BY a.attnum")as$K){preg_match('~([^([]+)(\((.*)\))?([a-z ]+)?((\[[0-9]*])*)$~',$K["full_type"],$B);list(,$U,$fe,$K["length"],$wa,$Fa)=$B;$K["length"].=$Fa;$eb=$U.$wa;if(isset($Ca[$eb])){$K["type"]=$Ca[$eb];$K["full_type"]=$K["type"].$fe.$Fa;}else{$K["type"]=$U;$K["full_type"]=$K["type"].$fe.$wa.$Fa;}$K["null"]=!$K["attnotnull"];$K["auto_increment"]=preg_match('~^nextval\\(~i',$K["default"]);$K["privileges"]=array("insert"=>1,"select"=>1,"update"=>1);if(preg_match('~(.+)::[^)]+(.*)~',$K["default"],$B))$K["default"]=($B[1][0]=="'"?idf_unescape($B[1]):$B[1]).$B[2];$J[$K["field"]]=$K;}return$J;}function
...
foreign_keys($R){global$Ye;$J=array();foreach(get_rows("SELECT conname, condeferrable::int AS deferrable, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
WHERE conrelid = (SELECT pc.oid FROM pg_class AS pc INNER JOIN pg_namespace AS pn ON (pn.oid = pc.relnamespace) WHERE pc.relname = ".q($R)." AND pn.nspname = current_schema())
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$B)){$K['source']=array_map('trim',explode(',',$B[1]));if(preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~',$B[2],$ne)){$K['ns']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$ne[2]));$K['table']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$ne[4]));}$K['target']=array_map('trim',explode(',',$B[3]));$K['on_delete']=(preg_match("~ON DELETE ($Ye)~",$B[4],$ne)?$ne[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($Ye)~",$B[4],$ne)?$ne[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){$c=array();$Zf=array();foreach($p
as$o){$d=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $d";else{$ri=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($d!=$X[0])$Zf[]="ALTER TABLE ".table($R)." RENAME $d TO $X[0]";$c[]="ALTER $d TYPE$X[1]";if(!$X[6]){$c[]="ALTER $d ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $d ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$ri!="")$Zf[]="COMMENT ON COLUMN ".table($R).".$X[0] IS ".($ri!=""?substr($ri,9):"''");}}$c=array_merge($c,$Uc);if($R=="")array_unshift($Zf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Zf,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""&&$R!=$C)$Zf[]="ALTER TABLE ".table($R)." RENAME TO ".table($C);if($R!=""||$vb!="")$Zf[]="COMMENT ON TABLE ".table($C)." IS ".q($vb);if($Ma!=""){}foreach($Zf
...
set_schema($Bg){global$g,$ai,$gh;$J=$g->query("SET search_path TO ".idf_escape($Bg));foreach(types()as$U){if(!isset($ai[$U])){$ai[$U]=0;$gh[lang(23)][]=$U;}}return$J;}function
...
as$Lc=>$o){$yf=idf_escape($o['field']).' '.$o['full_type'].(is_null($o['default'])?"":" DEFAULT $o[default]").($o['attnotnull']?"":" NOT NULL");$sg[]=$yf;if(preg_match('~nextval\(\'([^\']+)\'\)~',$o['default'],$oe)){$Jg=$oe[1];$Zg=reset(get_rows("SELECT * FROM $Jg"));$Kg[]="CREATE SEQUENCE $Jg INCREMENT $Zg[increment_by] MINVALUE $Zg[min_value] MAXVALUE $Zg[max_value] START ".($Ma?$Zg['last_value']:1)." CACHE $Zg[cache_value];";}}if(!empty($Kg))$J=implode("\n\n",$Kg)."\n\n$J";foreach($w
...
fk_support($S){return
true;}function
fields($R){$J=array();foreach(get_rows("SELECT * FROM all_tab_columns WHERE table_name = ".q($R)." ORDER BY column_id")as$K){$U=$K["DATA_TYPE"];$fe="$K[DATA_PRECISION],$K[DATA_SCALE]";if($fe==",")$fe=$K["DATA_LENGTH"];$J[$K["COLUMN_NAME"]]=array("field"=>$K["COLUMN_NAME"],"full_type"=>$U.($fe?"($fe)":""),"type"=>strtolower($U),"length"=>$fe,"default"=>$K["DATA_DEFAULT"],"null"=>($K["NULLABLE"]=="Y"),"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),);}return$J;}function
...
found_rows($S,$Z){}function
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){$c=$cc=array();foreach($p
...
unconvert_field($o,$J){return$J;}function
support($Jc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Jc);}$x="oracle";$ai=array();$gh=array();foreach(array(lang(24)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(25)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(26)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$ai+=$X;$gh[$y]=array_keys($X);}$hi=array();$df=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$cd=array("length","lower","round","upper");$hd=array("avg","count","count distinct","max","min","sum");$jc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$bc["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Mf=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
extends
Min_SQL{function
insertUpdate($R,$L,$Pf){foreach($L
as$O){$ii=array();$Z=array();foreach($O
as$y=>$X){$ii[]="$y = $X";if(isset($Pf[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$ii)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
WHERE o.schema_id = SCHEMA_ID(".q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($R))as$K){$U=$K["type"];$fe=(preg_match("~char|binary~",$U)?$K["max_length"]:($U=="decimal"?"$K[precision],$K[scale]":""));$J[$K["name"]]=array("field"=>$K["name"],"full_type"=>$U.($fe?"($fe)":""),"type"=>$U,"length"=>$fe,"default"=>$K["default"],"null"=>$K["is_nullable"],"auto_increment"=>$K["is_identity"],"collation"=>$K["collation_name"],"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),"primary"=>$K["is_identity"],);}return$J;}function
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){$c=array();foreach($p
...
true;}function
support($Jc){return
preg_match("~^(columns|sql|status|table)$~",$Jc);}$x="firebird";$df=array("=");$cd=array();$hd=array();$jc=array();}$bc["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Mf=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
...
fetch_field(){$Sd=array_keys($this->_rows[0]);return(object)array('name'=>$Sd[$this->_offset++]);}}}class
Min_Driver
extends
Min_SQL{public$Pf="itemName()";function
_chunkRequest($sd,$va,$F,$Bc=array()){global$g;foreach(array_chunk($sd,25)as$ib){$wf=$F;foreach($ib
as$s=>$t){$wf["Item.$s.ItemName"]=$t;foreach($Bc
as$y=>$X)$wf["Item.$s.$y"]=$X;}if(!sdb_request($va,$wf))return
false;}$g->affected_rows=count($sd);return
true;}function
_extractIds($R,$ag,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$ag,$oe))$J=array_map('idf_unescape',$oe[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$ag.($z?" LIMIT 1":"")))as$Ld)$J[]=$Ld->Name;}return$J;}function
select($R,$M,$Z,$fd,$if=array(),$z=1,$E=0,$Rf=false){global$g;$g->next=$_GET["next"];$J=parent::select($R,$M,$Z,$fd,$if,$z,$E,$Rf);$g->next=0;return$J;}function
...
table_status($C="",$Ic=false){$J=array();foreach(($C!=""?array($C=>true):tables_list())as$R=>$U){$K=array("Name"=>$R,"Auto_increment"=>"");if(!$Ic){$Ae=sdb_request('DomainMetadata',array('DomainName'=>$R));if($Ae){foreach(array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$y=>$X)$K[$y]=(string)$Ae->$X;}}if($C!="")return$K;$J[$R]=$K;}return$J;}function
...
engines(){return
array();}function
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($T){foreach($T
...
false;}libxml_use_internal_errors(true);$Ei=simplexml_load_string($Mc);if(!$Ei){$n=libxml_get_last_error();$g->error=$n->message;return
false;}if($Ei->Errors){$n=$Ei->Errors->Error;$g->error="$n->Message ($n->Code)";return
false;}$g->error='';$vh=$va."Result";return($Ei->$vh?$Ei->$vh:true);}function
sdb_request_all($va,$vh,$F=array(),$Dh=0){$J=array();$dh=($Dh?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$Ei=sdb_request($va,$F);if(!$Ei)break;foreach($Ei->$vh
as$mc)$J[]=$mc;if($z&&count($J)>=$z){$_GET["next"]=$Ei->NextToken;break;}if($Dh&&microtime(true)-$dh>$Dh)return
false;$F['NextToken']=$Ei->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($Ei->NextToken);return$J;}$x="simpledb";$df=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$cd=array();$hd=array("count");$jc=array(array("json"));}$bc["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Mf=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$error,$last_id,$_link,$_db;function
connect($N,$V,$G){global$b;$m=$b->database();$gf=array();if($V!=""){$gf["username"]=$V;$gf["password"]=$G;}if($m!="")$gf["db"]=$m;try{$this->_link=@new
MongoClient("mongodb://$N",$gf);return
...
fetch_row(){$J=$this->fetch_assoc();if(!$J)return$J;return
array_values($J);}function
fetch_field(){$Sd=array_keys($this->_rows[0]);$C=$Sd[$this->_offset++];return(object)array('name'=>$C,'charsetnr'=>$this->_charset[$C],);}}}class
Min_Driver
extends
Min_SQL{public$Pf="_id";function
select($R,$M,$Z,$fd,$if=array(),$z=1,$E=0,$Rf=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Wg=array();foreach($if
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Eb);$Wg[$X]=($Eb?-1:1);}return
new
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af){global$g;if($R==""){$g->_db->createCollection($C);return
true;}}function
...
support($Jc){return
preg_match("~database|indexes~",$Jc);}$x="mongo";$df=array("=");$cd=array();$hd=array();$jc=array(array("json"));}$bc["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Mf=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($Df,$_b=array(),$Be='GET'){@ini_set('track_errors',1);$Mc=@file_get_contents($this->_url.'/'.ltrim($Df,'/'),false,stream_context_create(array('http'=>array('method'=>$Be,'content'=>json_encode($_b),'ignore_errors'=>1,))));if(!$Mc){$this->error=$php_errormsg;return$Mc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Mc;return
...
query($Df,$_b=array(),$Be='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($Df,'/'),$_b,$Be);}function
...
extends
Min_SQL{function
select($R,$M,$Z,$fd,$if=array(),$z=1,$E=0,$Rf=false){global$b;$Jb=array();$H="$R/_search";if($M!=array("*"))$Jb["fields"]=$M;if($if){$Wg=array();foreach($if
as$nb){$nb=preg_replace('~ DESC$~','',$nb,1,$Eb);$Wg[]=($Eb?array($nb=>"desc"):$nb);}$Jb["sort"]=$Wg;}if($z){$Jb["size"]=+$z;if($E)$Jb["from"]=($E*$z);}foreach($Z
as$X){list($nb,$bf,$X)=explode(" ",$X,3);if($nb=="_id")$Jb["query"]["ids"]["values"][]=$X;elseif($nb.$X!=""){$zh=array("term"=>array(($nb!=""?$nb:"_all")=>$X));if($bf=="=")$Jb["query"]["filtered"]["filter"]["and"][]=$zh;else$Jb["query"]["filtered"]["query"]["bool"]["must"][]=$zh;}}if($Jb["query"]&&!$Jb["query"]["filtered"]["query"]&&!$Jb["query"]["ids"])$Jb["query"]["filtered"]["query"]=array("match_all"=>array());$dh=microtime(true);$Dg=$this->_conn->query($H,$Jb);if($Rf)echo$b->selectQuery("$H: ".print_r($Jb,true),format_time($dh));if(!$Dg)return
...
fields($R){global$g;$I=$g->query("$R/_mapping");$J=array();if($I){$le=$I[$R]['properties'];if(!$le)$le=$I[$g->_db]['mappings'][$R]['properties'];if($le){foreach($le
...
as$R)$J=$J&&$g->query(urlencode($R),array(),'DELETE');return$J;}$x="elastic";$df=array("=","query");$cd=array();$hd=array();$jc=array(array("json"));}$bc=array("server"=>"MySQL")+$bc;if(!defined("DRIVER")){$Mf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N="",$V="",$G="",$k=null,$If=null,$Vg=null){mysqli_report(MYSQLI_REPORT_OFF);list($pd,$If)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$pd:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),$k,(is_numeric($If)?$If:ini_get("mysqli.default_port")),(!is_numeric($If)?$If:$Vg));return$J;}function
...
insertUpdate($R,$L,$Pf){$e=array_keys(reset($L));$Nf="INSERT INTO ".table($R)." (".implode(", ",$e).") VALUES\n";$si=array();foreach($e
as$y)$si[$y]="$y = VALUES($y)";$jh="\nON DUPLICATE KEY UPDATE ".implode(", ",$si);$si=array();$fe=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($si&&(strlen($Nf)+$fe+strlen($Y)+strlen($jh)>1e6)){if(!queries($Nf.implode(",\n",$si).$jh))return
false;$si=array();$fe=0;}$si[]=$Y;$fe+=strlen($Y)+2;}return
queries($Nf.implode(",\n",$si).$jh);}}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
table($u){return
idf_escape($u);}function
connect(){global$b,$ai,$gh;$g=new
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2])){$g->set_charset(charset($g));$g->query("SET sql_quote_show_create = 1, autocommit = 1");if(version_compare($g->server_info,'5.7.8')>=0){$gh[lang(26)][]="json";$ai["json"]=4294967295;}return$g;}$J=$g->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($_g=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$_g;return$J;}function
...
foreign_keys($R){global$g,$Ye;static$Ff='`(?:[^`]|``)+`';$J=array();$Fb=$g->result("SHOW CREATE TABLE ".table($R),1);if($Fb){preg_match_all("~CONSTRAINT ($Ff) FOREIGN KEY ?\\(((?:$Ff,? ?)+)\\) REFERENCES ($Ff)(?:\\.($Ff))? \\(((?:$Ff,? ?)+)\\)(?: ON DELETE ($Ye))?(?: ON UPDATE ($Ye))?~",$Fb,$oe,PREG_SET_ORDER);foreach($oe
as$B){preg_match_all("~$Ff~",$B[2],$Xg);preg_match_all("~$Ff~",$B[5],$wh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Xg[0]),"target"=>array_map('idf_unescape',$wh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$Af,$Ee){$c=array();foreach($p
...
queries("CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)$P$Af");if($R!=$C)$c[]="RENAME TO ".table($C);if($P)$c[]=ltrim($P);if($Ee){foreach($Ee
as$y=>$Hi){$ch=substr($Hi,7,strpos($Hi,"` ")-6);foreach($c
as$Qd=>$ab){$Ga=substr($ab,strpos($ab,"NULL")+4,strlen($ab));$rb=substr($ab,strpos($ab,"CHANGE")+7,strpos($ab,"` ")-6);if($ch===$rb){unset($c[$Qd]);$c[$Qd]=$Hi.$Ga;unset($Ee[$y]);}}}foreach($Ee
as$Hi)$c[]=$Hi;}return($c||$Af?queries("ALTER TABLE ".table($R)."\n".implode(",\n",$c).$Af):true);}function
alter_indexes($R,$c){foreach($c
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$xi,$wh){$ng=array();foreach(array_merge($T,$xi)as$R)$ng[]=table($R)." TO ".idf_escape($wh).".".table($R);return
...
routine($C,$U){global$g,$tc,$Ad,$ai;$Ca=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Yh="((".implode("|",array_merge(array_keys($ai),$Ca)).")\\b(?:\\s*\\(((?:[^'\")]|$tc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$Ff="\\s*(".($U=="FUNCTION"?"":$Ad).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Yh";$i=$g->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$Ff\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Yh\\s+":"")."(.*)~is",$i,$B);$p=array();preg_match_all("~$Ff\\s*,?~is",$B[1],$oe,PREG_SET_ORDER);foreach($oe
as$vf){$C=str_replace("``","`",$vf[2]).$vf[3];$p[]=array("field"=>$C,"type"=>strtolower($vf[5]),"length"=>preg_replace_callback("~$tc~s",'normalize_enum',$vf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$vf[8] $vf[7]"))),"null"=>1,"full_type"=>$vf[4],"inout"=>strtoupper($vf[1]),"collation"=>strtolower($vf[9]),);}if($U!="FUNCTION")return
...
login($je,$G){global$x;if($x=="sqlite")return
lang(38,'<code>login()</code>');return
true;}function
tableName($nh){return
h($nh["Name"]);}function
fieldName($o,$if=0){return'<span title="'.h($o["full_type"]).'">'.h($o["field"]).'</span>';}function
selectLinks($nh,$O=""){echo'<p class="links">';$ie=array("select"=>lang(39));if(support("table")||support("indexes"))$ie["table"]=lang(40);if(support("table")){if(is_view($nh))$ie["view"]=lang(41);else$ie["create"]=lang(42);}if($O!==null)$ie["edit"]=lang(43);foreach($ie
...
selectVal($X,$_,$o,$qf){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$o["type"])&&!preg_match("~var~",$o["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$o["type"])&&!is_utf8($X))$J="<i>".lang(44,strlen($qf))."</i>";if(preg_match('~json~',$o["type"]))$J="<code class='jush-js'>$J</code>";return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
...
as$C=>$v){ksort($v["columns"]);$Rf=array();foreach($v["columns"]as$y=>$X)$Rf[]="<i>".h($X)."</i>".($v["lengths"][$y]?"(".$v["lengths"][$y].")":"").($v["descs"][$y]?" DESC":"");echo"<tr title='".h($C)."'><th>$v[type]<td>".implode(", ",$Rf)."\n";}echo"</table>\n";}function
...
selectOrderPrint($if,$e,$w){print_fieldset("sort",lang(55),$if);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'",$e,$X),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),lang(56))."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'",$e),checkbox("desc[$s]",1,false,lang(56))."</div>\n","</div></fieldset>\n";}function
...
false;}function
selectQueryBuild($M,$Z,$fd,$if,$z,$E){return"";}function
...
as$Ff=>$X){if(!$Ff||preg_match("~$Ff~",$o["type"]))$J.="/$X";}if($y&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$J.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J=lang(48);return
explode("/",$J);}function
...
dumpData($R,$hh,$H){global$g,$x;$qe=($x=="sqlite"?0:1048576);if($hh){if($_POST["format"]=="sql"){if($hh=="TRUNCATE+INSERT")echo
truncate_sql($R).";\n";$p=fields($R);}$I=$g->query($H,1);if($I){$Cd="";$Ya="";$Sd=array();$jh="";$Kc=($R!=''?'fetch_assoc':'fetch_row');while($K=$I->$Kc()){if(!$Sd){$si=array();foreach($K
as$X){$o=$I->fetch_field();if($p[$o->name]['is_virtual']){$_i[]=idf_escape($o->name);continue;}$Sd[]=$o->name;$y=idf_escape($o->name);$si[]="$y = VALUES($y)";}$jh=($hh=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$si):"").";\n";}if($_POST["format"]!="sql"){if($hh=="table"){dump_csv($Sd);$hh="INSERT";}dump_csv($K);}else{if(!$Cd)$Cd="INSERT INTO ".table($R)." (".implode(", ",array_map('idf_escape',$Sd)).") VALUES";foreach($K
as$y=>$X){if(in_array("`".$y."`",(array)$_i)){unset($K[$y]);continue;}$o=$p[$y];$K[$y]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$_g=($qe?"\n":" ")."(".implode(",\t",$K).")";if(!$Ya)$Ya=$Cd.$_g;elseif(strlen($Ya)+4+strlen($_g)+strlen($jh)<$qe)$Ya.=",$_g";else{echo$Ya.$jh;$Ya=$Cd.$_g;}}}if($Ya)echo$Ya.$jh;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$g->error)."\n";}}function
dumpFilename($rd){return
friendly_url($rd!=""?$rd:(SERVER!=""?SERVER:"localhost"));}function
dumpHeaders($rd,$Fe=false){$tf=$_POST["output"];$Ec=(preg_match('~sql~',$_POST["format"])?"sql":($Fe?"tar":"csv"));header("Content-Type: ".($tf=="gz"?"application/x-gzip":($Ec=="tar"?"application/x-tar":($Ec=="sql"||$tf!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($tf=="gz")ob_start('ob_gzencode',1e6);return$Ec;}function
...
true;}function
navigation($De){global$ia,$x,$bc,$g;echo'<h1>
...
';if($De=="auth"){$Pc=true;foreach((array)$_SESSION["pwds"]as$ui=>$Ng){foreach($Ng
as$N=>$pi){foreach($pi
as$V=>$G){if($G!==null){if($Pc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Pc=false;}$Nb=$_SESSION["db"][$ui][$N][$V];foreach(($Nb?array_keys($Nb):array(""))as$m)echo"<a href='".h(auth_url($ui,$N,$V,$m))."'>($bc[$ui]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";}}}}}else{if($_GET["ns"]!==""&&!$De&&DB!=""){$g->select_db(DB);$T=table_status('',true);}echo'<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=jush.js&amp;version=4.3.0",'"></script>
';if(support("sql")){echo'<script type="text/javascript">
';if($T){$ie=array();foreach($T
as$R=>$U)$ie[]=preg_quote($R,'/');echo"var jushLinks = { $x: [ '".js_escape(ME).(support("table")?"table=":"select=")."\$&', /\\b(".implode("|",$ie).")\\b/g ] };\n";foreach(array("bac","bra","sqlite_quo","mssql_bra")as$X)echo"jushLinks.$X = jushLinks.$x;\n";}echo'bodyLoad(\'',(is_object($g)?substr($g->server_info,0,3):""),'\');
</script>
';}$this->databasesPrint($De);if(DB==""||!$De){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".lang(61)."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".lang(69)."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".lang(70)."</a>\n";}if($_GET["ns"]!==""&&!$De&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".lang(71)."</a>\n";if(!$T)echo"<p class='message'>".lang(9)."\n";else$this->tablesPrint($T);}}}function
databasesPrint($De){global$b,$g;$l=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Lb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".lang(72)."'>DB</span>: ".($l?"<select name='db'$Lb>".optionlist(array(""=>"")+$l,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".lang(20)."'".($l?" class='hidden'":"").">\n";if($De!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".lang(73).": <select name='ns'$Lb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
...
Adminer);if($b->operators===null)$b->operators=$df;function
...
page_messages($n){$ji=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$_e=$_SESSION["messages"][$ji];if($_e){echo"<div class='message'>".implode("</div>\n<div class='message'>",$_e)."</div>\n";unset($_SESSION["messages"][$ji]);}if($n)echo"<div class='error'>$n</div>\n";}function
page_footer($De=""){global$b,$Lh;echo'</div>
...
long2str($W,$Ai){$_g='';foreach($W
as$X)$_g.=pack('V',$X);if($Ai)return
substr($_g,0,end($W));return$_g;}function
str2long($_g,$Ai){$W=array_values(unpack('V*',str_pad($_g,4*ceil(strlen($_g)/4),"\0")));if($Ai)$W[]=strlen($_g);return$W;}function
xxtea_mx($Gi,$Fi,$kh,$Nd){return
int32((($Gi>>5&0x7FFFFFF)^$Fi<<2)+(($Fi>>3&0x1FFFFFFF)^$Gi<<4))^int32(($kh^$Fi)+($Nd^$Gi));}function
encrypt_string($fh,$y){if($fh=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($fh,true);$He=count($W)-1;$Gi=$W[$He];$Fi=$W[0];$Yf=floor(6+52/($He+1));$kh=0;while($Yf-->0){$kh=int32($kh+0x9E3779B9);$ic=$kh>>2&3;for($uf=0;$uf<$He;$uf++){$Fi=$W[$uf+1];$Ge=xxtea_mx($Gi,$Fi,$kh,$y[$uf&3^$ic]);$Gi=int32($W[$uf]+$Ge);$W[$uf]=$Gi;}$Fi=$W[0];$Ge=xxtea_mx($Gi,$Fi,$kh,$y[$uf&3^$ic]);$Gi=int32($W[$He]+$Ge);$W[$He]=$Gi;}return
long2str($W,false);}function
decrypt_string($fh,$y){if($fh=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($fh,false);$He=count($W)-1;$Gi=$W[$He];$Fi=$W[0];$Yf=floor(6+52/($He+1));$kh=int32($Yf*0x9E3779B9);while($kh){$ic=$kh>>2&3;for($uf=$He;$uf>0;$uf--){$Gi=$W[$uf-1];$Ge=xxtea_mx($Gi,$Fi,$kh,$y[$uf&3^$ic]);$Fi=int32($W[$uf]-$Ge);$W[$uf]=$Fi;}$Gi=$W[$He];$Ge=xxtea_mx($Gi,$Fi,$kh,$y[$uf&3^$ic]);$Fi=int32($W[0]-$Ge);$W[0]=$Fi;$kh=int32($kh-0x9E3779B9);}return
long2str($W,true);}$g='';$ld=$_SESSION["token"];if(!$ld)$_SESSION["token"]=rand(1,1e6);$Lh=get_token();$Gf=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$Gf[$y]=$X;}}function
...
as$Gd=>$X){if($X[0]<$Ch)unset($Fd[$Gd]);}}$Ed=&$Fd[$b->bruteForceKey()];if(!$Ed)$Ed=array($Ch+30*60,0);$Ed[1]++;$Lg=serialize($Fd);rewind($ad);fwrite($ad,$Lg);ftruncate($ad,strlen($Lg));flock($ad,LOCK_UN);fclose($ad);}$La=$_POST["auth"];if($La){$Fd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Ed=$Fd[$b->bruteForceKey()];$Ne=($Ed[1]>30?$Ed[0]-time():0);if($Ne>0)auth_error(lang(78,ceil($Ne/60)));session_regenerate_id();$ui=$La["driver"];$N=$La["server"];$V=$La["username"];$G=(string)$La["password"];$m=$La["db"];set_password($ui,$N,$V,$G);$_SESSION["db"][$ui][$N][$V][$m]=true;if($La["permanent"]){$y=base64_encode($ui)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$Sf=$b->permanentLogin(true);$Gf[$y]="$y:".base64_encode($Sf?encrypt_string($G,$Sf):"");cookie("adminer_permanent",implode(" ",$Gf));}if(count($_POST)==1||DRIVER!=$ui||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($ui,$N,$V,$m));}elseif($_POST["logout"]){if($ld&&!verify_token()){page_header(lang(77),lang(79));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(80));}}elseif($Gf&&!$_SESSION["pwds"]){session_regenerate_id();$Sf=$b->permanentLogin();foreach($Gf
as$y=>$X){list(,$jb)=explode(":",$X);list($ui,$N,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($ui,$N,$V,decrypt_string(base64_decode($jb),$Sf));$_SESSION["db"][$ui][$N][$V][$m]=true;}}function
unset_permanent(){global$Gf;foreach($Gf
as$y=>$X){list($ui,$N,$V,$m)=array_map('base64_decode',explode("-",$y));if($ui==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($Gf[$y]);}cookie("adminer_permanent",implode(" ",$Gf));}function
auth_error($n){global$b,$ld;$Og=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Og]||$_GET[$Og])&&!$ld)$n=lang(81);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(82,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Og]&&$_GET[$Og]&&ini_bool("session.use_only_cookies"))$n=lang(83);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(84),lang(85,implode(", ",$Mf)),false);page_footer("auth");exit;}$g=connect();}$ac=new
Min_Driver($g);if(!is_object($g)||($je=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?h($g):(is_string($je)?$je:lang(86))));if($La&&$_POST["token"])$_POST["token"]=$Lh;$n='';if($_POST){if(!verify_token()){$_d="max_input_vars";$ue=ini_get($_d);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$ue||$X<$ue)){$_d=$y;$ue=$X;}}}$n=(!$_POST["token"]&&$ue?lang(87,"'$_d'"):lang(79).' '.lang(88));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(89,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(90);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
select($I,$h=null,$lf=array(),$z=0){global$x;$ie=array();$w=array();$e=array();$Ua=array();$ai=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Md=0;$Md<count($K);$Md++){$o=$I->fetch_field();$C=$o->name;$kf=$o->orgtable;$jf=$o->orgname;$J[$o->table]=$kf;if($lf&&$x=="sql")$ie[$Md]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($kf!=""){if(!isset($w[$kf])){$w[$kf]=array();foreach(indexes($kf,$h)as$v){if($v["type"]=="PRIMARY"){$w[$kf]=array_flip($v["columns"]);break;}}$e[$kf]=$w[$kf];}if(isset($e[$kf][$jf])){unset($e[$kf][$jf]);$w[$kf][$jf]=$Md;$ie[$Md]=$kf;}}if($o->charsetnr==63)$Ua[$Md]=true;$ai[$Md]=$o->type;echo"<th".($kf!=""||$o->name!=$jf?" title='".h(($kf!=""?"$kf.":"").$jf)."'":"").">".h($C).($lf?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$y=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ua[$y]&&!is_utf8($X))$X="<i>".lang(44,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($ai[$y]==254)$X="<code>$X</code>";}if(isset($ie[$y])&&!$e[$ie[$y]]){if($lf&&$x=="sql"){$R=$K[array_search("table=",$ie)];$_=$ie[$y].urlencode($lf[$R]!=""?$lf[$R]:$R);}else{$_="edit=".urlencode($ie[$y]);foreach($w[$ie[$y]]as$nb=>$Md)$_.="&where".urlencode("[".bracket_escape($nb)."]")."=".urlencode($K[$Md]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
...
edit_type($y,$o,$qb,$Wc=array(),$yi){global$gh,$ai,$hi,$Ye;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),' aria-labelledby="label-type">';if($U&&!isset($ai[$U])&&!isset($Wc[$U]))array_unshift($gh,$U);if($Wc)$gh[lang(91)]=$Wc;echo
optionlist($gh,$U),'</select>
<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';if($o["is_virtual"])textarea(h($y)."[virtual1]",$yi,$L=2,$sb=40);else{echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(92).')'.optionlist($qb,$o["collation"]).'</select>',($hi?"<select name='".h($y)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($hi,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(93).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Wc?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(94).")".optionlist(explode("|",$Ye),$o["on_delete"])."</select> ":" ");}}function
process_length($fe){global$tc;return(preg_match("~^\\s*\\(?\\s*$tc(?:\\s*,\\s*$tc)*+\\s*\\)?\\s*\$~",$fe)&&preg_match_all("~$tc~",$fe,$oe)?"(".implode(",",$oe[0]).")":preg_replace('~^[0-9].*~','(\0)',preg_replace('~[^-0-9,+()[\]]~','',$fe)));}function
...
';foreach($p
as$s=>$o){$s++;$mf=$o[($_POST?"orig":"field")];$Wb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$mf=="");echo'<tr',($Wb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$Ad),$o["inout"]):""),'<th>';if($Wb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($mf),'">
';$Yf=$Pg[$o["field"]];edit_type("fields[$s]",$o,$qb,$Wc,$Yf);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }" aria-labelledby="label-ai"></label><td><?php
echo
...
',(support("comment")?"<td".($wb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".lang(99)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.3.0' alt='^' title='".lang(100)."' onclick='return !editingMoveRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.3.0' alt='v' title='".lang(101)."' onclick='return !editingMoveRow(this, 0);'>&nbsp;":""),($mf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".lang(102)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
process_fields(&$p){$D=0;if($_POST["up"]){$Zd=0;foreach($p
as$y=>$o){if(key($_POST["up"])==$y){unset($p[$y]);array_splice($p,$Zd,0,array($o));break;}if(isset($o["field"]))$Zd=$D;$D++;}}elseif($_POST["down"]){$Yc=false;foreach($p
...
grant($dd,$Uf,$e,$Xe){if(!$Uf)return
true;if($Uf==array("ALL PRIVILEGES","GRANT OPTION"))return($dd=="GRANT"?queries("$dd ALL PRIVILEGES$Xe WITH GRANT OPTION"):queries("$dd ALL PRIVILEGES$Xe")&&queries("$dd GRANT OPTION$Xe"));return
queries("$dd ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~','\\1',implode("$e, ",$Uf).$e).$Xe);}function
drop_create($cc,$i,$dc,$_h,$fc,$A,$ze,$xe,$ye,$Ue,$Ke){if($_POST["drop"])query_redirect($cc,$A,$ze);elseif($Ue=="")query_redirect($i,$A,$ye);elseif($Ue!=$Ke){$Gb=queries($i);queries_redirect($A,$xe,$Gb&&queries($cc));if($Gb)queries($dc);}else
queries_redirect($A,$xe,queries($_h)&&queries($fc)&&queries($cc)&&queries($i));}function
create_trigger($Xe,$K){global$x;$Eh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($x=="mssql"?$Xe.$Eh:$Eh.$Xe).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
...
format_foreign_key($q){global$Ye;return" FOREIGN KEY (".implode(", ",array_map('idf_escape',$q["source"])).") REFERENCES ".table($q["table"])." (".implode(", ",array_map('idf_escape',$q["target"])).")".(preg_match("~^($Ye)\$~",$q["on_delete"])?" ON DELETE $q[on_delete]":"").(preg_match("~^($Ye)\$~",$q["on_update"])?" ON UPDATE $q[on_update]":"");}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".lang(128)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(131)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(67),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$qh=array();$rh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$oe,PREG_SET_ORDER);foreach($oe
as$s=>$B){$qh[$B[1]]=array($B[2],$B[3]);$rh[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$Mh=0;$Ra=-1;$Bg=array();$jg=array();$de=array();foreach(table_status('',true)as$R=>$S){if(is_view($S))continue;$Jf=0;$Bg[$R]["fields"]=array();foreach(fields($R)as$C=>$o){$Jf+=1.25;$o["pos"]=$Jf;$Bg[$R]["fields"][$C]=$o;}$Bg[$R]["pos"]=($qh[$R]?$qh[$R]:array($Mh,0));foreach($b->foreignKeys($R)as$X){if(!$X["db"]){$be=$Ra;if($qh[$R][1]||$qh[$X["table"]][1])$be=min(floatval($qh[$R][1]),floatval($qh[$X["table"]][1]))-1;else$Ra-=.1;while($de[(string)$be])$be-=.0001;$Bg[$R]["references"][$X["table"]][(string)$be]=array($X["source"],$X["target"]);$jg[$X["table"]][$R][(string)$be]=$X["target"];$de[(string)$be]=true;}}$Mh=max($Mh,$Bg[$R]["pos"][0]+2.5+$Jf);}echo'<div id="schema" style="height: ',$Mh,'em;" onselectstart="return false;">
<script type="text/javascript">
...
as$be=>$gg){$ce=$be-$qh[$C][1];$s=0;foreach($gg[0]as$Xg)echo"\n<div class='references' title='".h($xh)."' id='refs$be-".($s++)."' style='left: $ce"."em; top: ".$R["fields"][$Xg]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$ce)."em;'></div></div>";}}foreach((array)$jg[$C]as$xh=>$kg){foreach($kg
as$be=>$e){$ce=$be-$qh[$C][1];$s=0;foreach($e
as$wh)echo"\n<div class='references' title='".h($xh)."' id='refd$be-".($s++)."' style='left: $ce"."em; top: ".$R["fields"][$wh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$ce)."em;'></div></div>";}}echo"\n</div>\n";}foreach($Bg
as$C=>$R){foreach((array)$R["references"]as$xh=>$kg){foreach($kg
as$be=>$gg){$Ce=$Mh;$se=-10;foreach($gg[0]as$y=>$Xg){$Kf=$R["pos"][0]+$R["fields"][$Xg]["pos"];$Lf=$Bg[$xh]["pos"][0]+$Bg[$xh]["fields"][$gg[1][$y]]["pos"];$Ce=min($Ce,$Kf,$Lf);$se=max($se,$Kf,$Lf);}echo"<div class='references' id='refl$be' style='left: $be"."em; top: $Ce"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($se-$Ce)."em;'></div></div>\n";}}}echo'</div>
...
use_sql($m).";\n\n";$sf="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$wg){foreach(get_rows("SHOW $wg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE $wg ".idf_escape($K["Name"]),2));set_utf8mb4($i);$sf.=($hh!='DROP+CREATE'?"DROP $wg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($i);$sf.=($hh!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}if($sf)echo"DELIMITER ;;\n\n$sf"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$xi=array();foreach(table_status('',true)as$C=>$S){$R=(DB==""||in_array($C,(array)$_POST["tables"]));$Jb=(DB==""||in_array($C,(array)$_POST["data"]));if($R||$Jb){if($Ec=="tar"){$Jh=new
TmpFile;ob_start(array($Jh,'write'),1e5);}$b->dumpTable($C,($R?$_POST["table_style"]:""),(is_view($S)?2:0));if(is_view($S))$xi[]=$C;elseif($Jb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Id&&$_POST["triggers"]&&$R&&($Wh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Wh\nDELIMITER ;\n";if($Ec=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$Jh);}elseif($Id)echo"\n";}}foreach($xi
as$wi)$b->dumpTable($wi,$_POST["table_style"],1);if($Ec=="tar")echo
...
<table cellspacing="0">
';$Of=array();if(DB!=""){$fb=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$fb onclick='formCheck(this, /^tables\\[/);'>".lang(115)."</label>","<th style='text-align: right;'><label class='block'>".lang(137)."<input type='checkbox' id='check-data'$fb onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$xi="";$th=tables_list();foreach($th
as$C=>$U){$Nf=preg_replace('~_.*~','',$C);$fb=($a==""||$a==(substr($a,-1)=="%"?"$Nf%":$C));$Rf="<tr><td>".checkbox("tables[]",$C,$fb,$C,"checkboxClick(event, this); formUncheck('check-tables');","block");if($U!==null&&!preg_match('~table~i',$U))$xi.="$Rf\n";else
echo"$Rf<td align='right'><label class='block'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$fb,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Of[$Nf]++;}echo$xi;if($th)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$Nf=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$Nf%",$m,"formUncheck('check-databases');","block")."\n";$Of[$Nf]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Pc=true;foreach($Of
as$y=>$X){if($y!=""&&$X>1){echo($Pc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Pc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(68));echo'<p class="links"><a href="'.h(ME).'user=">'.lang(138)."</a>";$I=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$dd=$I;if(!$I)$I=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($dd?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$dd||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$nd=&get_session("queries");$md=&$nd[DB];if(!$n&&$_POST["clear"]){$md=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(69):lang(61)),$n);if(!$n&&$_POST){$ad=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$ad=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($ad?fread($ad,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Yf=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$md||reset(end($md))!=$Yf){restart_session();$md[]=array($Yf,time());set_session("queries",$nd);stop_session();}}$Yg="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Rb=";";$D=0;$qc=true;$h=connect();if(is_object($h)&&DB!="")$h->select_db(DB);$ub=0;$vc=array();$xf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\\*|-- |$'.($x=="pgsql"?'|\\$[^$]*\\$':'');$Nh=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$hc=$b->dumpFormat();unset($hc["sql"]);while($H!=""){if(!$D&&preg_match("~^$Yg*+DELIMITER\\s+(\\S+)~i",$H,$B)){$Rb=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Rb)."\\s*|$xf)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Yc,$Jf)=$B[0];if(!$Yc&&$ad&&!feof($ad))$H.=fread($ad,1e5);else{if(!$Yc&&rtrim($H)=="")break;$D=$Jf+strlen($Yc);if($Yc&&rtrim($Yc)!=$Rb){while(preg_match('('.($Yc=='/*'?'\\*/':($Yc=='['?']':(preg_match('~^-- |^#~',$Yc)?"\n":preg_quote($Yc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$_g=$B[0][0];if(!$_g&&$ad&&!feof($ad))$H.=fread($ad,1e5);else{$D=$B[0][1]+strlen($_g);if($_g[0]!="\\")break;}}}else{$qc=false;$Yf=substr($H,0,$Jf);$ub++;$Rf="<pre id='sql-$ub'><code class='jush-$x'>".$b->sqlCommandQuery($Yf)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$Yg*+ATTACH\\b~i",$Yf,$B)){echo$Rf,"<p class='error'>".lang(139)."\n";$vc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$Rf;ob_flush();flush();}$dh=microtime(true);if($g->multi_query($Yf)&&is_object($h)&&preg_match("~^$Yg*+USE\\b~i",$Yf))$h->query($Yf);do{$I=$g->store_result();$Ch=" <span class='time'>(".format_time($dh).")</span>".(strlen($Yf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Yf))."'>".lang(10)."</a>":"");if($g->error){echo($_POST["only_errors"]?$Rf:""),"<p class='error'>".lang(140).($g->errno?" ($g->errno)":"").": ".error()."\n";$vc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$lf=select($I,$h,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Oe=$I->num_rows;echo"<p>".($Oe?($z&&$Oe>$z?lang(141,$z):"").lang(142,$Oe):""),$Ch;$t="export-$ub";$Dc=", <a href='#$t' onclick=\"return !toggle('$t');\">".lang(70)."</a><span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$hc,$xa["format"])."<input type='hidden' name='query' value='".h($Yf)."'>"." <input type='submit' name='export' value='".lang(70)."'><input type='hidden' name='token' value='$Lh'></span>\n";if($h&&preg_match("~^($Yg|\\()*+SELECT\\b~i",$Yf)&&($Cc=explain($h,$Yf))){$t="explain-$ub";echo", <a href='#$t' onclick=\"return !toggle('$t');\">EXPLAIN</a>$Dc","<div id='$t' class='hidden'>\n";select($Cc,$h,$lf);echo"</div>\n";}else
echo$Dc;echo"</form>\n";}}else{if(preg_match("~^$Yg*+(CREATE|DROP|ALTER)$Yg++(DATABASE|SCHEMA)\\b~i",$Yf)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($g->info)."'>".lang(143,$g->affected_rows)."$Ch\n";}$dh=microtime(true);}while($g->next_result());}$H=substr($H,$D);$D=0;}}}}if($qc)echo"<p class='message'>".lang(144)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(145,$ub-count($vc))," <span class='time'>(".format_time($Nh).")</span>\n";}elseif($vc&&$ub>1)echo"<p class='error'>".lang(140).": ".implode("",$vc)."\n";}else
echo"<p class='error'>".upload_error($H)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$_c="<input type='submit' value='".lang(146)."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$Yf=$_GET["sql"];if($_POST)$Yf=$_POST["query"];elseif($_GET["history"]=="all")$Yf=$md;elseif($_GET["history"]!="")$Yf=$md[$_GET["history"]][0];echo"<p>";textarea("query",$Yf,20);echo($_POST?"":"<script type='text/javascript'>focus(document.getElementsByTagName('textarea')[0]);</script>\n"),"<p>$_c\n",lang(147).": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".lang(148)."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$_c":lang(149)),"</div></fieldset>\n","<fieldset><legend>".lang(150)."</legend><div>",lang(151,"<code>adminer.sql".(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.lang(152).'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(153))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(154))."\n","<input type='hidden' name='token' value='$Lh'>\n";if(!isset($_GET["import"])&&$md){print_fieldset("history",lang(155),$_GET["history"]!="");for($X=end($md);$X;$X=prev($md)){$y=key($md);list($Yf,$Ch,$lc)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$Ch)."'>".@date("H:i:s",$Ch)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Yf)))),80,"</code>").($lc?" <span class='time'>($lc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(156)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(157)."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(!isset($o["privileges"][$ii?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($ii?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$di=unique_array($_GET["where"],$w);$bg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(158),$ac->delete($a,$bg,!$di));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($ii){if(!$O)redirect($A);queries_redirect($A,lang(159),$ac->update($a,$O,$bg,!$di));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$ac->insert($a,$O);$ae=($I?last_id():0);queries_redirect($A,lang(160,($ae?" $ae":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
...
as$y=>$X){if(!$Z)$K[$y]=null;$p[$y]=array("field"=>$y,"null"=>($y!=$ac->primary),"auto_increment"=>($y==$ac->primary));}}}edit_form($a,$p,$K,$ii);}elseif(isset($_GET["create"])){$a=$_GET["create"];$zf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$y)$zf[$y]=$y;$ig=referencable_primary($a);$Wc=array();foreach($ig
as$oh=>$o)$Wc[str_replace("`","``",$oh)."`".str_replace("`","``",$o["field"])]=$oh;$of=array();$S=array();if($a!=""){$of=fields($a);$S=table_status($a);if(!$S)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(161),drop_tables(array($a)));else{$p=array();$Da=array();$mi=false;$Uc=array();$nf=reset($of);$Aa=" FIRST";$Ee=array();$Pg=tables_generated_columns("'".$K["name"]."'");$Qg=tables_generated_columns2("'".$K["name"]."'");foreach($K["fields"]as$y=>$o){$q=$Wc[$o["type"]];$Xh=($q!==null?$ig[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($y==$K["auto_increment_col"])$o["auto_increment"]=true;$Wf=process_field($o,$Xh);$Da[]=array($o["orig"],$Wf,$Aa);if($Wf!=process_field($nf,$nf)){$p[]=array($o["orig"],$Wf,$Aa);if($o["orig"]!=""||$Aa)$mi=true;}if($q!==null)$Uc[idf_escape($o["field"])]=($a!=""&&$x!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Wc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Xh["field"]),'on_delete'=>$o["on_delete"],));$Aa=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$mi=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$nf=next($of);if(!$nf)$Aa="";}if($o["virtual1"]){$Zh="";if($o["length"])$Zh=$o["type"]."(".$o["length"].")";else$Zh=$o["type"];if(($Pg[$o["field"]]!=$o["virtual1"])||($o["field"]!=$o["orig"])||($Qg[$o["field"]]!=$Zh)||($Aa)){if($o["length"])$Ee[]="CHANGE "."`".$o["orig"]."` "."`".$o["field"]."` ".$o["type"]."(".$o["length"].") as (".$o["virtual1"].")";else$Ee[]="CHANGE "."`".$o["orig"]."` "."`".$o["field"]."` ".$o["type"]." as (".$o["virtual1"].")";}}}$Af="";if($zf[$K["partition_by"]]){$Bf=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$y=>$X){$Y=$K["partition_values"][$y];$Bf[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$Af.="\nPARTITION BY $K[partition_by]($K[partition])".($Bf?" (".implode(",",$Bf)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$S["Create_options"]))$Af.="\nREMOVE PARTITIONING";$we=lang(162);if($a==""){cookie("adminer_engine",$K["Engine"]);$we=lang(163);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$we,alter_table($a,$C,($x=="sqlite"&&($mi||$Uc)?$Da:$p),$Uc,($K["Comment"]!=$S["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$S["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$S["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$Af,$Ee));}}page_header(($a!=""?lang(42):lang(71)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($ai["int"])?"int":(isset($ai["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$S;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($of
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$bd="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $bd ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$Bf=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $bd AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$Bf[""]="";$K["partition_names"]=array_keys($Bf);$K["partition_values"]=array_values($Bf);}}}$qb=collations();$sc=engines();foreach($sc
...
';$wb=($_POST?$_POST["comments"]:$K["Comment"]!="");if(!$_POST&&!$wb){foreach($K["fields"]as$o){if($o["comment"]!=""){$wb=true;break;}}}$Pg=tables_generated_columns("'".$K["name"]."'");edit_fields($K["fields"],$qb,"TABLE",$Wc,$wb,$Pg);echo'</table>
<p>
...
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}if(support("partitioning")){$_f=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(167),$K["partition_by"]);echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')",1).">".optionlist(array(""=>"")+$zf,$K["partition_by"])."</select>",'(<input name="partition" value="',h($K["partition"]),'">)
',lang(168),': <input type="number" name="partitions" class="size',($_f||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
';}elseif(isset($_GET["indexes"])){$a=$_GET["indexes"];$wd=array("PRIMARY","UNIQUE","INDEX");$S=table_status($a,true);if(preg_match('~MyISAM|M?aria'.($g->server_info>=5.6?'|InnoDB':'').'~i',$S["Engine"]))$wd[]="FULLTEXT";if(preg_match('~MyISAM|M?aria'.($g->server_info>=5.7?'|InnoDB':'').'~i',$S["Engine"]))$wd[]="SPATIAL";$w=indexes($a);$Pf=array();if($x=="mongo"){$Pf=$w["_id_"];unset($wd[0]);unset($w["_id_"]);}$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]){$c=array();foreach($K["indexes"]as$v){$C=$v["name"];if(in_array($v["type"],$wd)){$e=array();$ge=array();$Tb=array();$O=array();ksort($v["columns"]);foreach($v["columns"]as$y=>$d){if($d!=""){$fe=$v["lengths"][$y];$Sb=$v["descs"][$y];$O[]=idf_escape($d).($fe?"(".(+$fe).")":"").($Sb?" DESC":"");$e[]=$d;$ge[]=($fe?$fe:null);$Tb[]=$Sb;}}if($e){$Ac=$w[$C];if($Ac){ksort($Ac["columns"]);ksort($Ac["lengths"]);ksort($Ac["descs"]);if($v["type"]==$Ac["type"]&&array_values($Ac["columns"])===$e&&(!$Ac["lengths"]||array_values($Ac["lengths"])===$ge)&&array_values($Ac["descs"])===$Tb){unset($w[$C]);continue;}}$c[]=array($v["type"],$C,$O);}}}foreach($w
...
';if($Pf){echo"<tr><td>PRIMARY<td>";foreach($Pf["columns"]as$y=>$d){echo
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(182).": ".h($da),$n);$wg=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$ud=array();$sf=array();foreach($wg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$sf[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$ud[]=$s;}if(!$n&&$_POST){$Za=array();foreach($wg["fields"]as$y=>$o){if(in_array($y,$ud)){$X=process_input($o);if($X===false)$X="''";if(isset($sf[$y]))$g->query("SET @".idf_escape($o["field"])." = $X");}$Za[]=(isset($sf[$y])?"@".idf_escape($o["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Za).")";echo"<p><code class='jush-$x'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n";if(!$g->multi_query($H))echo"<p class='error'>".error()."\n";else{$h=connect();if(is_object($h))$h->select_db(DB);do{$I=$g->store_result();if(is_object($I))select($I,$h);else
echo"<p class='message'>".lang(183,$g->affected_rows)."\n";}while($g->next_result());if($sf)select($g->query("SELECT ".implode(", ",$sf)));}}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$we=($_POST["drop"]?lang(184):($C!=""?lang(185):lang(186)));$A=ME."table=".urlencode($a);if(!$_POST["drop"]){$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$wh=array();foreach($K["source"]as$y=>$X)$wh[$y]=$K["target"][$y];$K["target"]=$wh;}if($x=="sqlite")queries_redirect($A,$we,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$cc="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$cc,$A,$we);else{query_redirect($c.($C!=""?"$cc,":"")."\nADD".format_foreign_key($K),$A,$we);$n=lang(187)."<br>$n";}}}page_header(lang(188),$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($C!=""){$Wc=foreign_keys($a);$K=$Wc[$C];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$Xg=array_keys(fields($a));$wh=($a===$K["table"]?$Xg:array_keys(fields($K["table"])));$hg=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
';if($K["db"]==""&&$K["ns"]==""){echo
lang(189),':
...
';$Md=0;foreach($K["source"]as$y=>$X){echo"<tr>","<td>".html_select("source[".(+$y)."]",array(-1=>"")+$Xg,$X,($Md==count($K["source"])-1?"foreignAddRow(this);":1),"label-source"),"<td>".html_select("target[".(+$y)."]",$wh,$K["target"][$y],1,"label-target");$Md++;}echo'</table>
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$Ye),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$Ye),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$pf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$pf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ha=" AS\n$K[select]";$A=ME."table=".urlencode($C);$we=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$pf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ha,$A,$we);else{$yh=$C."_adminer_".uniqid();drop_create("DROP $pf ".table($a),"CREATE $U ".table($C).$Ha,"DROP $U ".table($C),"CREATE $U ".table($yh).$Ha,"DROP $U ".table($yh),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$we,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($pf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Vh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Vh["Timing"])&&in_array($_POST["Event"],$Vh["Event"])&&in_array($_POST["Type"],$Vh["Type"])){$Xe=" ON ".table($a);$cc="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$Xe:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($cc,$A,lang(222));else{if($C!="")queries($cc);queries_redirect($A,($C!=""?lang(223):lang(224)),queries(create_trigger($Xe,$_POST)));if($C!="")queries(create_trigger($Xe,$K+array("Type"=>reset($Vh["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(225).": ".h($C):lang(226)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
...
';}elseif(isset($_GET["user"])){$ha=$_GET["user"];$Uf=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$K){foreach(explode(",",($K["Privilege"]=="Grant option"?"":$K["Context"]))as$Bb)$Uf[$Bb][$K["Privilege"]]=$K["Comment"];}$Uf["Server Admin"]+=$Uf["File access on server"];$Uf["Databases"]["Create routine"]=$Uf["Procedures"]["Create routine"];unset($Uf["Procedures"]["Create routine"]);$Uf["Columns"]=array();foreach(array("Select","Insert","Update","References")as$X)$Uf["Columns"][$X]=$Uf["Tables"][$X];unset($Uf["Server Admin"]["Usage"]);foreach($Uf["Tables"]as$y=>$X)unset($Uf["Databases"][$y]);$Je=array();if($_POST){foreach($_POST["objects"]as$y=>$X)$Je[$X]=(array)$Je[$X]+(array)$_POST["grants"][$y];}$ed=array();$Ve="";if(isset($_GET["host"])&&($I=$g->query("SHOW GRANTS FOR ".q($ha)."@".q($_GET["host"])))){while($K=$I->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$K[0],$B)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$B[1],$oe,PREG_SET_ORDER)){foreach($oe
as$X){if($X[1]!="USAGE")$ed["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$ed["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ve=$B[1];}}if($_POST&&!$n){$We=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $We",ME."privileges=",lang(229));else{$Le=q($_POST["user"])."@".q($_POST["host"]);$Cf=$_POST["pass"];if($Cf!=''&&!$_POST["hashed"]){$Cf=$g->result("SELECT PASSWORD(".q($Cf).")");$n=!$Cf;}$Gb=false;if(!$n){if($We!=$Le){$Gb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Le IDENTIFIED BY PASSWORD ".q($Cf));$n=!$Gb;}elseif($Cf!=$Ve)queries("SET PASSWORD FOR $Le = ".q($Cf));}if(!$n){$tg=array();foreach($Je
as$Qe=>$dd){if(isset($_GET["grant"]))$dd=array_filter($dd);$dd=array_keys($dd);if(isset($_GET["grant"]))$tg=array_diff(array_keys(array_filter($Je[$Qe],'strlen')),$dd);elseif($We==$Le){$Te=array_keys((array)$ed[$Qe]);$tg=array_diff($Te,$dd);$dd=array_diff($dd,$Te);unset($ed[$Qe]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Qe,$B)&&(!grant("REVOKE",$tg,$B[2]," ON $B[1] FROM $Le")||!grant("GRANT",$dd,$B[2]," ON $B[1] TO $Le"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($We!=$Le)queries("DROP USER $We");elseif(!isset($_GET["grant"])){foreach($ed
as$Qe=>$tg){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Qe,$B))grant("REVOKE",array_keys($tg),$B[2]," ON $B[1] FROM $Le");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?lang(230):lang(231)),!$n);if($Gb)$g->query("DROP USER $Le");}}page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(138)),$n,array("privileges"=>array('',lang(68))));if($_POST){$K=$_POST;$ed=$Je;}else{$K=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Ve;if($Ve!="")$K["hashed"]=true;$ed[(DB==""||$ed?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
as$Qe=>$dd){echo'<th>'.($Qe!="*.*"?"<input name='objects[$s]' value='".h($Qe)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(123),"Columns"=>lang(45),"Procedures"=>lang(233),)as$Bb=>$Sb){foreach((array)$Uf[$Bb]as$Tf=>$vb){echo"<tr".odd()."><td".($Sb?">$Sb<td":" colspan='2'").' lang="en" title="'.h($vb).'">'.h($Tf);$s=0;foreach($ed
as$Qe=>$dd){$C="'grants[$s][".h(strtoupper($Tf))."]'";$Y=$dd[strtoupper($Tf)];if($Bb=="Server Admin"&&$Qe!=(isset($ed["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(234)."<option value='0'".($Y=="0"?" selected":"").">".lang(235)."</select>";else
echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Tf=="All privileges"?" id='grants-$s-all'":($Tf=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Lh,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Ud=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$Ud++;}queries_redirect(ME."processlist=",lang(236,$Ud),$Ud||!$_POST["kill"]);}page_header(lang(107),$n);echo'
<form action="" method="post">
...
';}elseif(isset($_GET["replication"])){page_header(lang(110));echo"<h3>".lang(240).doc_link(array("sql"=>"show-master-status.html"))."</h3>\n";$me=replication_status("MASTER");if(!$me)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($me[0]as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}$Ug=replication_status("SLAVE");if($Ug){echo"<h3>".lang(241).doc_link(array("sql"=>"show-slave-status.html"))."</h3>\n";foreach($Ug[0]as$Tg){echo"<table cellspacing='0'>\n";foreach($Tg
as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}}}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$p=fields($a);$Wc=column_foreign_keys($a);$Se="";if($S["Oid"]){$Se=($x=="sqlite"?"rowid":"oid");$w[]=array("type"=>"PRIMARY","columns"=>array($Se));}parse_str($_COOKIE["adminer_import"],$ya);$ug=array();$e=array();$Bh=null;foreach($p
as$y=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){if($o["is_virtual"])continue;$e[$y]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$Bh=$b->selectLengthProcess();}$ug+=$o["privileges"];}list($M,$fd)=$b->selectColumnsProcess($e,$w);$Hd=count($fd)<count($M);$Z=$b->selectSearchProcess($p,$w);$if=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$bd=($M?implode(", ",$M):"*".($Se?", $Se":"")).convert_fields($e,$p,$M)."\nFROM ".table($a);$gd=($fd&&$Hd?"\nGROUP BY ".implode(", ",$fd):"").($if?"\nORDER BY ".implode(", ",$if):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$ei=>$K){$Ha=convert_field($p[key($K)]);$M=array($Ha?$Ha:idf_escape(key($K)));$Z[]=where_check($ei,$p);$J=$ac->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$Ci=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$gb=array();foreach($_POST["check"]as$db)$gb[]=where_check($db,$p);$Ci[]="((".implode(") OR (",$gb)."))";}$Ci=($Ci?"\nWHERE ".implode(" AND ",$Ci):"");$Pf=$gi=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Pf=array_flip($v["columns"]);$gi=($M?$Pf:array());break;}}foreach((array)$gi
as$y=>$X){if(in_array(idf_escape($y),$M))unset($gi[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$gi===array())$H="SELECT $bd$Ci$gd";else{$ci=array();foreach($_POST["check"]as$X)$ci[]="(SELECT".limit($bd,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$gd,1).")";$H=implode(" UNION ALL ",$ci);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Wc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($e
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($gi===array()&&is_array($_POST["check"]))||$Hd){$I=($_POST["delete"]?$ac->delete($a,$Ci):($_POST["clone"]?queries("INSERT $H$Ci"):$ac->update($a,$O,$Ci)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Bi="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$ac->delete($a,$Bi,1):($_POST["clone"]?queries("INSERT".limit1($H,$Bi)):$ac->update($a,$O,$Bi)));if(!$I)break;$za+=$g->affected_rows;}}}$we=lang(242,$za);if($_POST["clone"]&&$I&&$za==1){$ae=last_id();if($ae)$we=lang(160," $ae");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$we,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(243);else{$I=true;$za=0;foreach($_POST["val"]as$ei=>$K){$O=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$I=$ac->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($ei,$p),!($Hd||$gi===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(242,$za),$I);}}elseif(!is_string($Mc=get_file("csv_file",true)))$n=upload_error($Mc);elseif(!preg_match('~~u',$Mc))$n=lang(244);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$sb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Mc,$oe);$za=count($oe[0]);$ac->begin();$Ig=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($oe[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Ig]*)$Ig~",$X.$Ig,$pe);if(!$y&&!array_diff($pe[1],$sb)){$sb=$pe[1];$za--;}else{$O=array();foreach($pe[1]as$s=>$nb)$O[idf_escape($sb[$s])]=($nb==""&&$p[$sb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$nb))));$L[]=$O;}}$I=(!$L||$ac->insertUpdate($a,$L,$Pf));if($I)$I=$ac->commit();queries_redirect(remove_from_uri("page"),lang(245,$za),$I);$ac->rollback();}}}$oh=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(50).": $oh",$n);$O=null;if(isset($ug["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Wc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e&&support("table"))echo"<p class='error'>".lang(246).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$w);$b->selectOrderPrint($if,$e,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($Bh);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Zc=$g->result(count_rows($a,$Z,$Hd,$fd));$E=floor(max(0,$Zc-1)/$z);}$Fg=$M;if(!$Fg){$Fg[]="*";if($Se)$Fg[]=$Se;}$Cb=convert_fields($e,$p,$M);if($Cb)$Fg[]=substr($Cb,2);$I=$ac->select($a,$Fg,$Z,$fd,$if,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$I->seek($z*$E);$pc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$fd&&$Hd&&$x=="sql")$Zc=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Qa=$b->backwardKeys($a,$oh);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$fd&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(247)."</a>");$Ie=array();$cd=array();reset($M);$dg=1;foreach($L[0]as$y=>$X){if($y!=$Se){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$y];$C=($o?$b->fieldName($o,$dg):($X["fun"]?"*":$y));if($C!=""){$dg++;$Ie[$y]=$C;$d=idf_escape($y);$qd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Sb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($qd.($if[0]==$d||$if[0]==$y||(!$if&&$Hd&&$fd[0]==$d)?$Sb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($qd.$Sb)."' title='".lang(56)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($y)).'\'); return false;" title="'.lang(53).'" class="text jsonly"> =</a>';echo"</span>";}$cd[$y]=$X["fun"];next($M);}}$ge=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$y=>$X)$ge[$y]=max($ge[$y],min(40,strlen(utf8_decode($X))));}}echo($Qa?"<th>".lang(248):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Wc)as$He=>$K){$di=unique_array($L[$He],$w);if(!$di){$di=array();foreach($L[$He]as$y=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$y))$di[$y]=$X;}}$ei="";foreach($di
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x=='sql'&&preg_match("~^utf8_~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$ei.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$fd&&$M?"":"<td>".checkbox("check[]",substr($ei,1),in_array(substr($ei,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Hd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$ei)."'>".lang(249)."</a>"));foreach($K
as$y=>$X){if(isset($Ie[$y])){$o=$p[$y];if($X!=""&&(!isset($pc[$y])||$pc[$y]!=""))$pc[$y]=(is_mail($X)?$Ie[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$ei;if(!$_&&$X!==null){foreach((array)$Wc[$y]as$q){if(count($Wc[$y])==1||end($q["source"])==$y){$_="";foreach($q["source"]as$s=>$Xg)$_.=where_link($s,$q["target"][$s],$L[$He][$Xg]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if($q["ns"])$_=preg_replace('~([?&]ns=)[^&]+~','\\1'.urlencode($q["ns"]),$_);if(count($q["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$di))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($di
as$Nd=>$W)$_.=where_link($s++,$Nd,$W);}$X=select_value($X,$_,$o,$Bh);$t=h("val[$ei][".bracket_escape($y)."]");$Y=$_POST["val"][$ei][bracket_escape($y)];$kc=!is_array($K[$y])&&is_utf8($X)&&$L[$He][$y]==$K[$y]&&!$cd[$y];$Ah=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$kc)||$Y!==null){$id=h($Y!==null?$Y:$K[$y]);echo"<td>".($Ah?"<textarea name='$t' cols='30' rows='".(substr_count($K[$y],"\n")+1)."'>$id</textarea>":"<input name='$t' value='$id' size='$ge[$y]'>");}else{$ke=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($ke?2:($Ah?1:0)).($kc?"":", '".h(lang(250))."'").");\">$X";}}}if($Qa)echo"<td>";$b->backwardKeysPrint($Qa,$L[$He]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$zc=true;if($_GET["page"]!="last"){if(!+$z)$Zc=count($L);elseif($x!="sql"||!$Hd){$Zc=($Hd?false:found_rows($S,$Z));if($Zc<max(1e4,2*($E+1)*$z))$Zc=reset(slow_query(count_rows($a,$Z,$Hd,$fd)));else$zc=false;}}if(+$z&&($Zc===false||$Zc>$z||$E)){echo"<p class='pages'>";$re=($Zc===false?$E+(count($L)>=$z?2:1):floor(($Zc-1)/$z));if($x!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(251)."', '".($E+1)."'), event); return false;\">".lang(251)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($re,$E+5);$s++)echo
pagination($s,$E);if($re>0){echo($E+5<$re?" ...":""),($zc&&$Zc!==false?pagination($re,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$re'>".lang(252)."</a>");}echo(($Zc===false?count($L)+1:$Zc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(253).'...\');" class="loadmore">'.lang(254).'</a>':'');}else{echo
lang(251).":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($re>$E?pagination($E+1,$E).($re>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Zc!==false?"(".($zc?"":"~ ").lang(142,$Zc).") ":"");$Xb=($zc?"":"~ ").$Zc;echo
...
';}$Xc=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($Xc['sql']);break;}}if($Xc){print_fieldset("export",lang(70)." <span id='selected2'></span>");$tf=$b->dumpOutput();echo($tf?html_select("output",$tf,$ya["output"])." ":""),html_select("format",$Xc,$ya["format"])," <input type='submit' name='export' value='".lang(70)."'>\n","</div></fieldset>\n";}echo(!$fd&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",lang(69),!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ya["format"],1);echo" <input type='submit' name='import' value='".lang(69)."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($pc,'strlen'),$e);echo"<p><input type='hidden' name='token' value='$Lh'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$P=isset($_GET["status"]);page_header($P?lang(109):lang(108));$ti=($P?show_status():show_variables());if(!$ti)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($ti
