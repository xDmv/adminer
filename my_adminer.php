...
* @version 4.3.0
*/error_reporting(6135);$Oc=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($Oc||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$ei=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($ei)$$X=$ei;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");header("Cache-Control: immutable");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
idf_unescape($u){$Yd=substr($u,-1);return
str_replace($Yd.$Yd,$Yd,substr($u,1,-1));}function
escape_string($X){return
substr(q($X),1,-1);}function
number($X){return
preg_replace('~[^0-9]+~','',$X);}function
remove_slashes($Uf,$Oc=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($Uf)){foreach($X
as$Nd=>$W){unset($Uf[$y][$Nd]);if(is_array($W)){$Uf[$y][stripslashes($Nd)]=$W;$Uf[]=&$Uf[$y][stripslashes($Nd)];}else$Uf[$y][stripslashes($Nd)]=($Oc?$W:stripslashes($W));}}}}function
...
nl_br($Q){return
str_replace("\n","<br>",$Q);}function
checkbox($C,$Y,$fb,$Ud="",$Ze="",$kb="",$Vd=""){$J="<input type='checkbox' name='$C' value='".h($Y)."'".($fb?" checked":"").($Vd?" aria-labelledby='$Vd'":"").($Ze?' onclick="'.h($Ze).'"':'').">";return($Ud!=""||$kb?"<label".($kb?" class='$kb'":"").">$J".h($Ud)."</label>":$J);}function
optionlist($ff,$Gg=null,$mi=false){$J="";foreach($ff
as$Nd=>$W){$gf=array($Nd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Nd).'">';$gf=$W;}foreach($gf
as$y=>$X)$J.='<option'.($mi||is_string($y)?' value="'.h($y).'"':'').(($mi||is_string($y)?(string)$y:$X)===$Gg?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$ff,$Y="",$Ye=true,$Vd=""){if($Ye)return"<select name='".h($C)."'".(is_string($Ye)?' onchange="'.h($Ye).'"':"").($Vd?" aria-labelledby='$Vd'":"").">".optionlist($ff,$Y)."</select>";$J="";foreach($ff
as$y=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($y)."'".($y==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ka,$ff,$Y="",$Gf=""){return($ff?"<select$Ka><option value=''>$Gf".optionlist($ff,$Y,true)."</select>":"<input$Ka size='10' value='".h($Y)."' placeholder='$Gf'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($t,$de,$yi=false,$Ze=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($Ze)."return !toggle('fieldset-$t');\">$de</a></legend><div id='fieldset-$t'".($yi?"":" class='hidden'").">\n";}function
...
ini_bool($_d){$X=ini_get($_d);return(preg_match('~^(on|true|yes)$~i',$X)||(int)$X);}function
sid(){static$J;if($J===null)$J=(SID&&!($_COOKIE&&ini_bool("session.use_cookies")));return$J;}function
set_password($ti,$N,$V,$G){$_SESSION["pwds"][$ti][$N][$V]=($_COOKIE["adminer_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
...
get_rows($H,$h=null,$n="<p class='error'>"){global$g;$yb=(is_object($h)?$h:$g);$J=array();$I=$yb->query($H);if(is_object($I)){while($K=$I->fetch_assoc())$J[]=$K;}elseif(!$I&&!is_object($h)&&$n&&defined("PAGE_HEADER"))echo$n.error()."\n";return$J;}function
...
cookie($C,$Y,$ge=2592000){global$ba;return
header("Set-Cookie: $C=".urlencode($Y).($ge?"; expires=".gmdate("D, d M Y H:i:s",time()+$ge)." GMT":"")."; path=".preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]).($ba?"; secure":"")."; HttpOnly; SameSite=lax");}function
restart_session(){if(!ini_bool("session.use_cookies"))session_start();}function
stop_session(){if(!ini_bool("session.use_cookies"))session_write_close();}function&get_session($y){return$_SESSION[$y][DRIVER][SERVER][$_GET["username"]];}function
set_session($y,$X){$_SESSION[$y][DRIVER][SERVER][$_GET["username"]]=$X;}function
auth_url($ti,$N,$V,$m=null){global$bc;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($bc))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($ti!="server"||$N!=""?urlencode($ti)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
...
query_redirect($H,$A,$ve,$fg=true,$_c=true,$Hc=false,$Ch=""){global$g,$n,$b;if($_c){$dh=microtime(true);$Hc=!$g->query($H);$Ch=format_time($dh);}$Zg="";if($H)$Zg=$b->messageQuery($H,$Ch);if($Hc){$n=error().$Zg;return
false;}if($fg)redirect($A,$ve.$Zg);return
true;}function
queries($H){global$g;static$Zf=array();static$dh;if(!$dh)$dh=microtime(true);if($H===null)return
array(implode("\n",$Zf),format_time($dh));$Zf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$g->query($H);}function
apply_queries($H,$T,$wc='table'){foreach($T
as$R){if(!queries("$H ".$wc($R)))return
false;}return
true;}function
queries_redirect($A,$ve,$fg){list($Zf,$Ch)=queries(null);return
query_redirect($Zf,$A,$ve,$fg,false,!$fg,$Ch);}function
format_time($dh){return
lang(1,max(0,microtime(true)-$dh));}function
remove_from_uri($uf=""){return
substr(preg_replace("~(?<=[?&])($uf".(SID?"":"|".session_name()).")=[^&]*&~",'',"$_SERVER[REQUEST_URI]&"),0,-1);}function
pagination($E,$Hb){return" ".($E==$Hb?$E+1:'<a href="'.h(remove_from_uri("page").($E?"&page=$E".($_GET["next"]?"&next=".urlencode($_GET["next"]):""):"")).'">'.($E+1)."</a>");}function
get_file($y,$Ob=false){$Mc=$_FILES[$y];if(!$Mc)return
null;foreach($Mc
as$y=>$X)$Mc[$y]=(array)$X;$J='';foreach($Mc["error"]as$y=>$n){if($n)return$n;$C=$Mc["name"][$y];$Kh=$Mc["tmp_name"][$y];$_b=file_get_contents($Ob&&preg_match('~\\.gz$~',$C)?"compress.zlib://$Kh":$Kh);if($Ob){$dh=substr($_b,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$dh,$lg))$_b=iconv("utf-16","utf-8",$_b);elseif($dh=="\xEF\xBB\xBF")$_b=substr($_b,3);$J.=$_b."\n\n";}else$J.=$_b;}return$J;}function
upload_error($n){$se=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($se?" ".lang(3,$se):""):lang(4));}function
repeat_pattern($Ef,$ee){return
str_repeat("$Ef{0,65535}",$ee/65535)."$Ef{0,".($ee%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($Q,$ee=80,$jh=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{10FFFF}]",$ee).")($)?)u",$Q,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$ee).")($)?)",$Q,$B);return
h($B[1]).$jh.(isset($B[2])?"":"<i>...</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Uf,$td=array()){while(list($y,$X)=each($Uf)){if(!in_array($y,$td)){if(is_array($X)){foreach($X
as$Nd=>$W)$Uf[$y."[$Nd]"]=$W;}else
...
enum_input($U,$Ka,$o,$Y,$qc=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$ne);$J=($qc!==null?"<label><input type='$U'$Ka value='$qc'".((is_array($Y)?in_array($qc,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($ne[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$fb=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Ka value='".($s+1)."'".($fb?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$J;}function
input($o,$Y,$r){global$g,$Zh,$b,$x;$C=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$r){$Ea=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$Ea[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$Ea);$r="json";}$og=($x=="mssql"&&$o["auto_increment"]);if($og&&!$_POST["save"])$r=null;$cd=(isset($_GET["select"])||$og?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ka=" name='fields[$C]'";if($o["type"]=="enum")echo
nbsp($cd[""])."<td>".$b->editInput($_GET["edit"],$o,$Ka,$Y);else{$Pc=0;foreach($cd
as$y=>$X){if($y===""||!$X)break;$Pc++;}$Ye=($Pc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Pc > f.selectedIndex) f.selectedIndex = $Pc;\" onkeyup='keyupChange.call(this);'":"");$Ka.=$Ye;$kd=(in_array($r,$cd)||isset($cd[$r]));echo(count($cd)>1?"<select name='function[$C]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($cd,$r===null||$kd?$r:"")."</select>":nbsp(reset($cd))).'<td>';$Bd=$b->editInput($_GET["edit"],$o,$Ka,$Y);if($Bd!="")echo$Bd;elseif(preg_match('~bool~',$o["type"]))echo"<input type='hidden'$Ka value='0'>"."<input type='checkbox'".(in_array(strtolower($Y),array('1','t','true','y','yes','on'))?" checked='checked'":"")."$Ka value='1'>";elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$ne);foreach($ne[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$fb=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($fb?' checked':'')."$Ye>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$C'$Ye>";elseif(($Ah=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($Ah&&$x!="sqlite")$Ka.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Ka.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Ka>".h($Y).'</textarea>';}elseif($r=="json"||preg_match('~^jsonb?$~',$o["type"]))echo"<textarea$Ka cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$ue=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$B)?((preg_match("~binary~",$o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($Zh[$o["type"]]?$Zh[$o["type"]]+($o["unsigned"]?0:1):0));if($x=='sql'&&$g->server_info>=5.6&&preg_match('~time~',$o["type"]))$ue+=7;echo"<input".((!$kd||$r==="")&&preg_match('~(?<!o)int~',$o["type"])&&!preg_match('~\[\]~',$o["full_type"])?" type='number'":"")." value='".h($Y)."'".($ue?" data-maxlength='$ue'":"").(preg_match('~char|binary~',$o["type"])&&$ue>20?" size='40'":"")."$Ka>";}}}function
...
false;return$Y;}if(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads")){$Mc=get_file("fields-$u");if(!is_string($Mc))return
false;return
q($Mc);}return$b->processInput($o,$Y,$r);}function
fields_from_edit(){global$ac;$J=array();foreach((array)$_POST["field_keys"]as$y=>$X){if($X!=""){$X=bracket_escape($X);$_POST["function"][$X]=$_POST["field_funs"][$y];$_POST["fields"][$X]=$_POST["field_vals"][$y];}}foreach((array)$_POST["fields"]as$y=>$X){$C=bracket_escape($y,1);$J[$C]=array("field"=>$C,"privileges"=>array("insert"=>1,"update"=>1),"null"=>1,"auto_increment"=>($y==$ac->primary),);}return$J;}function
search_tables(){global$b,$g;$_GET["where"][0]["op"]="LIKE %%";$_GET["where"][0]["val"]=$_POST["query"];$Yc=false;foreach(table_status('',true)as$R=>$S){$C=$b->tableName($S);if(isset($S["Engine"])&&$C!=""&&(!$_POST["tables"]||in_array($R,$_POST["tables"]))){$I=$g->query("SELECT".limit("1 FROM ".table($R)," WHERE ".implode(" AND ",$b->selectSearchProcess(fields($R),array())),1));if(!$I||$I->fetch_row()){if(!$Yc){echo"<ul>\n";$Yc=true;}echo"<li>".($I?"<a href='".h(ME."select=".urlencode($R)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$C</a>\n":"$C: <span class='error'>".error()."</span>\n");}}}echo($Yc?"</ul>":"<p class='message'>".lang(9))."\n";}function
dump_headers($rd,$Ee=false){global$b;$J=$b->dumpHeaders($rd,$Ee);$sf=$_POST["output"];if($sf!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($rd).".$J".($sf!="file"&&!preg_match('~[^0-9a-z]~',$sf)?".$sf":""));session_write_close();ob_flush();flush();return$J;}function
dump_csv($K){foreach($K
as$y=>$X){if(preg_match("~[\"\n,;\t]~",$X)||$X==="")$K[$y]='"'.str_replace('"','""',$X).'"';}echo
implode(($_POST["format"]=="csv"?",":($_POST["format"]=="tsv"?"\t":";")),$K)."\r\n";}function
apply_sql_function($r,$d){return($r?($r=="unixepoch"?"DATETIME($d, '$r')":($r=="count distinct"?"COUNT(DISTINCT ":strtoupper("$r("))."$d)"):$d);}function
get_temp_dir(){$J=ini_get("upload_tmp_dir");if(!$J){if(function_exists('sys_get_temp_dir'))$J=sys_get_temp_dir();else{$Nc=@tempnam("","");if(!$Nc)return
false;$J=dirname($Nc);unlink($Nc);}}return$J;}function
password_file($i){$Nc=get_temp_dir()."/adminer.key";$J=@file_get_contents($Nc);if($J||!$i)return$J;$ad=@fopen($Nc,"w");if($ad){chmod($Nc,0660);$J=rand_string();fwrite($ad,$J);fclose($ad);}return$J;}function
rand_string(){return
md5(uniqid(mt_rand(),true));}function
select_value($X,$_,$o,$Bh){global$b,$ba;if(is_array($X)){$J="";foreach($X
as$Nd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Nd):"")."<td>".select_value($W,$_,$o,$Bh);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="mailto:$X";if($Wf=is_url($X))$_=(($Wf=="http"&&$ba)||preg_match('~WebKit|Firefox~i',$_SERVER["HTTP_USER_AGENT"])?$X:"https://www.adminer.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$o);if($J!==null){if($J==="")$J="&nbsp;";elseif(!is_utf8($J))$J="\0";elseif($Bh!=""&&is_shortable($o))$J=shorten_utf8($J,max(0,+$Bh));else$J=h($J);}return$b->selectVal($J,$_,$o,$X);}function
is_mail($nc){$Ia='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Zb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$Ef="$Ia+(\\.$Ia+)*@($Zb?\\.)+$Zb";return
is_string($nc)&&preg_match("(^$Ef(,\\s*$Ef)*\$)i",$nc);}function
is_url($Q){$Zb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($Zb?\\.)+$Zb(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$Q,$B)?strtolower($B[1]):"");}function
is_shortable($o){return
preg_match('~char|text|lob|geometry|point|linestring|polygon|string|bytea|json~',$o["type"]);}function
count_rows($R,$Z,$Hd,$fd){global$x;$H=" FROM ".table($R).($Z?" WHERE ".implode(" AND ",$Z):"");return($Hd&&($x=="sql"||count($fd)==1)?"SELECT COUNT(DISTINCT ".implode(", ",$fd).")$H":"SELECT COUNT(*)".($Hd?" FROM (SELECT 1$H$gd) x":$H));}function
slow_query($H){global$b,$Lh;$m=$b->database();$Dh=$b->queryTimeout();if(support("kill")&&is_object($h=connect())&&($m==""||$h->select_db($m))){$Sd=$h->result(connection_id());echo'<script type="text/javascript">
...
fetch_field(){$K=(object)$this->getColumnMeta($this->_offset++);$K->orgtable=$K->table;$K->orgname=$K->name;$K->charsetnr=(in_array("blob",(array)$K->flags)?63:0);return$K;}}}$bc=array();class
Min_SQL{var$_conn;function
__construct($g){$this->_conn=$g;}function
select($R,$M,$Z,$fd,$hf=array(),$z=1,$E=0,$Qf=false){global$b,$x;$Hd=(count($fd)<count($M));$H=$b->selectQueryBuild($M,$Z,$fd,$hf,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$fd&&$Hd&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($fd&&$Hd?"\nGROUP BY ".implode(", ",$fd):"").($hf?"\nORDER BY ".implode(", ",$hf):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");$dh=microtime(true);$J=$this->_conn->query($H);if($Qf)echo$b->selectQuery($H,format_time($dh));return$J;}function
delete($R,$ag,$z=0){$H="FROM ".table($R);return
queries("DELETE".($z?limit1($H,$ag):" $H$ag"));}function
update($R,$O,$ag,$z=0,$Ig="\n"){$ri=array();foreach($O
as$y=>$X)$ri[]="$y = $X";$H=table($R)." SET$Ig".implode(",$Ig",$ri);return
queries("UPDATE".($z?limit1($H,$ag):" $H$ag"));}function
insert($R,$O){return
queries("INSERT INTO ".table($R).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":" DEFAULT VALUES"));}function
insertUpdate($R,$L,$Of){return
false;}function
begin(){return
...
rollback(){return
queries("ROLLBACK");}}$bc["sqlite"]="SQLite 3";$bc["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$Lf=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($Nc){$this->_link=new
SQLite3($Nc);$ui=$this->_link->version();$this->server_info=$ui["versionString"];}function
...
__construct($Nc){$this->server_info=sqlite_libversion();$this->_link=new
SQLiteDatabase($Nc);}function
query($H,$ai=false){$Ae=($ai?"unbufferedQuery":"query");$I=@$this->_link->$Ae($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
fetch_field(){$C=$this->_result->fieldName($this->_offset++);$Ef='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($Ef\\.)?$Ef\$~",$C,$B)){$R=($B[3]!=""?$B[3]:idf_unescape($B[2]));$C=($B[5]!=""?$B[5]:idf_unescape($B[4]));}return(object)array("name"=>$C,"orgname"=>$C,"orgtable"=>$R,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
Min_PDO{var$extension="PDO_SQLite";function
__construct($Nc){$this->dsn(DRIVER.":$Nc","","");}}}if(class_exists("Min_SQLite")){class
Min_DB
extends
Min_SQLite{function
__construct(){parent::__construct(":memory:");}function
select_db($Nc){if(is_readable($Nc)&&$this->query("ATTACH ".$this->quote(preg_match("~(^[/\\\\]|:)~",$Nc)?$Nc:dirname($_SERVER["SCRIPT_FILENAME"])."/$Nc")." AS a")){parent::__construct($Nc);return
true;}return
false;}function
multi_query($H){return$this->_result=$this->query($H);}function
next_result(){return
...
extends
Min_SQL{function
insertUpdate($R,$L,$Of){$ri=array();foreach($L
...
array();}function
limit($H,$Z,$z,$D=0,$Ig=" "){return" $H$Z".($z!==null?$Ig."LIMIT $z".($D?" OFFSET $D":""):"");}function
...
fields($R){global$g;$J=array();$Of="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Pb=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Pb,$B)?str_replace("''","'",$B[1]):($Pb=="NULL"?null:$Pb)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Of!="")$J[$Of]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Of=$C;}}$Zg=$g->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Zg,$ne,PREG_SET_ORDER);foreach($ne
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$Zg=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Zg,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$ne,PREG_SET_ORDER);foreach($ne
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($R)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$bh=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$h);foreach(get_rows("PRAGMA index_list(".table($R).")",$h)as$K){$C=$K["name"];$v=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$v["lengths"]=array();$v["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$h)as$zg){$v["columns"][]=$zg["name"];$v["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$bh[$C],$lg)){preg_match_all('/("[^"]*+")+( DESC)?/',$lg[2],$ne);foreach($ne[2]as$y=>$X){if($X)$v["descs"][$y]='1';}}if(!$J[""]||$v["type"]!="UNIQUE"||$v["columns"]!=$J[""]["columns"]||$v["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$v;}return$J;}function
...
h($g->error);}function
check_sqlite_name($C){global$g;$Gc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Gc)\$~",$C)){$g->error=lang(21,str_replace("|",", ",$Gc));return
false;}return
true;}function
create_database($m,$pb){global$g;if(file_exists($m)){$g->error=lang(22);return
false;}if(!check_sqlite_name($m))return
false;try{$_=new
Min_SQLite($m);}catch(Exception$yc){$g->error=$yc->getMessage();return
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){$li=($R==""||$Uc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$li=true;break;}}$c=array();$qf=array();foreach($p
as$o){if($o[1]){$c[]=($li?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$qf[$o[0]]=$o[1][0];}}if(!$li){foreach($c
as$X){if(!queries("ALTER TABLE ".table($R)." $X"))return
false;}if($R!=$C&&!queries("ALTER TABLE ".table($R)." RENAME TO ".table($C)))return
false;}elseif(!recreate_table($R,$C,$c,$qf,$Uc))return
false;if($Ma)queries("UPDATE sqlite_sequence SET seq = $Ma WHERE name = ".q($C));return
true;}function
recreate_table($R,$C,$p,$qf,$Uc,$w=array()){if($R!=""){if(!$p){foreach(fields($R)as$y=>$o){$p[]=process_field($o,$o);$qf[$y]=idf_escape($y);}}$Pf=false;foreach($p
as$o){if($o[6])$Pf=true;}$ec=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$ec[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$Qd=>$v){$e=array();foreach($v["columns"]as$y=>$d){if(!$qf[$d])continue
2;$e[]=$qf[$d].($v["descs"][$y]?" DESC":"");}if(!$ec[$Qd]){if($v["type"]!="PRIMARY"||!$Pf)$w[]=array($v["type"],$Qd,$e);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$Uc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($R)as$Qd=>$q){foreach($q["source"]as$y=>$d){if(!$qf[$d])continue
2;$q["source"][$y]=idf_unescape($qf[$d]);}if(!isset($Uc[" $Qd"]))$Uc[]=" ".format_foreign_key($q);}queries("BEGIN");}foreach($p
as$y=>$o)$p[$y]="  ".implode($o);$p=array_merge($p,array_filter($Uc));if(!queries("CREATE TABLE ".table($R!=""?"adminer_$C":$C)." (\n".implode(",\n",$p)."\n)"))return
false;if($R!=""){if($qf&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ",$qf).") SELECT ".implode(", ",array_map('idf_escape',array_keys($qf)))." FROM ".table($R)))return
...
move_tables($T,$wi,$wh){return
false;}function
trigger($C){global$g;if($C=="")return
array("Statement"=>"BEGIN\n\t;\nEND");$u='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$Vh=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|",$Vh["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$g->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$Qe=$B[3];return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($Qe?" OF":""),"Of"=>($Qe[0]=='`'||$Qe[0]=='"'?idf_unescape($Qe):$Qe),"Trigger"=>$C,"Statement"=>$B[4],);}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$ef){list($y,$X)=explode("=",$ef,2);$J[$y]=$X;}return$J;}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Jc){return
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$Jc);}$x="sqlite";$Zh=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$gh=array_keys($Zh);$gi=array();$cf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$cd=array("hex","length","lower","round","unixepoch","upper");$hd=array("avg","count","count distinct","group_concat","max","min","sum");$jc=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$bc["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$Lf=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
Min_DB{var$extension="PgSQL",$_link,$_result,$_string,$_database=true,$server_info,$affected_rows,$error;function
_error($uc,$n){if(ini_bool("html_errors"))$n=html_entity_decode(strip_tags($n));$n=preg_replace('~^[^:]*: ~','',$n);$this->error=$n;}function
...
query($H,$ai=false){$I=@pg_query($this->_link,$H);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);return
...
extends
Min_SQL{function
insertUpdate($R,$L,$Of){global$g;foreach($L
as$O){$hi=array();$Z=array();foreach($O
as$y=>$X){$hi[]="$y = $X";if(isset($Of[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$hi)." WHERE ".implode(" AND ",$Z))&&$g->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
...
AND a.attnum > 0
ORDER BY a.attnum")as$K){preg_match('~([^([]+)(\((.*)\))?([a-z ]+)?((\[[0-9]*])*)$~',$K["full_type"],$B);list(,$U,$ee,$K["length"],$wa,$Fa)=$B;$K["length"].=$Fa;$eb=$U.$wa;if(isset($Ca[$eb])){$K["type"]=$Ca[$eb];$K["full_type"]=$K["type"].$ee.$Fa;}else{$K["type"]=$U;$K["full_type"]=$K["type"].$ee.$wa.$Fa;}$K["null"]=!$K["attnotnull"];$K["auto_increment"]=preg_match('~^nextval\\(~i',$K["default"]);$K["privileges"]=array("insert"=>1,"select"=>1,"update"=>1);if(preg_match('~(.+)::[^)]+(.*)~',$K["default"],$B))$K["default"]=($B[1][0]=="'"?idf_unescape($B[1]):$B[1]).$B[2];$J[$K["field"]]=$K;}return$J;}function
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$ph=$h->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($R));$e=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $ph AND attnum > 0",$h);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $ph AND ci.oid = i.indexrelid",$h)as$K){$mg=$K["relname"];$J[$mg]["type"]=($K["indispartial"]?"INDEX":($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX")));$J[$mg]["columns"]=array();foreach(explode(" ",$K["indkey"])as$xd)$J[$mg]["columns"][]=$e[$xd];$J[$mg]["descs"]=array();foreach(explode(" ",$K["indoption"])as$yd)$J[$mg]["descs"][]=($yd&1?'1':null);$J[$mg]["lengths"]=array();}return$J;}function
foreign_keys($R){global$Xe;$J=array();foreach(get_rows("SELECT conname, condeferrable::int AS deferrable, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
WHERE conrelid = (SELECT pc.oid FROM pg_class AS pc INNER JOIN pg_namespace AS pn ON (pn.oid = pc.relnamespace) WHERE pc.relname = ".q($R)." AND pn.nspname = current_schema())
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$B)){$K['source']=array_map('trim',explode(',',$B[1]));if(preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~',$B[2],$me)){$K['ns']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$me[2]));$K['table']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$me[4]));}$K['target']=array_map('trim',explode(',',$B[3]));$K['on_delete']=(preg_match("~ON DELETE ($Xe)~",$B[4],$me)?$me[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($Xe)~",$B[4],$me)?$me[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){$c=array();$Zf=array();foreach($p
as$o){$d=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $d";else{$qi=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($d!=$X[0])$Zf[]="ALTER TABLE ".table($R)." RENAME $d TO $X[0]";$c[]="ALTER $d TYPE$X[1]";if(!$X[6]){$c[]="ALTER $d ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $d ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$qi!="")$Zf[]="COMMENT ON COLUMN ".table($R).".$X[0] IS ".($qi!=""?substr($qi,9):"''");}}$c=array_merge($c,$Uc);if($R=="")array_unshift($Zf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Zf,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""&&$R!=$C)$Zf[]="ALTER TABLE ".table($R)." RENAME TO ".table($C);if($R!=""||$vb!="")$Zf[]="COMMENT ON TABLE ".table($C)." IS ".q($vb);if($Ma!=""){}foreach($Zf
as$H){if(!queries($H))return
false;}return
true;}function
alter_indexes($R,$c){$i=array();$cc=array();$Zf=array();foreach($c
as$X){if($X[0]!="INDEX")$i[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$cc[]=idf_escape($X[1]);else$Zf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R)." (".implode(", ",$X[2]).")";}if($i)array_unshift($Zf,"ALTER TABLE ".table($R).implode(",",$i));if($cc)array_unshift($Zf,"DROP INDEX ".implode(", ",$cc));foreach($Zf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($T){return
...
false;}return
true;}function
move_tables($T,$wi,$wh){foreach(array_merge($T,$wi)as$R){$P=table_status($R);if(!queries("ALTER ".strtoupper($P["Engine"])." ".table($R)." SET SCHEMA ".idf_escape($wh)))return
false;}return
true;}function
...
found_rows($S,$Z){global$g;if(preg_match("~ rows=([0-9]+)~",$g->result("EXPLAIN SELECT * FROM ".idf_escape($S["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$lg))return$lg[1];return
false;}function
types(){return
...
set_schema($Bg){global$g,$Zh,$gh;$J=$g->query("SET search_path TO ".idf_escape($Bg));foreach(types()as$U){if(!isset($Zh[$U])){$Zh[$U]=0;$gh[lang(23)][]=$U;}}return$J;}function
create_sql($R,$Ma){global$g;$J='';$sg=array();$Kg=array();$P=table_status($R);$p=fields($R);$w=indexes($R);ksort($w);$Sc=foreign_keys($R);ksort($Sc);$Wh=triggers($R);if(!$P||empty($p))return
false;$J="CREATE TABLE ".idf_escape($P['nspname']).".".idf_escape($P['Name'])." (\n    ";foreach($p
as$Lc=>$o){$xf=idf_escape($o['field']).' '.$o['full_type'].(is_null($o['default'])?"":" DEFAULT $o[default]").($o['attnotnull']?"":" NOT NULL");$sg[]=$xf;if(preg_match('~nextval\(\'([^\']+)\'\)~',$o['default'],$ne)){$Jg=$ne[1];$Yg=reset(get_rows("SELECT * FROM $Jg"));$Kg[]="CREATE SEQUENCE $Jg INCREMENT $Yg[increment_by] MINVALUE $Yg[min_value] MAXVALUE $Yg[max_value] START ".($Ma?$Yg['last_value']:1)." CACHE $Yg[cache_value];";}}if(!empty($Kg))$J=implode("\n\n",$Kg)."\n\n$J";foreach($w
as$vd=>$v){switch($v['type']){case'UNIQUE':$sg[]="CONSTRAINT ".idf_escape($vd)." UNIQUE (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;case'PRIMARY':$sg[]="CONSTRAINT ".idf_escape($vd)." PRIMARY KEY (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;}}foreach($Sc
as$Rc=>$Qc)$sg[]="CONSTRAINT ".idf_escape($Rc)." $Qc[definition] ".($Qc['deferrable']?'DEFERRABLE':'NOT DEFERRABLE');$J.=implode(",\n    ",$sg)."\n) WITH (oids = ".($P['Oid']?'true':'false').");";foreach($w
as$vd=>$v){if($v['type']=='INDEX')$J.="\n\nCREATE INDEX ".idf_escape($vd)." ON ".idf_escape($P['nspname']).".".idf_escape($P['Name'])." USING btree (".implode(', ',array_map('idf_escape',$v['columns'])).");";}if($P['Comment'])$J.="\n\nCOMMENT ON TABLE ".idf_escape($P['nspname']).".".idf_escape($P['Name'])." IS ".q($P['Comment']).";";foreach($p
as$Lc=>$o){if($o['comment'])$J.="\n\nCOMMENT ON COLUMN ".idf_escape($P['nspname']).".".idf_escape($P['Name']).".".idf_escape($Lc)." IS ".q($o['comment']).";";}foreach($Wh
...
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Jc){global$g;return
preg_match('~^(database|table|columns|sql|indexes|comment|view|'.($g->server_info>=9.3?'materializedview|':'').'scheme|processlist|sequence|trigger|type|variables|drop_col|kill|dump)$~',$Jc);}function
kill_process($X){return
queries("SELECT pg_terminate_backend(".number($X).")");}function
connection_id(){return"SELECT pg_backend_pid()";}function
max_connections(){global$g;return$g->result("SHOW max_connections");}$x="pgsql";$Zh=array();$gh=array();foreach(array(lang(24)=>array("smallint"=>5,"integer"=>10,"bigint"=>19,"boolean"=>1,"numeric"=>0,"real"=>7,"double precision"=>16,"money"=>20),lang(25)=>array("date"=>13,"time"=>17,"timestamp"=>20,"timestamptz"=>21,"interval"=>0),lang(26)=>array("character"=>0,"character varying"=>0,"text"=>0,"tsquery"=>0,"tsvector"=>0,"uuid"=>0,"xml"=>0),lang(27)=>array("bit"=>0,"bit varying"=>0,"bytea"=>0),lang(28)=>array("cidr"=>43,"inet"=>43,"macaddr"=>17,"txid_snapshot"=>0),lang(29)=>array("box"=>0,"circle"=>0,"line"=>0,"lseg"=>0,"path"=>0,"point"=>0,"polygon"=>0),)as$y=>$X){$Zh+=$X;$gh[$y]=array_keys($X);}$gi=array();$cf=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","ILIKE","ILIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$cd=array("char_length","lower","round","to_hex","to_timestamp","upper");$hd=array("avg","count","count distinct","max","min","sum");$jc=array(array("char"=>"md5","date|time"=>"now",),array("int|numeric|real|money"=>"+/-","date|time"=>"+ interval/- interval","char|text"=>"||",));}$bc["oracle"]="Oracle";if(isset($_GET["oracle"])){$Lf=array("OCI8","PDO_OCI");define("DRIVER","oracle");if(extension_loaded("oci8")){class
...
select_db($k){return
true;}function
query($H,$ai=false){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$n=oci_error($this->_link);$this->errno=$n["code"];$this->error=$n["message"];return
...
limit($H,$Z,$z,$D=0,$Ig=" "){return($D?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $H$Z) t WHERE rownum <= ".($z+$D).") WHERE rnum > $D":($z!==null?" * FROM (SELECT $H$Z) WHERE rownum <= ".($z+$D):" $H$Z"));}function
limit1($H,$Z){return" $H$Z";}function
...
count_tables($l){return
array();}function
table_status($C=""){$J=array();$Dg=q($C);foreach(get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($C!=""?" AND table_name = $Dg":"")."
UNION SELECT view_name, 'view', 0, 0 FROM user_views".($C!=""?" WHERE view_name = $Dg":"")."
ORDER BY 1")as$K){if($C!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
is_view($S){return$S["Engine"]=="view";}function
fk_support($S){return
true;}function
fields($R){$J=array();foreach(get_rows("SELECT * FROM all_tab_columns WHERE table_name = ".q($R)." ORDER BY column_id")as$K){$U=$K["DATA_TYPE"];$ee="$K[DATA_PRECISION],$K[DATA_SCALE]";if($ee==",")$ee=$K["DATA_LENGTH"];$J[$K["COLUMN_NAME"]]=array("field"=>$K["COLUMN_NAME"],"full_type"=>$U.($ee?"($ee)":""),"type"=>strtolower($U),"length"=>$ee,"default"=>$K["DATA_DEFAULT"],"null"=>($K["NULLABLE"]=="Y"),"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),);}return$J;}function
indexes($R,$h=null){$J=array();foreach(get_rows("SELECT uic.*, uc.constraint_type
FROM user_ind_columns uic
LEFT JOIN user_constraints uc ON uic.index_name = uc.constraint_name AND uic.table_name = uc.table_name
WHERE uic.table_name = ".q($R)."
ORDER BY uc.constraint_type, uic.column_position",$h)as$K){$vd=$K["INDEX_NAME"];$J[$vd]["type"]=($K["CONSTRAINT_TYPE"]=="P"?"PRIMARY":($K["CONSTRAINT_TYPE"]=="U"?"UNIQUE":"INDEX"));$J[$vd]["columns"][]=$K["COLUMN_NAME"];$J[$vd]["lengths"][]=($K["CHAR_LENGTH"]&&$K["CHAR_LENGTH"]!=$K["COLUMN_LENGTH"]?$K["CHAR_LENGTH"]:null);$J[$vd]["descs"][]=($K["DESCEND"]?'1':null);}return$J;}function
...
found_rows($S,$Z){}function
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){$c=$cc=array();foreach($p
as$o){$X=$o[1];if($X&&$o[0]!=""&&idf_escape($o[0])!=$X[0])queries("ALTER TABLE ".table($R)." RENAME COLUMN ".idf_escape($o[0])." TO $X[0]");if($X)$c[]=($R!=""?($o[0]!=""?"MODIFY (":"ADD ("):"  ").implode($X).($R!=""?")":"");else$cc[]=idf_escape($o[0]);}if($R=="")return
...
set_schema($Cg){global$g;return$g->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($Cg));}function
show_variables(){return
...
unconvert_field($o,$J){return$J;}function
support($Jc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Jc);}$x="oracle";$Zh=array();$gh=array();foreach(array(lang(24)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(25)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(26)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$Zh+=$X;$gh[$y]=array_keys($X);}$gi=array();$cf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$cd=array("length","lower","round","upper");$hd=array("avg","count","count distinct","max","min","sum");$jc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$bc["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Lf=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
Min_DB{var$extension="sqlsrv",$_link,$_result,$server_info,$affected_rows,$errno,$error;function
_get_error(){$this->error="";foreach(sqlsrv_errors()as$n){$this->errno=$n["code"];$this->error.="$n[message]\n";}$this->error=rtrim($this->error);}function
connect($N,$V,$G){$this->_link=@sqlsrv_connect($N,array("UID"=>$V,"PWD"=>$G,"CharacterSet"=>"UTF-8"));if($this->_link){$zd=sqlsrv_server_info($this->_link);$this->server_info=$zd['SQLServerVersion'];}else$this->_get_error();return(bool)$this->_link;}function
quote($Q){return"'".str_replace("'","''",$Q)."'";}function
select_db($k){return$this->query("USE ".idf_escape($k));}function
query($H,$ai=false){$I=sqlsrv_query($this->_link,$H);$this->error="";if(!$I){$this->_get_error();return
...
query($H,$ai=false){$I=@mssql_query($H,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
...
extends
Min_SQL{function
insertUpdate($R,$L,$Of){foreach($L
as$O){$hi=array();$Z=array();foreach($O
as$y=>$X){$hi[]="$y = $X";if(isset($Of[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$hi)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
limit($H,$Z,$z,$D=0,$Ig=" "){return($z!==null?" TOP (".($z+$D).")":"")." $H$Z";}function
limit1($H,$Z){return
...
WHERE o.schema_id = SCHEMA_ID(".q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($R))as$K){$U=$K["type"];$ee=(preg_match("~char|binary~",$U)?$K["max_length"]:($U=="decimal"?"$K[precision],$K[scale]":""));$J[$K["name"]]=array("field"=>$K["name"],"full_type"=>$U.($ee?"($ee)":""),"type"=>$U,"length"=>$ee,"default"=>$K["default"],"null"=>$K["is_nullable"],"auto_increment"=>$K["is_identity"],"collation"=>$K["collation_name"],"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),"primary"=>$K["is_identity"],);}return$J;}function
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){$c=array();foreach($p
as$o){$d=idf_escape($o[0]);$X=$o[1];if(!$X)$c["DROP"][]=" COLUMN $d";else{$X[1]=preg_replace("~( COLLATE )'(\\w+)'~","\\1\\2",$X[1]);if($o[0]=="")$c["ADD"][]="\n  ".implode("",$X).($R==""?substr($Uc[$X[0]],16+strlen($X[0])):"");else{unset($X[6]);if($d!=$X[0])queries("EXEC sp_rename ".q(table($R).".$d").", ".q(idf_unescape($X[0])).", 'COLUMN'");$c["ALTER COLUMN ".implode("",$X)][]="";}}}if($R=="")return
queries("CREATE TABLE ".table($C)." (".implode(",",(array)$c["ADD"])."\n)");if($R!=$C)queries("EXEC sp_rename ".q(table($R)).", ".q($C));if($Uc)$c[""]=$Uc;foreach($c
as$y=>$X){if(!queries("ALTER TABLE ".idf_escape($C)." $y".implode(",",$X)))return
false;}return
true;}function
alter_indexes($R,$c){$v=array();$cc=array();foreach($c
as$X){if($X[2]=="DROP"){if($X[0]=="PRIMARY")$cc[]=idf_escape($X[1]);else$v[]=idf_escape($X[1])." ON ".table($R);}elseif(!queries(($X[0]!="PRIMARY"?"CREATE $X[0] ".($X[0]!="INDEX"?"INDEX ":"").idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R):"ALTER TABLE ".table($R)." ADD PRIMARY KEY")." (".implode(", ",$X[2]).")"))return
false;}return(!$v||queries("DROP INDEX ".implode(", ",$v)))&&(!$cc||queries("ALTER TABLE ".table($R)." DROP ".implode(", ",$cc)));}function
...
move_tables($T,$wi,$wh){return
apply_queries("ALTER SCHEMA ".idf_escape($wh)." TRANSFER",array_merge($T,$wi));}function
...
unconvert_field($o,$J){return$J;}function
support($Jc){return
preg_match('~^(columns|database|drop_col|indexes|scheme|sql|table|trigger|view|view_trigger)$~',$Jc);}$x="mssql";$Zh=array();$gh=array();foreach(array(lang(24)=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),lang(25)=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),lang(26)=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),lang(27)=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$y=>$X){$Zh+=$X;$gh[$y]=array_keys($X);}$gi=array();$cf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$cd=array("len","lower","round","upper");$hd=array("avg","count","count distinct","max","min","sum");$jc=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$bc['firebird']='Firebird (alpha)';if(isset($_GET["firebird"])){$Lf=array("interbase");define("DRIVER","firebird");if(extension_loaded("interbase")){class
Min_DB{var$extension="Firebird",$server_info,$affected_rows,$errno,$error,$_link,$_result;function
connect($N,$V,$G){$this->_link=ibase_connect($N,$V,$G);if($this->_link){$ji=explode(':',$N);$this->service_link=ibase_service_attach($ji[0],$V,$G);$this->server_info=ibase_server_info($this->service_link,IBASE_SVC_SERVER_VERSION);}else{$this->errno=ibase_errcode();$this->error=ibase_errmsg();}return(bool)$this->_link;}function
quote($Q){return"'".str_replace("'","''",$Q)."'";}function
select_db($k){return($k=="domain");}function
query($H,$ai=false){$I=ibase_query($H,$this->_link);if(!$I){$this->errno=ibase_errcode();$this->error=ibase_errmsg();return
...
limit($H,$Z,$z,$D=0,$Ig=" "){$J='';$J.=($z!==null?$Ig."FIRST $z".($D?" SKIP $D":""):"");$J.=" $H$Z";return$J;}function
limit1($H,$Z){return
limit($H,$Z,1);}function
...
count_tables($l){return
array();}function
table_status($C="",$Ic=false){global$g;$J=array();$Jb=tables_list();foreach($Jb
...
true;}function
support($Jc){return
preg_match("~^(columns|sql|status|table)$~",$Jc);}$x="firebird";$cf=array("=");$cd=array();$hd=array();$jc=array();}$bc["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Lf=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
Min_DB{var$extension="SimpleXML",$server_info='2009-04-15',$error,$timeout,$next,$affected_rows,$_result;function
select_db($k){return($k=="domain");}function
query($H,$ai=false){$F=array('SelectExpression'=>$H,'ConsistentRead'=>'true');if($this->next)$F['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$F,$this->timeout);if($I===false)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$H)){$kh=0;foreach($I
as$Ld)$kh+=$Ld->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$kh,))));}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result(){return$this->_result;}function
...
_processValue($mc){return(is_object($mc)&&$mc['encoding']=='base64'?base64_decode($mc):(string)$mc);}function
...
Min_Driver
extends
Min_SQL{public$Of="itemName()";function
_chunkRequest($sd,$va,$F,$Bc=array()){global$g;foreach(array_chunk($sd,25)as$ib){$vf=$F;foreach($ib
as$s=>$t){$vf["Item.$s.ItemName"]=$t;foreach($Bc
as$y=>$X)$vf["Item.$s.$y"]=$X;}if(!sdb_request($va,$vf))return
false;}$g->affected_rows=count($sd);return
true;}function
_extractIds($R,$ag,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$ag,$ne))$J=array_map('idf_unescape',$ne[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$ag.($z?" LIMIT 1":"")))as$Ld)$J[]=$Ld->Name;}return$J;}function
select($R,$M,$Z,$fd,$hf=array(),$z=1,$E=0,$Qf=false){global$g;$g->next=$_GET["next"];$J=parent::select($R,$M,$Z,$fd,$hf,$z,$E,$Qf);$g->next=0;return$J;}function
delete($R,$ag,$z=0){return$this->_chunkRequest($this->_extractIds($R,$ag,$z),'BatchDeleteAttributes',array('DomainName'=>$R));}function
update($R,$O,$ag,$z=0,$Ig="\n"){$Qb=array();$Cd=array();$s=0;$sd=$this->_extractIds($R,$ag,$z);$t=idf_unescape($O["`itemName()`"]);unset($O["`itemName()`"]);foreach($O
as$y=>$X){$y=idf_unescape($y);if($X=="NULL"||($t!=""&&array($t)!=$sd))$Qb["Attribute.".count($Qb).".Name"]=$y;if($X!="NULL"){foreach((array)$X
as$Nd=>$W){$Cd["Attribute.$s.Name"]=$y;$Cd["Attribute.$s.Value"]=(is_array($X)?$W:idf_unescape($W));if(!$Nd)$Cd["Attribute.$s.Replace"]="true";$s++;}}}$F=array('DomainName'=>$R);return(!$Cd||$this->_chunkRequest(($t!=""?array($t):$sd),'BatchPutAttributes',$F,$Cd))&&(!$Qb||$this->_chunkRequest($sd,'BatchDeleteAttributes',$F,$Qb));}function
...
table_status($C="",$Ic=false){$J=array();foreach(($C!=""?array($C=>true):tables_list())as$R=>$U){$K=array("Name"=>$R,"Auto_increment"=>"");if(!$Ic){$_e=sdb_request('DomainMetadata',array('DomainName'=>$R));if($_e){foreach(array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$y=>$X)$K[$y]=(string)$_e->$X;}}if($C!="")return$K;$J[$R]=$K;}return$J;}function
...
idf_escape($u);}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
limit($H,$Z,$z,$D=0,$Ig=" "){return" $H$Z".($z!==null?$Ig."LIMIT $z":"");}function
unconvert_field($o,$J){return$J;}function
fk_support($S){}function
engines(){return
array();}function
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($T){foreach($T
...
last_id(){}function
hmac($Ba,$Jb,$y,$eg=false){$Va=64;if(strlen($y)>$Va)$y=pack("H*",$Ba($y));$y=str_pad($y,$Va,"\0");$Od=$y^str_repeat("\x36",$Va);$Pd=$y^str_repeat("\x5C",$Va);$J=$Ba($Pd.pack("H*",$Ba($Od.$Jb)));if($eg)$J=pack("H*",$J);return$J;}function
sdb_request($va,$F=array()){global$b,$g;list($pd,$F['AWSAccessKeyId'],$Eg)=$b->credentials();$F['Action']=$va;$F['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$F['Version']='2009-04-15';$F['SignatureVersion']=2;$F['SignatureMethod']='HmacSHA1';ksort($F);$H='';foreach($F
as$y=>$X)$H.='&'.rawurlencode($y).'='.rawurlencode($X);$H=str_replace('%7E','~',substr($H,1));$H.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$pd)."\n/\n$H",$Eg,true)));@ini_set('track_errors',1);$Mc=@file_get_contents((preg_match('~^https?://~',$pd)?$pd:"http://$pd"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$H,'ignore_errors'=>1,))));if(!$Mc){$g->error=$php_errormsg;return
false;}libxml_use_internal_errors(true);$Di=simplexml_load_string($Mc);if(!$Di){$n=libxml_get_last_error();$g->error=$n->message;return
false;}if($Di->Errors){$n=$Di->Errors->Error;$g->error="$n->Message ($n->Code)";return
false;}$g->error='';$vh=$va."Result";return($Di->$vh?$Di->$vh:true);}function
sdb_request_all($va,$vh,$F=array(),$Dh=0){$J=array();$dh=($Dh?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$Di=sdb_request($va,$F);if(!$Di)break;foreach($Di->$vh
as$mc)$J[]=$mc;if($z&&count($J)>=$z){$_GET["next"]=$Di->NextToken;break;}if($Dh&&microtime(true)-$dh>$Dh)return
false;$F['NextToken']=$Di->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($Di->NextToken);return$J;}$x="simpledb";$cf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$cd=array();$hd=array("count");$jc=array(array("json"));}$bc["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Lf=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$error,$last_id,$_link,$_db;function
connect($N,$V,$G){global$b;$m=$b->database();$ff=array();if($V!=""){$ff["username"]=$V;$ff["password"]=$G;}if($m!="")$ff["db"]=$m;try{$this->_link=@new
MongoClient("mongodb://$N",$ff);return
true;}catch(Exception$yc){$this->error=$yc->getMessage();return
false;}}function
query($H){return
false;}function
...
Min_Driver
extends
Min_SQL{public$Of="_id";function
select($R,$M,$Z,$fd,$hf=array(),$z=1,$E=0,$Qf=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Vg=array();foreach($hf
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Eb);$Vg[$X]=($Eb?-1:1);}return
new
Min_Result($this->_conn->_db->selectCollection($R)->find(array(),$M)->sort($Vg)->limit(+$z)->skip($E*$z));}function
insert($R,$O){try{$J=$this->_conn->_db->selectCollection($R)->insert($O);$this->_conn->errno=$J['code'];$this->_conn->error=$J['err'];$this->_conn->last_id=$O['_id'];return!$J['err'];}catch(Exception$yc){$this->_conn->error=$yc->getMessage();return
false;}}}function
connect(){global$b;$g=new
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2]))return$g;return$g->error;}function
error(){global$g;return
h($g->error);}function
logged_user(){global$b;$j=$b->credentials();return$j[1];}function
get_databases($Tc){global$g;$J=array();$Nb=$g->_link->listDBs();foreach($Nb['databases']as$m)$J[]=$m['name'];return$J;}function
collations(){return
array();}function
db_collation($m,$qb){}function
count_tables($l){global$g;$J=array();foreach($l
...
table_status($C="",$Ic=false){$J=array();foreach(tables_list()as$R=>$U){$J[$R]=array("Name"=>$R);if($C==$R)return$J[$R];}return$J;}function
information_schema(){}function
is_view($S){}function
drop_databases($l){global$g;foreach($l
as$m){$pg=$g->_link->selectDB($m)->drop();if(!$pg['ok'])return
false;}return
true;}function
indexes($R,$h=null){global$g;$J=array();foreach($g->_db->selectCollection($R)->getIndexInfo()as$v){$Tb=array();foreach($v["key"]as$d=>$U)$Tb[]=($U==-1?'1':null);$J[$v["name"]]=array("type"=>($v["name"]=="_id_"?"PRIMARY":($v["unique"]?"UNIQUE":"INDEX")),"columns"=>array_keys($v["key"]),"lengths"=>array(),"descs"=>$Tb,);}return$J;}function
fields($R){return
fields_from_edit();}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f){global$g;if($R==""){$g->_db->createCollection($C);return
true;}}function
drop_tables($T){global$g;foreach($T
as$R){$pg=$g->_db->selectCollection($R)->drop();if(!$pg['ok'])return
false;}return
true;}function
truncate_tables($T){global$g;foreach($T
as$R){$pg=$g->_db->selectCollection($R)->remove();if(!$pg['ok'])return
false;}return
true;}function
...
as$d){$d=preg_replace('~ DESC$~','',$d,1,$Eb);$e[$d]=($Eb?-1:1);}$J=$g->_db->selectCollection($R)->ensureIndex($e,array("unique"=>($U=="UNIQUE"),"name"=>$C,));}if($J['errmsg']){$g->error=$J['errmsg'];return
false;}}return
true;}function
...
support($Jc){return
preg_match("~database|indexes~",$Jc);}$x="mongo";$cf=array("=");$cd=array();$hd=array();$jc=array(array("json"));}$bc["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Lf=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($Cf,$_b=array(),$Ae='GET'){@ini_set('track_errors',1);$Mc=@file_get_contents($this->_url.'/'.ltrim($Cf,'/'),false,stream_context_create(array('http'=>array('method'=>$Ae,'content'=>json_encode($_b),'ignore_errors'=>1,))));if(!$Mc){$this->error=$php_errormsg;return$Mc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Mc;return
false;}$J=json_decode($Mc,true);if($J===null){$this->errno=json_last_error();if(function_exists('json_last_error_msg'))$this->error=json_last_error_msg();else{$zb=get_defined_constants(true);foreach($zb['json']as$C=>$Y){if($Y==$this->errno&&preg_match('~^JSON_ERROR_~',$C)){$this->error=$C;break;}}}}return$J;}function
query($Cf,$_b=array(),$Ae='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($Cf,'/'),$_b,$Ae);}function
...
extends
Min_SQL{function
select($R,$M,$Z,$fd,$hf=array(),$z=1,$E=0,$Qf=false){global$b;$Jb=array();$H="$R/_search";if($M!=array("*"))$Jb["fields"]=$M;if($hf){$Vg=array();foreach($hf
as$nb){$nb=preg_replace('~ DESC$~','',$nb,1,$Eb);$Vg[]=($Eb?array($nb=>"desc"):$nb);}$Jb["sort"]=$Vg;}if($z){$Jb["size"]=+$z;if($E)$Jb["from"]=($E*$z);}foreach($Z
as$X){list($nb,$af,$X)=explode(" ",$X,3);if($nb=="_id")$Jb["query"]["ids"]["values"][]=$X;elseif($nb.$X!=""){$zh=array("term"=>array(($nb!=""?$nb:"_all")=>$X));if($af=="=")$Jb["query"]["filtered"]["filter"]["and"][]=$zh;else$Jb["query"]["filtered"]["query"]["bool"]["must"][]=$zh;}}if($Jb["query"]&&!$Jb["query"]["filtered"]["query"]&&!$Jb["query"]["ids"])$Jb["query"]["filtered"]["query"]=array("match_all"=>array());$dh=microtime(true);$Dg=$this->_conn->query($H,$Jb);if($Qf)echo$b->selectQuery("$H: ".print_r($Jb,true),format_time($dh));if(!$Dg)return
false;$J=array();foreach($Dg['hits']['hits']as$od){$K=array();if($M==array("*"))$K["_id"]=$od["_id"];$p=$od['_source'];if($M!=array("*")){$p=array();foreach($M
as$y)$p[$y]=$od['fields'][$y];}foreach($p
as$y=>$X){if($Jb["fields"])$X=$X[0];$K[$y]=(is_array($X)?json_encode($X):$X);}$J[]=$K;}return
new
...
table_status($C="",$Ic=false){global$g;$Dg=$g->query("_search?search_type=count",array("facets"=>array("count_by_type"=>array("terms"=>array("field"=>"_type",)))),"POST");$J=array();if($Dg){foreach($Dg["facets"]["count_by_type"]["terms"]as$R){$J[$R["term"]]=array("Name"=>$R["term"],"Engine"=>"table","Rows"=>$R["count"],);if($C!=""&&$C==$R["term"])return$J[$C];}}return$J;}function
error(){global$g;return
h($g->error);}function
information_schema(){}function
is_view($S){}function
...
fields($R){global$g;$I=$g->query("$R/_mapping");$J=array();if($I){$ke=$I[$R]['properties'];if(!$ke)$ke=$I[$g->_db]['mappings'][$R]['properties'];if($ke){foreach($ke
...
as$R)$J=$J&&$g->query(urlencode($R),array(),'DELETE');return$J;}$x="elastic";$cf=array("=","query");$cd=array();$hd=array();$jc=array(array("json"));}$bc=array("server"=>"MySQL")+$bc;if(!defined("DRIVER")){$Lf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N="",$V="",$G="",$k=null,$Hf=null,$Ug=null){mysqli_report(MYSQLI_REPORT_OFF);list($pd,$Hf)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$pd:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),$k,(is_numeric($Hf)?$Hf:ini_get("mysqli.default_port")),(!is_numeric($Hf)?$Hf:$Ug));return$J;}function
...
query($H,$ai=false){$I=@($ai?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
select_db($k){return$this->query("USE ".idf_escape($k));}function
query($H,$ai=false){$this->setAttribute(1000,!$ai);return
parent::query($H,$ai);}}}class
Min_Driver
extends
Min_SQL{function
insert($R,$O){return($O?parent::insert($R,$O):queries("INSERT INTO ".table($R)." ()\nVALUES ()"));}function
insertUpdate($R,$L,$Of){$e=array_keys(reset($L));$Mf="INSERT INTO ".table($R)." (".implode(", ",$e).") VALUES\n";$ri=array();foreach($e
as$y)$ri[$y]="$y = VALUES($y)";$jh="\nON DUPLICATE KEY UPDATE ".implode(", ",$ri);$ri=array();$ee=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($ri&&(strlen($Mf)+$ee+strlen($Y)+strlen($jh)>1e6)){if(!queries($Mf.implode(",\n",$ri).$jh))return
false;$ri=array();$ee=0;}$ri[]=$Y;$ee+=strlen($Y)+2;}return
queries($Mf.implode(",\n",$ri).$jh);}}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
table($u){return
idf_escape($u);}function
connect(){global$b,$Zh,$gh;$g=new
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2])){$g->set_charset(charset($g));$g->query("SET sql_quote_show_create = 1, autocommit = 1");if(version_compare($g->server_info,'5.7.8')>=0){$gh[lang(26)][]="json";$Zh["json"]=4294967295;}return$g;}$J=$g->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($_g=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$_g;return$J;}function
get_databases($Tc){global$g;$J=get_session("dbs");if($J===null){$H=($g->server_info>=5?"SELECT SCHEMA_NAME FROM information_schema.SCHEMATA":"SHOW DATABASES");$J=($Tc?slow_query($H):get_vals($H));restart_session();set_session("dbs",$J);stop_session();}return$J;}function
limit($H,$Z,$z,$D=0,$Ig=" "){return" $H$Z".($z!==null?$Ig."LIMIT $z".($D?" OFFSET $D":""):"");}function
limit1($H,$Z){return
limit($H,$Z,1);}function
...
table_status($C="",$Ic=false){global$g;$J=array();foreach(get_rows($Ic&&$g->server_info>=5?"SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ".($C!=""?"AND TABLE_NAME = ".q($C):"ORDER BY Name"):"SHOW TABLE STATUS".($C!=""?" LIKE ".q(addcslashes($C,"%_\\")):""))as$K){if($K["Engine"]=="InnoDB")$K["Comment"]=preg_replace('~(?:(.+); )?InnoDB free: .*~','\\1',$K["Comment"]);if(!isset($K["Engine"]))$K["Comment"]="";if($C!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
...
foreign_keys($R){global$g,$Xe;static$Ef='`(?:[^`]|``)+`';$J=array();$Fb=$g->result("SHOW CREATE TABLE ".table($R),1);if($Fb){preg_match_all("~CONSTRAINT ($Ef) FOREIGN KEY ?\\(((?:$Ef,? ?)+)\\) REFERENCES ($Ef)(?:\\.($Ef))? \\(((?:$Ef,? ?)+)\\)(?: ON DELETE ($Xe))?(?: ON UPDATE ($Xe))?~",$Fb,$ne,PREG_SET_ORDER);foreach($ne
as$B){preg_match_all("~$Ef~",$B[2],$Wg);preg_match_all("~$Ef~",$B[5],$wh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Wg[0]),"target"=>array_map('idf_unescape',$wh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$g;return
...
rename_database($C,$pb){$J=false;if(create_database($C,$pb)){$ng=array();foreach(tables_list()as$R=>$U)$ng[]=table($R)." TO ".idf_escape($C).".".table($R);$J=(!$ng||queries("RENAME TABLE ".implode(", ",$ng)));if($J)queries("DROP DATABASE ".idf_escape(DB));restart_session();set_session("dbs",null);}return$J;}function
...
alter_table($R,$C,$p,$Uc,$vb,$rc,$pb,$Ma,$_f,$De){$c=array();foreach($p
as$o)$c[]=($o[1]?($R!=""?($o[0]!=""?"CHANGE ".idf_escape($o[0]):"ADD"):" ")." ".implode($o[1]).($R!=""?$o[2]:""):"DROP ".idf_escape($o[0]));$c=array_merge($c,$Uc);$P=($vb!==null?" COMMENT=".q($vb):"").($rc?" ENGINE=".q($rc):"").($pb?" COLLATE ".q($pb):"").($Ma!=""?" AUTO_INCREMENT=$Ma":"");if($R=="")return
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$wi,$wh){$ng=array();foreach(array_merge($T,$wi)as$R)$ng[]=table($R)." TO ".idf_escape($wh).".".table($R);return
queries("RENAME TABLE ".implode(", ",$ng));}function
copy_tables($T,$wi,$wh){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($T
as$R){$C=($wh==DB?table("copy_$R"):idf_escape($wh).".".table($R));if(!queries("\nDROP TABLE IF EXISTS $C")||!queries("CREATE TABLE $C LIKE ".table($R))||!queries("INSERT INTO $C SELECT * FROM ".table($R)))return
false;}foreach($wi
as$R){$C=($wh==DB?table("copy_$R"):idf_escape($wh).".".table($R));$vi=view($R);if(!queries("DROP VIEW IF EXISTS $C")||!queries("CREATE VIEW $C AS $vi[select]"))return
false;}return
true;}function
trigger($C){if($C=="")return
...
routine($C,$U){global$g,$tc,$Ad,$Zh;$Ca=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Yh="((".implode("|",array_merge(array_keys($Zh),$Ca)).")\\b(?:\\s*\\(((?:[^'\")]|$tc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$Ef="\\s*(".($U=="FUNCTION"?"":$Ad).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Yh";$i=$g->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$Ef\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Yh\\s+":"")."(.*)~is",$i,$B);$p=array();preg_match_all("~$Ef\\s*,?~is",$B[1],$ne,PREG_SET_ORDER);foreach($ne
as$uf){$C=str_replace("``","`",$uf[2]).$uf[3];$p[]=array("field"=>$C,"type"=>strtolower($uf[5]),"length"=>preg_replace_callback("~$tc~s",'normalize_enum',$uf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$uf[8] $uf[7]"))),"null"=>1,"full_type"=>$uf[4],"inout"=>strtoupper($uf[1]),"collation"=>strtolower($uf[9]),);}if($U!="FUNCTION")return
...
support($Jc){global$g;return!preg_match("~scheme|sequence|type|view_trigger|materializedview".($g->server_info<5.1?"|event|partitioning".($g->server_info<5?"|routine|trigger|view":""):"")."~",$Jc);}function
kill_process($X){return
queries("KILL ".number($X));}function
connection_id(){return"SELECT CONNECTION_ID()";}function
max_connections(){global$g;return$g->result("SELECT @@max_connections");}$x="sql";$Zh=array();$gh=array();foreach(array(lang(24)=>array("tinyint"=>3,"smallint"=>5,"mediumint"=>8,"int"=>10,"bigint"=>20,"decimal"=>66,"float"=>12,"double"=>21),lang(25)=>array("date"=>10,"datetime"=>19,"timestamp"=>19,"time"=>10,"year"=>4),lang(26)=>array("char"=>255,"varchar"=>65535,"tinytext"=>255,"text"=>65535,"mediumtext"=>16777215,"longtext"=>4294967295),lang(30)=>array("enum"=>65535,"set"=>64),lang(27)=>array("bit"=>20,"binary"=>255,"varbinary"=>65535,"tinyblob"=>255,"blob"=>65535,"mediumblob"=>16777215,"longblob"=>4294967295),lang(29)=>array("geometry"=>0,"point"=>0,"linestring"=>0,"polygon"=>0,"multipoint"=>0,"multilinestring"=>0,"multipolygon"=>0,"geometrycollection"=>0),)as$y=>$X){$Zh+=$X;$gh[$y]=array_keys($X);}$gi=array("unsigned","zerofill","unsigned zerofill");$cf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","REGEXP","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$cd=array("char_length","date","from_unixtime","lower","round","sec_to_time","time_to_sec","upper");$hd=array("avg","count","count distinct","group_concat","max","min","sum");$jc=array(array("char"=>"md5/sha1/password/encrypt/uuid","binary"=>"md5/sha1","date|time"=>"now",),array("(^|[^o])int|float|double|decimal"=>"+/-","date"=>"+ interval/- interval","time"=>"addtime/subtime","char|text"=>"concat",));}define("SERVER",$_GET[DRIVER]);define("DB",$_GET["db"]);define("ME",preg_replace('~^[^?]*/([^?]*).*~','\\1',$_SERVER["REQUEST_URI"]).'?'.(sid()?SID.'&':'').(SERVER!==null?DRIVER."=".urlencode(SERVER).'&':'').(isset($_GET["username"])?"username=".urlencode($_GET["username"]).'&':'').(DB!=""?'db='.urlencode(DB).'&'.(isset($_GET["ns"])?"ns=".urlencode($_GET["ns"])."&":""):''));$ia="4.3.0";class
...
head(){return
true;}function
loginForm(){global$bc;echo'<table cellspacing="0">
<tr><th>',lang(31),'<td>',html_select("auth[driver]",$bc,DRIVER),'<tr><th>',lang(32),'<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
login($ie,$G){global$x;if($x=="sqlite")return
lang(38,'<code>login()</code>');return
true;}function
tableName($nh){return
h($nh["Name"]);}function
fieldName($o,$hf=0){return'<span title="'.h($o["full_type"]).'">'.h($o["field"]).'</span>';}function
selectLinks($nh,$O=""){echo'<p class="links">';$he=array("select"=>lang(39));if(support("table")||support("indexes"))$he["table"]=lang(40);if(support("table")){if(is_view($nh))$he["view"]=lang(41);else$he["create"]=lang(42);}if($O!==null)$he["edit"]=lang(43);foreach($he
...
rowDescriptions($L,$Vc){return$L;}function
selectLink($X,$o){}function
selectVal($X,$_,$o,$pf){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$o["type"])&&!preg_match("~var~",$o["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$o["type"])&&!is_utf8($X))$J="<i>".lang(44,strlen($pf))."</i>";if(preg_match('~json~',$o["type"]))$J="<code class='jush-js'>$J</code>";return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
...
as$C=>$v){ksort($v["columns"]);$Qf=array();foreach($v["columns"]as$y=>$X)$Qf[]="<i>".h($X)."</i>".($v["lengths"][$y]?"(".$v["lengths"][$y].")":"").($v["descs"][$y]?" DESC":"");echo"<tr title='".h($C)."'><th>$v[type]<td>".implode(", ",$Qf)."\n";}echo"</table>\n";}function
selectColumnsPrint($M,$e){global$cd,$hd;print_fieldset("select",lang(50),$M);$s=0;$M[""]=array();foreach($M
as$y=>$X){$X=$_GET["columns"][$y];$d=select_input(" name='columns[$s][col]' onchange='".($y!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$e,$X["col"]);echo"<div>".($cd||$hd?"<select name='columns[$s][fun]' onchange='helpClose();".($y!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array(lang(51)=>$cd,lang(52)=>$hd)),$X["fun"])."</select>"."($d)":$d)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($hf,$e,$w){print_fieldset("sort",lang(55),$hf);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'",$e,$X),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),lang(56))."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'",$e),checkbox("desc[$s]",1,false,lang(56))."</div>\n","</div></fieldset>\n";}function
...
as$v){$Ib=reset($v["columns"]);if($v["type"]!="FULLTEXT"&&$Ib)$e[$Ib]=1;}$e[""]=1;foreach($e
as$y=>$X)json_row($y);echo";\n","selectFieldChange(document.getElementById('form'));\n","</script>\n","</div></fieldset>\n";}function
selectCommandPrint(){return!information_schema(DB);}function
selectImportPrint(){return!information_schema(DB);}function
selectEmailPrint($oc,$e){}function
selectColumnsProcess($e,$w){global$cd,$hd;$M=array();$fd=array();foreach((array)$_GET["columns"]as$y=>$X){if($X["fun"]=="count"||($X["col"]!=""&&(!$X["fun"]||in_array($X["fun"],$cd)||in_array($X["fun"],$hd)))){$M[$y]=apply_sql_function($X["fun"],($X["col"]!=""?idf_escape($X["col"]):"*"));if(!in_array($X["fun"],$hd))$fd[]=$M[$y];}}return
array($M,$fd);}function
selectSearchProcess($p,$w){global$g,$x;$J=array();foreach($w
as$s=>$v){if($v["type"]=="FULLTEXT"&&$_GET["fulltext"][$s]!="")$J[]="MATCH (".implode(", ",array_map('idf_escape',$v["columns"])).") AGAINST (".q($_GET["fulltext"][$s]).(isset($_GET["boolean"][$s])?" IN BOOLEAN MODE":"").")";}foreach((array)$_GET["where"]as$X){if("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)){$xb=" $X[op]";if(preg_match('~IN$~',$X["op"])){$ud=process_length($X["val"]);$xb.=" ".($ud!=""?$ud:"(NULL)");}elseif($X["op"]=="SQL")$xb=" $X[val]";elseif($X["op"]=="LIKE %%")$xb=" LIKE ".$this->processInput($p[$X["col"]],"%$X[val]%");elseif($X["op"]=="ILIKE %%")$xb=" ILIKE ".$this->processInput($p[$X["col"]],"%$X[val]%");elseif(!preg_match('~NULL$~',$X["op"]))$xb.=" ".$this->processInput($p[$X["col"]],$X["val"]);if($X["col"]!="")$J[]=idf_escape($X["col"]).$xb;else{$sb=array();foreach($p
as$C=>$o){$Jd=preg_match('~char|text|enum|set~',$o["type"]);if((is_numeric($X["val"])||!preg_match('~(^|[^o])int|float|double|decimal|bit~',$o["type"]))&&(!preg_match("~[\x80-\xFF]~",$X["val"])||$Jd)){$C=idf_escape($C);$sb[]=($x=="sql"&&$Jd&&!preg_match("~^utf8_~",$o["collation"])?"CONVERT($C USING ".charset($g).")":$C);}}$J[]=($sb?"(".implode("$xb OR ",$sb)."$xb)":"0");}}}return$J;}function
...
false;}function
selectQueryBuild($M,$Z,$fd,$hf,$z,$E){return"";}function
messageQuery($H,$Ch){global$x;restart_session();$md=&get_session("queries");$t="sql-".count($md[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$md[$_GET["db"]][]=array($H,time(),$Ch);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$t' onclick=\"return !toggle('$t');\">".lang(61)."</a>"."<div id='$t' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($H,1000).'</code></pre>'.($Ch?" <span class='time'>($Ch)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($md[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';}function
editFunctions($o){global$jc;$J=($o["null"]?"NULL/":"");foreach($jc
as$y=>$cd){if(!$y||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($cd
as$Ef=>$X){if(!$Ef||preg_match("~$Ef~",$o["type"]))$J.="/$X";}if($y&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$J.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J=lang(48);return
explode("/",$J);}function
...
dumpTable($R,$hh,$Kd=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($hh)dump_csv(array_keys(fields($R)));}else{if($Kd==2){$p=array();foreach(fields($R)as$C=>$o)$p[]=idf_escape($C)." $o[full_type]";$i="CREATE TABLE ".table($R)." (".implode(", ",$p).")";}else$i=create_sql($R,$_POST["auto_increment"]);set_utf8mb4($i);if($hh&&$i){if($hh=="DROP+CREATE"||$Kd==1)echo"DROP ".($Kd==2?"VIEW":"TABLE")." IF EXISTS ".table($R).";\n";if($Kd==1)$i=remove_definer($i);echo"$i;\n\n";}}}function
dumpData($R,$hh,$H){global$g,$x;$pe=($x=="sqlite"?0:1048576);if($hh){if($_POST["format"]=="sql"){if($hh=="TRUNCATE+INSERT")echo
truncate_sql($R).";\n";$p=fields($R);}$I=$g->query($H,1);if($I){$Cd="";$Ya="";$Rd=array();$jh="";$Kc=($R!=''?'fetch_assoc':'fetch_row');while($K=$I->$Kc()){if(!$Rd){$ri=array();foreach($K
as$X){$o=$I->fetch_field();if($p[$o->name]['is_virtual']){$zi[]=idf_escape($o->name);continue;}$Rd[]=$o->name;$y=idf_escape($o->name);$ri[]="$y = VALUES($y)";}$jh=($hh=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$ri):"").";\n";}if($_POST["format"]!="sql"){if($hh=="table"){dump_csv($Rd);$hh="INSERT";}dump_csv($K);}else{if(!$Cd)$Cd="INSERT INTO ".table($R)." (".implode(", ",array_map('idf_escape',$Rd)).") VALUES";foreach($K
as$y=>$X){if(in_array("`".$y."`",(array)$zi)){unset($K[$y]);continue;}$o=$p[$y];$K[$y]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$_g=($pe?"\n":" ")."(".implode(",\t",$K).")";if(!$Ya)$Ya=$Cd.$_g;elseif(strlen($Ya)+4+strlen($_g)+strlen($jh)<$pe)$Ya.=",$_g";else{echo$Ya.$jh;$Ya=$Cd.$_g;}}}if($Ya)echo$Ya.$jh;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$g->error)."\n";}}function
dumpFilename($rd){return
friendly_url($rd!=""?$rd:(SERVER!=""?SERVER:"localhost"));}function
dumpHeaders($rd,$Ee=false){$sf=$_POST["output"];$Ec=(preg_match('~sql~',$_POST["format"])?"sql":($Ee?"tar":"csv"));header("Content-Type: ".($sf=="gz"?"application/x-gzip":($Ec=="tar"?"application/x-tar":($Ec=="sql"||$sf!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($sf=="gz")ob_start('ob_gzencode',1e6);return$Ec;}function
...
true;}function
navigation($Ce){global$ia,$x,$bc,$g;echo'<h1>
...
';if($Ce=="auth"){$Pc=true;foreach((array)$_SESSION["pwds"]as$ti=>$Ng){foreach($Ng
as$N=>$oi){foreach($oi
as$V=>$G){if($G!==null){if($Pc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Pc=false;}$Nb=$_SESSION["db"][$ti][$N][$V];foreach(($Nb?array_keys($Nb):array(""))as$m)echo"<a href='".h(auth_url($ti,$N,$V,$m))."'>($bc[$ti]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";}}}}}else{if($_GET["ns"]!==""&&!$Ce&&DB!=""){$g->select_db(DB);$T=table_status('',true);}echo'<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=jush.js&amp;version=4.3.0",'"></script>
';if(support("sql")){echo'<script type="text/javascript">
';if($T){$he=array();foreach($T
as$R=>$U)$he[]=preg_quote($R,'/');echo"var jushLinks = { $x: [ '".js_escape(ME).(support("table")?"table=":"select=")."\$&', /\\b(".implode("|",$he).")\\b/g ] };\n";foreach(array("bac","bra","sqlite_quo","mssql_bra")as$X)echo"jushLinks.$X = jushLinks.$x;\n";}echo'bodyLoad(\'',(is_object($g)?substr($g->server_info,0,3):""),'\');
</script>
';}$this->databasesPrint($Ce);if(DB==""||!$Ce){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".lang(61)."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".lang(69)."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".lang(70)."</a>\n";}if($_GET["ns"]!==""&&!$Ce&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".lang(71)."</a>\n";if(!$T)echo"<p class='message'>".lang(9)."\n";else$this->tablesPrint($T);}}}function
databasesPrint($Ce){global$b,$g;$l=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Lb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".lang(72)."'>DB</span>: ".($l?"<select name='db'$Lb>".optionlist(array(""=>"")+$l,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".lang(20)."'".($l?" class='hidden'":"").">\n";if($Ce!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".lang(73).": <select name='ns'$Lb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
...
page_header($Fh,$n="",$Xa=array(),$Gh=""){global$ca,$ia,$b,$bc,$x;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$Hh=$Fh.($Gh!=""?": $Gh":"");$Ih=strip_tags($Hh.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
<div id="content">
';if($Xa!==null){$_=substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1);echo'<p id="breadcrumb"><a href="'.h($_?$_:".").'">'.$bc[DRIVER].'</a> &raquo; ';$_=substr(preg_replace('~\b(db|ns)=[^&]*&~','',ME),0,-1);$N=(SERVER!=""?h(SERVER):lang(32));if($Xa===false)echo"$N\n";else{echo"<a href='".($_?h($_):".")."' accesskey='1' title='Alt+Shift+1'>$N</a> &raquo; ";if($_GET["ns"]!=""||(DB!=""&&is_array($Xa)))echo'<a href="'.h($_."&db=".urlencode(DB).(support("scheme")?"&ns=":"")).'">'.h(DB).'</a> &raquo; ';if(is_array($Xa)){if($_GET["ns"]!="")echo'<a href="'.h(substr(ME,0,-1)).'">'.h($_GET["ns"]).'</a> &raquo; ';foreach($Xa
as$y=>$X){$Sb=(is_array($X)?$X[1]:h($X));if($Sb!="")echo"<a href='".h(ME."$y=").urlencode(is_array($X)?$X[0]:$X)."'>$Sb</a> &raquo; ";}}echo"$Fh\n";}}echo"<h2>$Hh</h2>\n","<div id='ajaxstatus' class='jsonly hidden'></div>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$ii=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$ze=$_SESSION["messages"][$ii];if($ze){echo"<div class='message'>".implode("</div>\n<div class='message'>",$ze)."</div>\n";unset($_SESSION["messages"][$ii]);}if($n)echo"<div class='error'>$n</div>\n";}function
page_footer($Ce=""){global$b,$Lh;echo'</div>

';switch_lang();if($Ce!="auth"){echo'<form action="" method="post">
<p class="logout">
...
';}echo'<div id="menu">
';$b->navigation($Ce);echo'</div>
<script type="text/javascript">setupSubmitHighlight(document);</script>
';}function
int32($Ge){while($Ge>=2147483648)$Ge-=4294967296;while($Ge<=-2147483649)$Ge+=4294967296;return(int)$Ge;}function
long2str($W,$_i){$_g='';foreach($W
as$X)$_g.=pack('V',$X);if($_i)return
substr($_g,0,end($W));return$_g;}function
str2long($_g,$_i){$W=array_values(unpack('V*',str_pad($_g,4*ceil(strlen($_g)/4),"\0")));if($_i)$W[]=strlen($_g);return$W;}function
xxtea_mx($Fi,$Ei,$kh,$Nd){return
int32((($Fi>>5&0x7FFFFFF)^$Ei<<2)+(($Ei>>3&0x1FFFFFFF)^$Fi<<4))^int32(($kh^$Ei)+($Nd^$Fi));}function
encrypt_string($fh,$y){if($fh=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($fh,true);$Ge=count($W)-1;$Fi=$W[$Ge];$Ei=$W[0];$Xf=floor(6+52/($Ge+1));$kh=0;while($Xf-->0){$kh=int32($kh+0x9E3779B9);$ic=$kh>>2&3;for($tf=0;$tf<$Ge;$tf++){$Ei=$W[$tf+1];$Fe=xxtea_mx($Fi,$Ei,$kh,$y[$tf&3^$ic]);$Fi=int32($W[$tf]+$Fe);$W[$tf]=$Fi;}$Ei=$W[0];$Fe=xxtea_mx($Fi,$Ei,$kh,$y[$tf&3^$ic]);$Fi=int32($W[$Ge]+$Fe);$W[$Ge]=$Fi;}return
long2str($W,false);}function
decrypt_string($fh,$y){if($fh=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($fh,false);$Ge=count($W)-1;$Fi=$W[$Ge];$Ei=$W[0];$Xf=floor(6+52/($Ge+1));$kh=int32($Xf*0x9E3779B9);while($kh){$ic=$kh>>2&3;for($tf=$Ge;$tf>0;$tf--){$Fi=$W[$tf-1];$Fe=xxtea_mx($Fi,$Ei,$kh,$y[$tf&3^$ic]);$Ei=int32($W[$tf]-$Fe);$W[$tf]=$Ei;}$Fi=$W[$Ge];$Fe=xxtea_mx($Fi,$Ei,$kh,$y[$tf&3^$ic]);$Ei=int32($W[0]-$Fe);$W[0]=$Ei;$kh=int32($kh-0x9E3779B9);}return
long2str($W,true);}$g='';$ld=$_SESSION["token"];if(!$ld)$_SESSION["token"]=rand(1,1e6);$Lh=get_token();$Ff=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$Ff[$y]=$X;}}function
add_invalid_login(){global$b;$Nc=get_temp_dir()."/adminer.invalid";$ad=@fopen($Nc,"r+");if(!$ad){$ad=@fopen($Nc,"w");if(!$ad)return;}flock($ad,LOCK_EX);$Fd=unserialize(stream_get_contents($ad));$Ch=time();if($Fd){foreach($Fd
as$Gd=>$X){if($X[0]<$Ch)unset($Fd[$Gd]);}}$Ed=&$Fd[$b->bruteForceKey()];if(!$Ed)$Ed=array($Ch+30*60,0);$Ed[1]++;$Lg=serialize($Fd);rewind($ad);fwrite($ad,$Lg);ftruncate($ad,strlen($Lg));flock($ad,LOCK_UN);fclose($ad);}$La=$_POST["auth"];if($La){$Fd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Ed=$Fd[$b->bruteForceKey()];$Me=($Ed[1]>30?$Ed[0]-time():0);if($Me>0)auth_error(lang(78,ceil($Me/60)));session_regenerate_id();$ti=$La["driver"];$N=$La["server"];$V=$La["username"];$G=(string)$La["password"];$m=$La["db"];set_password($ti,$N,$V,$G);$_SESSION["db"][$ti][$N][$V][$m]=true;if($La["permanent"]){$y=base64_encode($ti)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$Rf=$b->permanentLogin(true);$Ff[$y]="$y:".base64_encode($Rf?encrypt_string($G,$Rf):"");cookie("adminer_permanent",implode(" ",$Ff));}if(count($_POST)==1||DRIVER!=$ti||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($ti,$N,$V,$m));}elseif($_POST["logout"]){if($ld&&!verify_token()){page_header(lang(77),lang(79));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(80));}}elseif($Ff&&!$_SESSION["pwds"]){session_regenerate_id();$Rf=$b->permanentLogin();foreach($Ff
as$y=>$X){list(,$jb)=explode(":",$X);list($ti,$N,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($ti,$N,$V,decrypt_string(base64_decode($jb),$Rf));$_SESSION["db"][$ti][$N][$V][$m]=true;}}function
unset_permanent(){global$Ff;foreach($Ff
as$y=>$X){list($ti,$N,$V,$m)=array_map('base64_decode',explode("-",$y));if($ti==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($Ff[$y]);}cookie("adminer_permanent",implode(" ",$Ff));}function
auth_error($n){global$b,$ld;$Og=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Og]||$_GET[$Og])&&!$ld)$n=lang(81);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(82,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Og]&&$_GET[$Og]&&ini_bool("session.use_only_cookies"))$n=lang(83);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(84),lang(85,implode(", ",$Lf)),false);page_footer("auth");exit;}$g=connect();}$ac=new
Min_Driver($g);if(!is_object($g)||($ie=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?h($g):(is_string($ie)?$ie:lang(86))));if($La&&$_POST["token"])$_POST["token"]=$Lh;$n='';if($_POST){if(!verify_token()){$_d="max_input_vars";$te=ini_get($_d);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$te||$X<$te)){$_d=$y;$te=$X;}}}$n=(!$_POST["token"]&&$te?lang(87,"'$_d'"):lang(79).' '.lang(88));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(89,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(90);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
select($I,$h=null,$kf=array(),$z=0){global$x;$he=array();$w=array();$e=array();$Ua=array();$Zh=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Md=0;$Md<count($K);$Md++){$o=$I->fetch_field();$C=$o->name;$jf=$o->orgtable;$if=$o->orgname;$J[$o->table]=$jf;if($kf&&$x=="sql")$he[$Md]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($jf!=""){if(!isset($w[$jf])){$w[$jf]=array();foreach(indexes($jf,$h)as$v){if($v["type"]=="PRIMARY"){$w[$jf]=array_flip($v["columns"]);break;}}$e[$jf]=$w[$jf];}if(isset($e[$jf][$if])){unset($e[$jf][$if]);$w[$jf][$if]=$Md;$he[$Md]=$jf;}}if($o->charsetnr==63)$Ua[$Md]=true;$Zh[$Md]=$o->type;echo"<th".($jf!=""||$o->name!=$if?" title='".h(($jf!=""?"$jf.":"").$if)."'":"").">".h($C).($kf?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$y=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ua[$y]&&!is_utf8($X))$X="<i>".lang(44,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Zh[$y]==254)$X="<code>$X</code>";}if(isset($he[$y])&&!$e[$he[$y]]){if($kf&&$x=="sql"){$R=$K[array_search("table=",$he)];$_=$he[$y].urlencode($kf[$R]!=""?$kf[$R]:$R);}else{$_="edit=".urlencode($he[$y]);foreach($w[$he[$y]]as$nb=>$Md)$_.="&where".urlencode("[".bracket_escape($nb)."]")."=".urlencode($K[$Md]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
referencable_primary($Hg){$J=array();foreach(table_status('',true)as$oh=>$R){if($oh!=$Hg&&fk_support($R)){foreach(fields($oh)as$o){if($o["primary"]){if($J[$oh]){unset($J[$oh]);break;}$J[$oh]=$o;}}}}return$J;}function
...
edit_type($y,$o,$qb,$Wc=array(),$xi){global$gh,$Zh,$gi,$Xe;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),' aria-labelledby="label-type">';if($U&&!isset($Zh[$U])&&!isset($Wc[$U]))array_unshift($gh,$U);if($Wc)$gh[lang(91)]=$Wc;echo
optionlist($gh,$U),'</select>
<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';if($o["is_virtual"])textarea(h($y)."[virtual1]",$xi,$L=2,$sb=40);else{echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(92).')'.optionlist($qb,$o["collation"]).'</select>',($gi?"<select name='".h($y)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($gi,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(93).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Wc?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(94).")".optionlist(explode("|",$Xe),$o["on_delete"])."</select> ":" ");}}function
process_length($ee){global$tc;return(preg_match("~^\\s*\\(?\\s*$tc(?:\\s*,\\s*$tc)*+\\s*\\)?\\s*\$~",$ee)&&preg_match_all("~$tc~",$ee,$ne)?"(".implode(",",$ne[0]).")":preg_replace('~^[0-9].*~','(\0)',preg_replace('~[^-0-9,+()[\]]~','',$ee)));}function
process_type($o,$ob="COLLATE"){global$gi;return" $o[type]".process_length($o["length"]).(preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&in_array($o["unsigned"],$gi)?" $o[unsigned]":"").(preg_match('~char|text|enum|set~',$o["type"])&&$o["collation"]?" $ob ".q($o["collation"]):"");}function
process_field($o,$Xh){global$x;$Pb=$o["default"];return
array(idf_escape(trim($o["field"])),process_type($Xh),($o["null"]?" NULL":" NOT NULL"),(isset($Pb)?" DEFAULT ".((preg_match('~time~',$o["type"])&&preg_match('~^CURRENT_TIMESTAMP$~i',$Pb))||($x=="sqlite"&&preg_match('~^CURRENT_(TIME|TIMESTAMP|DATE)$~i',$Pb))||($o["type"]=="bit"&&preg_match("~^([0-9]+|b'[0-1]+')\$~",$Pb))||($x=="pgsql"&&preg_match("~^[a-z]+\\(('[^']*')+\\)\$~",$Pb))?$Pb:q($Pb)):""),(preg_match('~timestamp|datetime~',$o["type"])&&$o["on_update"]?" ON UPDATE $o[on_update]":""),(support("comment")&&$o["comment"]!=""?" COMMENT ".q($o["comment"]):""),($o["auto_increment"]?auto_increment():null),);}function
type_class($U){foreach(array('char'=>'text','date'=>'time|year','binary'=>'blob','enum'=>'set',)as$y=>$X){if(preg_match("~$y|$X~",$U))return" class='$y'";}}function
edit_fields($p,$qb,$U="TABLE",$Wc=array(),$wb=false,$Pg){global$g,$Ad;$p=array_values($p);echo'<thead><tr class="wrap">
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="',lang(48),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default">',lang(49),(support("comment")?"<td id='label-comment'".($wb?"":" class='hidden'").">".lang(47):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".lang(99)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$s=>$o){$s++;$lf=$o[($_POST?"orig":"field")];$Wb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$lf=="");echo'<tr',($Wb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$Ad),$o["inout"]):""),'<th>';if($Wb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($lf),'">
';$Yf=substr($Pg[substr($s,-2,1)-1],stripos($Pg[substr($s,-2,1)-1],"as")+3);$Xf=substr($Yf,0,strpos($Yf,"VIRTUAL"));edit_type("fields[$s]",$o,$qb,$Wc,$Xf);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }" aria-labelledby="label-ai"></label><td><?php
echo
...
',(support("comment")?"<td".($wb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".lang(99)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.3.0' alt='^' title='".lang(100)."' onclick='return !editingMoveRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.3.0' alt='v' title='".lang(101)."' onclick='return !editingMoveRow(this, 0);'>&nbsp;":""),($lf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".lang(102)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
process_fields(&$p){$D=0;if($_POST["up"]){$Yd=0;foreach($p
as$y=>$o){if(key($_POST["up"])==$y){unset($p[$y]);array_splice($p,$Yd,0,array($o));break;}if(isset($o["field"]))$Yd=$D;$D++;}}elseif($_POST["down"]){$Yc=false;foreach($p
as$y=>$o){if(isset($o["field"])&&$Yc){unset($p[key($_POST["down"])]);array_splice($p,$D,0,array($Yc));break;}if(key($_POST["down"])==$y)$Yc=$o;$D++;}}elseif($_POST["add"]){$p=array_values($p);array_splice($p,key($_POST["add"]),0,array(array()));}elseif(!$_POST["drop_col"])return
false;return
true;}function
normalize_enum($B){return"'".str_replace("'","''",addcslashes(stripcslashes(str_replace($B[0][0].$B[0][0],$B[0][0],substr($B[0],1,-1))),'\\'))."'";}function
grant($dd,$Tf,$e,$We){if(!$Tf)return
true;if($Tf==array("ALL PRIVILEGES","GRANT OPTION"))return($dd=="GRANT"?queries("$dd ALL PRIVILEGES$We WITH GRANT OPTION"):queries("$dd ALL PRIVILEGES$We")&&queries("$dd GRANT OPTION$We"));return
queries("$dd ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~','\\1',implode("$e, ",$Tf).$e).$We);}function
drop_create($cc,$i,$dc,$_h,$fc,$A,$ye,$we,$xe,$Te,$Je){if($_POST["drop"])query_redirect($cc,$A,$ye);elseif($Te=="")query_redirect($i,$A,$xe);elseif($Te!=$Je){$Gb=queries($i);queries_redirect($A,$we,$Gb&&queries($cc));if($Gb)queries($dc);}else
queries_redirect($A,$we,queries($_h)&&queries($fc)&&queries($cc)&&queries($i));}function
create_trigger($We,$K){global$x;$Eh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($x=="mssql"?$We.$Eh:$Eh.$We).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
create_routine($wg,$K){global$Ad;$O=array();$p=(array)$K["fields"];ksort($p);foreach($p
as$o){if($o["field"]!="")$O[]=(preg_match("~^($Ad)\$~",$o["inout"])?"$o[inout] ":"").idf_escape($o["field"]).process_type($o,"CHARACTER SET");}return"CREATE $wg ".idf_escape(trim($K["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($K["returns"],"CHARACTER SET"):"").($K["language"]?" LANGUAGE $K[language]":"").rtrim("\n$K[definition]",";").";";}function
remove_definer($H){return
preg_replace('~^([A-Z =]+) DEFINER=`'.preg_replace('~@(.*)~','`@`(%|\\1)',logged_user()).'`~','\\1',$H);}function
format_foreign_key($q){global$Xe;return" FOREIGN KEY (".implode(", ",array_map('idf_escape',$q["source"])).") REFERENCES ".table($q["table"])." (".implode(", ",array_map('idf_escape',$q["target"])).")".(preg_match("~^($Xe)\$~",$q["on_delete"])?" ON DELETE $q[on_delete]":"").(preg_match("~^($Xe)\$~",$q["on_update"])?" ON UPDATE $q[on_update]":"");}function
tar_file($Nc,$Jh){$J=pack("a100a8a8a8a12a12",$Nc,644,0,0,decoct($Jh->size),decoct(time()));$hb=8*32;for($s=0;$s<strlen($J);$s++)$hb+=ord($J[$s]);$J.=sprintf("%06o",$hb)."\0 ";echo$J,str_repeat("\0",512-strlen($J));$Jh->send();echo
str_repeat("\0",511-($Jh->size+511)%512);}function
ini_bytes($_d){$X=ini_get($_d);switch(strtolower(substr($X,-1))){case'g':$X*=1024;case'm':$X*=1024;case'k':$X*=1024;}return$X;}function
doc_link($Df){global$x,$g;$ki=array('sql'=>"http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/",'sqlite'=>"http://www.sqlite.org/",'pgsql'=>"http://www.postgresql.org/docs/".substr($g->server_info,0,3)."/static/",'mssql'=>"http://msdn.microsoft.com/library/",'oracle'=>"http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/",);return($Df[$x]?"<a href='$ki[$x]$Df[$x]' target='_blank' rel='noreferrer'><sup>?</sup></a>":"");}function
...
connect_error(){global$b,$g,$Lh,$n,$bc;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header(lang(35).": ".h(DB),lang(103),true);}else{if($_POST["db"]&&!$n)queries_redirect(substr(ME,0,-1),lang(104),drop_databases($_POST["db"]));page_header(lang(105),$n,false);echo"<p class='links'>\n";foreach(array('database'=>lang(106),'privileges'=>lang(68),'processlist'=>lang(107),'variables'=>lang(108),'status'=>lang(109),'replication'=>lang(110),)as$y=>$X){if(support($y))echo"<a href='".h(ME)."$y='>$X</a>\n";}echo"<p>".lang(111,$bc[DRIVER],"<b>".h($g->server_info)."</b>","<b>$g->extension</b>")."\n","<p>".lang(112,"<b>".h(logged_user())."</b>")."\n";$l=$b->databases();if($l){$Cg=support("scheme");$qb=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".lang(35)." - <a href='".h(ME)."refresh=1'>".lang(113)."</a>"."<td>".lang(114)."<td>".lang(115)."<td>".lang(116)." - <a href='".h(ME)."dbsize=1' onclick=\"return !ajaxSetHtml('".h(js_escape(ME))."script=connect');\">".lang(117)."</a>"."</thead>\n";$l=($_GET["dbsize"]?count_tables($l):array_flip($l));foreach($l
as$m=>$T){$vg=h(ME)."db=".urlencode($m);$t=h("Db-".$m);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$m,in_array($m,(array)$_POST["db"]),"","","",$t):""),"<th><a href='$vg' id='$m'>".h($m)."</a>";$pb=nbsp(db_collation($m,$qb));echo"<td>".(support("database")?"<a href='$vg".($Cg?"&amp;ns=":"")."&amp;database=' title='".lang(64)."'>$pb</a>":$pb),"<td align='right'><a href='$vg&amp;schema=' id='tables-".h($m)."' title='".lang(67)."'>".($_GET["dbsize"]?$T:"?")."</a>","<td align='right' id='size-".h($m)."'>".($_GET["dbsize"]?db_size($m):"?"),"\n";}echo"</table>\n",(support("database")?"<fieldset><legend>".lang(118)." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^db/));\">\n"."<input type='submit' name='drop' value='".lang(119)."'".confirm().">\n"."</div></fieldset>\n":""),"<script type='text/javascript'>tableCheck();</script>\n","<input type='hidden' name='token' value='$Lh'>\n","</form>\n";}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["replication"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header(lang(73).": ".h($_GET["ns"]),lang(120),true);page_footer("ns");exit;}}$Xe="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
TmpFile{var$handler;var$size;function
__construct(){$this->handler=tmpfile();}function
write($Ab){$this->size+=strlen($Ab);fwrite($this->handler,$Ab);}function
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$tc="'(?:''|[^'\\\\]|\\\\.)*'";$Ad="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$p=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$M=array(idf_escape($_GET["field"]));$I=$ac->select($a,$M,array(where($_GET,$p)),$M);$K=($I?$I->fetch_row():array());echo$K[0];exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$p=fields($a);if(!$p)$n=error();$S=table_status1($a,true);page_header(($p&&is_view($S)?$S['Engine']=='materialized view'?lang(121):lang(122):lang(123)).": ".h($a),$n);$b->selectLinks($S);$vb=$S["Comment"];if($vb!="")echo"<p>".lang(47).": ".h($vb)."\n";if($p)$b->tableStructurePrint($p);if(!is_view($S)){if(support("indexes")){echo"<h3 id='indexes'>".lang(124)."</h3>\n";$w=indexes($a);if($w)$b->tableIndexesPrint($w);echo'<p class="links"><a href="'.h(ME).'indexes='.urlencode($a).'">'.lang(125)."</a>\n";}if(fk_support($S)){echo"<h3 id='foreign-keys'>".lang(91)."</h3>\n";$Wc=foreign_keys($a);if($Wc){echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(126)."<td>".lang(127)."<td>".lang(94)."<td>".lang(93)."<td>&nbsp;</thead>\n";foreach($Wc
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".lang(128)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(131)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(67),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$qh=array();$rh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$ne,PREG_SET_ORDER);foreach($ne
as$s=>$B){$qh[$B[1]]=array($B[2],$B[3]);$rh[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$Mh=0;$Ra=-1;$Bg=array();$jg=array();$ce=array();foreach(table_status('',true)as$R=>$S){if(is_view($S))continue;$If=0;$Bg[$R]["fields"]=array();foreach(fields($R)as$C=>$o){$If+=1.25;$o["pos"]=$If;$Bg[$R]["fields"][$C]=$o;}$Bg[$R]["pos"]=($qh[$R]?$qh[$R]:array($Mh,0));foreach($b->foreignKeys($R)as$X){if(!$X["db"]){$ae=$Ra;if($qh[$R][1]||$qh[$X["table"]][1])$ae=min(floatval($qh[$R][1]),floatval($qh[$X["table"]][1]))-1;else$Ra-=.1;while($ce[(string)$ae])$ae-=.0001;$Bg[$R]["references"][$X["table"]][(string)$ae]=array($X["source"],$X["target"]);$jg[$X["table"]][$R][(string)$ae]=$X["target"];$ce[(string)$ae]=true;}}$Mh=max($Mh,$Bg[$R]["pos"][0]+2.5+$If);}echo'<div id="schema" style="height: ',$Mh,'em;" onselectstart="return false;">
<script type="text/javascript">
...
';foreach($Bg
as$C=>$R){echo"<div class='table' style='top: ".$R["pos"][0]."em; left: ".$R["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($C).'"><b>'.h($C)."</b></a>";foreach($R["fields"]as$o){$X='<span'.type_class($o["type"]).' title="'.h($o["full_type"].($o["null"]?" NULL":'')).'">'.h($o["field"]).'</span>';echo"<br>".($o["primary"]?"<i>$X</i>":$X);}foreach((array)$R["references"]as$xh=>$kg){foreach($kg
as$ae=>$gg){$be=$ae-$qh[$C][1];$s=0;foreach($gg[0]as$Wg)echo"\n<div class='references' title='".h($xh)."' id='refs$ae-".($s++)."' style='left: $be"."em; top: ".$R["fields"][$Wg]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$be)."em;'></div></div>";}}foreach((array)$jg[$C]as$xh=>$kg){foreach($kg
as$ae=>$e){$be=$ae-$qh[$C][1];$s=0;foreach($e
as$wh)echo"\n<div class='references' title='".h($xh)."' id='refd$ae-".($s++)."' style='left: $be"."em; top: ".$R["fields"][$wh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$be)."em;'></div></div>";}}echo"\n</div>\n";}foreach($Bg
as$C=>$R){foreach((array)$R["references"]as$xh=>$kg){foreach($kg
as$ae=>$gg){$Be=$Mh;$re=-10;foreach($gg[0]as$y=>$Wg){$Jf=$R["pos"][0]+$R["fields"][$Wg]["pos"];$Kf=$Bg[$xh]["pos"][0]+$Bg[$xh]["fields"][$gg[1][$y]]["pos"];$Be=min($Be,$Jf,$Kf);$re=max($re,$Jf,$Kf);}echo"<div class='references' id='refl$ae' style='left: $ae"."em; top: $Be"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($re-$Be)."em;'></div></div>\n";}}}echo'</div>
<p class="links"><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">',lang(132),'</a>
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$Db="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$y)$Db.="&$y=".urlencode($_POST[$y]);cookie("adminer_export",substr($Db,1));$T=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Ec=dump_headers((count($T)==1?key($T):DB),(DB==""||count($T)>1));$Id=preg_match('~sql~',$_POST["format"]);if($Id){echo"-- Adminer $ia ".$bc[DRIVER]." dump\n\n";if($x=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
as$m){$b->dumpDatabase($m);if($g->select_db($m)){if($Id&&preg_match('~CREATE~',$hh)&&($i=$g->result("SHOW CREATE DATABASE ".idf_escape($m),1))){set_utf8mb4($i);if($hh=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($m).";\n";echo"$i;\n";}if($Id){if($hh)echo
use_sql($m).";\n\n";$rf="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$wg){foreach(get_rows("SHOW $wg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE $wg ".idf_escape($K["Name"]),2));set_utf8mb4($i);$rf.=($hh!='DROP+CREATE'?"DROP $wg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($i);$rf.=($hh!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}if($rf)echo"DELIMITER ;;\n\n$rf"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$wi=array();foreach(table_status('',true)as$C=>$S){$R=(DB==""||in_array($C,(array)$_POST["tables"]));$Jb=(DB==""||in_array($C,(array)$_POST["data"]));if($R||$Jb){if($Ec=="tar"){$Jh=new
TmpFile;ob_start(array($Jh,'write'),1e5);}$b->dumpTable($C,($R?$_POST["table_style"]:""),(is_view($S)?2:0));if(is_view($S))$wi[]=$C;elseif($Jb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Id&&$_POST["triggers"]&&$R&&($Wh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Wh\nDELIMITER ;\n";if($Ec=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$Jh);}elseif($Id)echo"\n";}}foreach($wi
as$vi)$b->dumpTable($vi,$_POST["table_style"],1);if($Ec=="tar")echo
...
<table cellspacing="0">
';$Mb=array('','USE','DROP+CREATE','CREATE');$sh=array('','DROP+CREATE','CREATE');$Kb=array('','TRUNCATE+INSERT','INSERT');if($x=="sql")$Kb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".lang(133)."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".lang(134)."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($x=="sqlite"?"":"<tr><th>".lang(35)."<td>".html_select('db_style',$Mb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],lang(135)):"").(support("event")?checkbox("events",1,$K["events"],lang(136)):"")),"<tr><th>".lang(115)."<td>".html_select('table_style',$sh,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],lang(48)).(support("trigger")?checkbox("triggers",1,$K["triggers"],lang(130)):""),"<tr><th>".lang(137)."<td>".html_select('data_style',$Kb,$K["data_style"]),'</table>
<p><input type="submit" value="',lang(70),'">
<input type="hidden" name="token" value="',$Lh,'">

<table cellspacing="0">
';$Nf=array();if(DB!=""){$fb=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$fb onclick='formCheck(this, /^tables\\[/);'>".lang(115)."</label>","<th style='text-align: right;'><label class='block'>".lang(137)."<input type='checkbox' id='check-data'$fb onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$wi="";$th=tables_list();foreach($th
as$C=>$U){$Mf=preg_replace('~_.*~','',$C);$fb=($a==""||$a==(substr($a,-1)=="%"?"$Mf%":$C));$Qf="<tr><td>".checkbox("tables[]",$C,$fb,$C,"checkboxClick(event, this); formUncheck('check-tables');","block");if($U!==null&&!preg_match('~table~i',$U))$wi.="$Qf\n";else
echo"$Qf<td align='right'><label class='block'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$fb,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Nf[$Mf]++;}echo$wi;if($th)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$Mf=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$Mf%",$m,"formUncheck('check-databases');","block")."\n";$Nf[$Mf]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Pc=true;foreach($Nf
as$y=>$X){if($y!=""&&$X>1){echo($Pc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Pc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(68));echo'<p class="links"><a href="'.h(ME).'user=">'.lang(138)."</a>";$I=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$dd=$I;if(!$I)$I=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($dd?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$dd||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$nd=&get_session("queries");$md=&$nd[DB];if(!$n&&$_POST["clear"]){$md=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(69):lang(61)),$n);if(!$n&&$_POST){$ad=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$ad=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($ad?fread($ad,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Xf=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$md||reset(end($md))!=$Xf){restart_session();$md[]=array($Xf,time());set_session("queries",$nd);stop_session();}}$Xg="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Rb=";";$D=0;$qc=true;$h=connect();if(is_object($h)&&DB!="")$h->select_db(DB);$ub=0;$vc=array();$wf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\\*|-- |$'.($x=="pgsql"?'|\\$[^$]*\\$':'');$Nh=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$hc=$b->dumpFormat();unset($hc["sql"]);while($H!=""){if(!$D&&preg_match("~^$Xg*+DELIMITER\\s+(\\S+)~i",$H,$B)){$Rb=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Rb)."\\s*|$wf)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Yc,$If)=$B[0];if(!$Yc&&$ad&&!feof($ad))$H.=fread($ad,1e5);else{if(!$Yc&&rtrim($H)=="")break;$D=$If+strlen($Yc);if($Yc&&rtrim($Yc)!=$Rb){while(preg_match('('.($Yc=='/*'?'\\*/':($Yc=='['?']':(preg_match('~^-- |^#~',$Yc)?"\n":preg_quote($Yc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$_g=$B[0][0];if(!$_g&&$ad&&!feof($ad))$H.=fread($ad,1e5);else{$D=$B[0][1]+strlen($_g);if($_g[0]!="\\")break;}}}else{$qc=false;$Xf=substr($H,0,$If);$ub++;$Qf="<pre id='sql-$ub'><code class='jush-$x'>".$b->sqlCommandQuery($Xf)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$Xg*+ATTACH\\b~i",$Xf,$B)){echo$Qf,"<p class='error'>".lang(139)."\n";$vc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$Qf;ob_flush();flush();}$dh=microtime(true);if($g->multi_query($Xf)&&is_object($h)&&preg_match("~^$Xg*+USE\\b~i",$Xf))$h->query($Xf);do{$I=$g->store_result();$Ch=" <span class='time'>(".format_time($dh).")</span>".(strlen($Xf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Xf))."'>".lang(10)."</a>":"");if($g->error){echo($_POST["only_errors"]?$Qf:""),"<p class='error'>".lang(140).($g->errno?" ($g->errno)":"").": ".error()."\n";$vc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$kf=select($I,$h,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Ne=$I->num_rows;echo"<p>".($Ne?($z&&$Ne>$z?lang(141,$z):"").lang(142,$Ne):""),$Ch;$t="export-$ub";$Dc=", <a href='#$t' onclick=\"return !toggle('$t');\">".lang(70)."</a><span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$hc,$xa["format"])."<input type='hidden' name='query' value='".h($Xf)."'>"." <input type='submit' name='export' value='".lang(70)."'><input type='hidden' name='token' value='$Lh'></span>\n";if($h&&preg_match("~^($Xg|\\()*+SELECT\\b~i",$Xf)&&($Cc=explain($h,$Xf))){$t="explain-$ub";echo", <a href='#$t' onclick=\"return !toggle('$t');\">EXPLAIN</a>$Dc","<div id='$t' class='hidden'>\n";select($Cc,$h,$kf);echo"</div>\n";}else
echo$Dc;echo"</form>\n";}}else{if(preg_match("~^$Xg*+(CREATE|DROP|ALTER)$Xg++(DATABASE|SCHEMA)\\b~i",$Xf)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($g->info)."'>".lang(143,$g->affected_rows)."$Ch\n";}$dh=microtime(true);}while($g->next_result());}$H=substr($H,$D);$D=0;}}}}if($qc)echo"<p class='message'>".lang(144)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(145,$ub-count($vc))," <span class='time'>(".format_time($Nh).")</span>\n";}elseif($vc&&$ub>1)echo"<p class='error'>".lang(140).": ".implode("",$vc)."\n";}else
echo"<p class='error'>".upload_error($H)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$_c="<input type='submit' value='".lang(146)."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$Xf=$_GET["sql"];if($_POST)$Xf=$_POST["query"];elseif($_GET["history"]=="all")$Xf=$md;elseif($_GET["history"]!="")$Xf=$md[$_GET["history"]][0];echo"<p>";textarea("query",$Xf,20);echo($_POST?"":"<script type='text/javascript'>focus(document.getElementsByTagName('textarea')[0]);</script>\n"),"<p>$_c\n",lang(147).": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".lang(148)."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$_c":lang(149)),"</div></fieldset>\n","<fieldset><legend>".lang(150)."</legend><div>",lang(151,"<code>adminer.sql".(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.lang(152).'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(153))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(154))."\n","<input type='hidden' name='token' value='$Lh'>\n";if(!isset($_GET["import"])&&$md){print_fieldset("history",lang(155),$_GET["history"]!="");for($X=end($md);$X;$X=prev($md)){$y=key($md);list($Xf,$Ch,$lc)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$Ch)."'>".@date("H:i:s",$Ch)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Xf)))),80,"</code>").($lc?" <span class='time'>($lc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(156)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(157)."</a>\n","</div></fieldset>\n";}echo'</form>
';}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$hi=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(!isset($o["privileges"][$hi?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($hi?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$ci=unique_array($_GET["where"],$w);$bg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(158),$ac->delete($a,$bg,!$ci));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($hi){if(!$O)redirect($A);queries_redirect($A,lang(159),$ac->update($a,$O,$bg,!$ci));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$ac->insert($a,$O);$Zd=($I?last_id():0);queries_redirect($A,lang(160,($Zd?" $Zd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
as$C=>$o){if($o["is_virtual"]){unset($p[$C]);continue;}if(isset($o["privileges"]["select"])){$Ha=convert_field($o);if($_POST["clone"]&&$o["auto_increment"])$Ha="''";if($x=="sql"&&preg_match("~enum|set~",$o["type"]))$Ha="1*".idf_escape($C);$M[]=($Ha?"$Ha AS ":"").idf_escape($C);}}$K=array();if(!support("table"))$M=array("*");if($M){$I=$ac->select($a,$M,array($Z),$M,array(),(isset($_GET["select"])?2:1));$K=$I->fetch_assoc();if(!$K)$K=false;if(isset($_GET["select"])&&(!$K||$I->fetch_assoc()))$K=null;}}if(!support("table")&&!$p){if(!$Z){$I=$ac->select($a,array("*"),$Z,array("*"));$K=($I?$I->fetch_assoc():false);if(!$K)$K=array($ac->primary=>"");}if($K){foreach($K
as$y=>$X){if(!$Z)$K[$y]=null;$p[$y]=array("field"=>$y,"null"=>($y!=$ac->primary),"auto_increment"=>($y==$ac->primary));}}}edit_form($a,$p,$K,$hi);}elseif(isset($_GET["create"])){$a=$_GET["create"];$yf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$y)$yf[$y]=$y;$ig=referencable_primary($a);$Wc=array();foreach($ig
as$oh=>$o)$Wc[str_replace("`","``",$oh)."`".str_replace("`","``",$o["field"])]=$oh;$nf=array();$S=array();if($a!=""){$nf=fields($a);$S=table_status($a);if(!$S)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(161),drop_tables(array($a)));else{$p=array();$Da=array();$li=false;$Uc=array();$mf=reset($nf);$Aa=" FIRST";$De=array();foreach($K["fields"]as$y=>$o){$q=$Wc[$o["type"]];$Xh=($q!==null?$ig[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($y==$K["auto_increment_col"])$o["auto_increment"]=true;$Vf=process_field($o,$Xh);$Da[]=array($o["orig"],$Vf,$Aa);if($Vf!=process_field($mf,$mf)){$p[]=array($o["orig"],$Vf,$Aa);if($o["orig"]!=""||$Aa)$li=true;}if($q!==null)$Uc[idf_escape($o["field"])]=($a!=""&&$x!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Wc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Xh["field"]),'on_delete'=>$o["on_delete"],));$Aa=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$li=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$mf=next($nf);if(!$mf)$Aa="";}if($o["virtual1"]){if(!$o["length"])$De[]="`".$o["field"]."` ".$o["type"]." as ".$o["virtual1"];else$De[]="`".$o["field"]."` ".$o["type"]."(".$o["length"].") as ".$o["virtual1"];}}$_f="";if($yf[$K["partition_by"]]){$Af=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$y=>$X){$Y=$K["partition_values"][$y];$Af[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$_f.="\nPARTITION BY $K[partition_by]($K[partition])".($Af?" (".implode(",",$Af)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$S["Create_options"]))$_f.="\nREMOVE PARTITIONING";$ve=lang(162);if($a==""){cookie("adminer_engine",$K["Engine"]);$ve=lang(163);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$ve,alter_table($a,$C,($x=="sqlite"&&($li||$Uc)?$Da:$p),$Uc,($K["Comment"]!=$S["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$S["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$S["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$_f,$De));}}page_header(($a!=""?lang(42):lang(71)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Zh["int"])?"int":(isset($Zh["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$S;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($nf
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$bd="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $bd ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$Af=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $bd AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$Af[""]="";$K["partition_names"]=array_keys($Af);$K["partition_values"]=array_values($Af);}}}$qb=collations();$sc=engines();foreach($sc
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($sc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".lang(165).")")+$sc,$K["Engine"])."</select>":""),' ',($qb&&!preg_match("~sqlite|mssql~",$x)?html_select("Collation",array(""=>"(".lang(92).")")+$qb,$K["Collation"]):""),' <input type="submit" value="',lang(14),'">
';}echo'
';if(support("columns")){echo'<table cellspacing="0" id="edit-fields" class="nowrap">
';$wb=($_POST?$_POST["comments"]:$K["Comment"]!="");if(!$_POST&&!$wb){foreach($K["fields"]as$o){if($o["comment"]!=""){$wb=true;break;}}}$Pg=explode(",",create_sql($K["name"],""));edit_fields($K["fields"],$qb,"TABLE",$Wc,$wb,$Pg);echo'</table>
<p>
',lang(48),': <input type="number" name="Auto_increment" size="6" value="',h($K["Auto_increment"]),'">
',checkbox("defaults",1,true,lang(166),"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($wb?" checked":"").">".lang(47)."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($wb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="',lang(14),'">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}if(support("partitioning")){$zf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(167),$K["partition_by"]);echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')",1).">".optionlist(array(""=>"")+$yf,$K["partition_by"])."</select>",'(<input name="partition" value="',h($K["partition"]),'">)
',lang(168),': <input type="number" name="partitions" class="size',($zf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
';}elseif(isset($_GET["indexes"])){$a=$_GET["indexes"];$wd=array("PRIMARY","UNIQUE","INDEX");$S=table_status($a,true);if(preg_match('~MyISAM|M?aria'.($g->server_info>=5.6?'|InnoDB':'').'~i',$S["Engine"]))$wd[]="FULLTEXT";if(preg_match('~MyISAM|M?aria'.($g->server_info>=5.7?'|InnoDB':'').'~i',$S["Engine"]))$wd[]="SPATIAL";$w=indexes($a);$Of=array();if($x=="mongo"){$Of=$w["_id_"];unset($wd[0]);unset($w["_id_"]);}$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]){$c=array();foreach($K["indexes"]as$v){$C=$v["name"];if(in_array($v["type"],$wd)){$e=array();$fe=array();$Tb=array();$O=array();ksort($v["columns"]);foreach($v["columns"]as$y=>$d){if($d!=""){$ee=$v["lengths"][$y];$Sb=$v["descs"][$y];$O[]=idf_escape($d).($ee?"(".(+$ee).")":"").($Sb?" DESC":"");$e[]=$d;$fe[]=($ee?$ee:null);$Tb[]=$Sb;}}if($e){$Ac=$w[$C];if($Ac){ksort($Ac["columns"]);ksort($Ac["lengths"]);ksort($Ac["descs"]);if($v["type"]==$Ac["type"]&&array_values($Ac["columns"])===$e&&(!$Ac["lengths"]||array_values($Ac["lengths"])===$fe)&&array_values($Ac["descs"])===$Tb){unset($w[$C]);continue;}}$c[]=array($v["type"],$C,$O);}}}foreach($w
as$C=>$Ac)$c[]=array($Ac["type"],$C,"DROP");if(!$c)redirect(ME."table=".urlencode($a));queries_redirect(ME."table=".urlencode($a),lang(171),alter_indexes($a,$c));}page_header(lang(124),$n,array("table"=>$a),h($a));$p=array_keys(fields($a));if($_POST["add"]){foreach($K["indexes"]as$y=>$v){if($v["columns"][count($v["columns"])]!="")$K["indexes"][$y]["columns"][]="";}$v=end($K["indexes"]);if($v["type"]||array_filter($v["columns"],'strlen'))$K["indexes"][]=array("columns"=>array(1=>""));}if(!$K){foreach($w
...
';if($Of){echo"<tr><td>PRIMARY<td>";foreach($Of["columns"]as$y=>$d){echo
select_input(" disabled",$p,$d),"<label><input disabled type='checkbox'>".lang(56)."</label> ";}echo"<td><td>\n";}$Md=1;foreach($K["indexes"]as$v){if(!$_POST["drop_col"]||$Md!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Md][type]",array(-1=>"")+$wd,$v["type"],($Md==count($K["indexes"])?"indexesAddRow(this);":1),"label-type"),"<td>";ksort($v["columns"]);$s=1;foreach($v["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$Md][columns][$s]' onchange=\"".($s==count($v["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".h(js_escape($x=="sql"?"":$_GET["indexes"]."_"))."');\" title='".lang(45)."'",($p?array_combine($p,$p):$p),$d),($x=="sql"||$x=="mssql"?"<input type='number' name='indexes[$Md][lengths][$s]' class='size' value='".h($v["lengths"][$y])."' title='".lang(97)."'>":""),($x!="sql"?checkbox("indexes[$Md][descs][$s]",1,$v["descs"][$y],lang(56)):"")," </span>";$s++;}echo"<td><input name='indexes[$Md][name]' value='".h($v["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$Md]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".lang(102)."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$Md++;}echo'</table>
<p>
<input type="submit" value="',lang(14),'">
<input type="hidden" name="token" value="',$Lh,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),lang(175),drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),lang(176),rename_database($C,$K["collation"]));}else{$l=explode("\n",str_replace("\r","",$C));$ih=true;$Yd="";foreach($l
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$ih=false;$Yd=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($Yd),lang(177),$ih);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(178));}}page_header(DB!=""?lang(64):lang(106),$n,array(),h(DB));$qb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$qb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$dd){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$dd,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($qb?html_select("collation",array(""=>"(".lang(92).")")+$qb,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(182).": ".h($da),$n);$wg=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$ud=array();$rf=array();foreach($wg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$rf[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$ud[]=$s;}if(!$n&&$_POST){$Za=array();foreach($wg["fields"]as$y=>$o){if(in_array($y,$ud)){$X=process_input($o);if($X===false)$X="''";if(isset($rf[$y]))$g->query("SET @".idf_escape($o["field"])." = $X");}$Za[]=(isset($rf[$y])?"@".idf_escape($o["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Za).")";echo"<p><code class='jush-$x'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n";if(!$g->multi_query($H))echo"<p class='error'>".error()."\n";else{$h=connect();if(is_object($h))$h->select_db(DB);do{$I=$g->store_result();if(is_object($I))select($I,$h);else
echo"<p class='message'>".lang(183,$g->affected_rows)."\n";}while($g->next_result());if($rf)select($g->query("SELECT ".implode(", ",$rf)));}}echo'
<form action="" method="post">
';if($ud){echo"<table cellspacing='0'>\n";foreach($ud
as$y){$o=$wg["fields"][$y];$C=$o["field"];echo"<tr><th>".$b->fieldName($o);$Y=$_POST["fields"][$C];if($Y!=""){if($o["type"]=="enum")$Y=+$Y;if($o["type"]=="set")$Y=array_sum($Y);}input($o,$Y,(string)$_POST["function"][$C]);echo"\n";}echo"</table>\n";}echo'<p>
<input type="submit" value="',lang(182),'">
<input type="hidden" name="token" value="',$Lh,'">
</form>
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$ve=($_POST["drop"]?lang(184):($C!=""?lang(185):lang(186)));$A=ME."table=".urlencode($a);if(!$_POST["drop"]){$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$wh=array();foreach($K["source"]as$y=>$X)$wh[$y]=$K["target"][$y];$K["target"]=$wh;}if($x=="sqlite")queries_redirect($A,$ve,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$cc="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$cc,$A,$ve);else{query_redirect($c.($C!=""?"$cc,":"")."\nADD".format_foreign_key($K),$A,$ve);$n=lang(187)."<br>$n";}}}page_header(lang(188),$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($C!=""){$Wc=foreign_keys($a);$K=$Wc[$C];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$Wg=array_keys(fields($a));$wh=($a===$K["table"]?$Wg:array_keys(fields($K["table"])));$hg=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
';if($K["db"]==""&&$K["ns"]==""){echo
lang(189),':
',html_select("table",$hg,$K["table"],"this.form['change-js'].value = '1'; this.form.submit();"),'<input type="hidden" name="change-js" value="">
...
';$Md=0;foreach($K["source"]as$y=>$X){echo"<tr>","<td>".html_select("source[".(+$y)."]",array(-1=>"")+$Wg,$X,($Md==count($K["source"])-1?"foreignAddRow(this);":1),"label-source"),"<td>".html_select("target[".(+$y)."]",$wh,$K["target"][$y],1,"label-target");$Md++;}echo'</table>
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$Xe),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$Xe),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$of="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$of=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ha=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ve=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$of=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ha,$A,$ve);else{$yh=$C."_adminer_".uniqid();drop_create("DROP $of ".table($a),"CREATE $U ".table($C).$Ha,"DROP $U ".table($C),"CREATE $U ".table($yh).$Ha,"DROP $U ".table($yh),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$ve,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($of!="VIEW");if(!$n)$n=error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>',lang(203),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Dd,$K["INTERVAL_FIELD"]),'<tr><th>',lang(109),'<td>',html_select("STATUS",$eh,$K["STATUS"]),'<tr><th>',lang(47),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$wg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$yh="$K[name]_adminer_".uniqid();drop_create("DROP $wg ".idf_escape($da),create_routine($wg,$K),"DROP $wg ".idf_escape($K["name"]),create_routine($wg,array("name"=>$yh)+$K),"DROP $wg ".idf_escape($yh),substr(ME,0,-1),lang(205),lang(206),lang(207),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(208):lang(209)).": ".h($da):(isset($_GET["function"])?lang(210):lang(211))),$n);if(!$_POST&&$da!=""){$K=routine($da,$wg);$K["name"]=$da;}$qb=get_vals("SHOW CHARACTER SET");sort($qb);$xg=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(174),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($xg?lang(19).": ".html_select("language",$xg,$K["language"]):""),'<input type="submit" value="',lang(14),'">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$qb,$wg);if(isset($_GET["function"])){echo"<tr><td>".lang(212);edit_type("returns",$K["returns"],$qb);}echo'</table>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Vh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Vh["Timing"])&&in_array($_POST["Event"],$Vh["Event"])&&in_array($_POST["Type"],$Vh["Type"])){$We=" ON ".table($a);$cc="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$We:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($cc,$A,lang(222));else{if($C!="")queries($cc);queries_redirect($A,($C!=""?lang(223):lang(224)),queries(create_trigger($We,$_POST)));if($C!="")queries(create_trigger($We,$K+array("Type"=>reset($Vh["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(225).": ".h($C):lang(226)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
...
';}elseif(isset($_GET["user"])){$ha=$_GET["user"];$Tf=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$K){foreach(explode(",",($K["Privilege"]=="Grant option"?"":$K["Context"]))as$Bb)$Tf[$Bb][$K["Privilege"]]=$K["Comment"];}$Tf["Server Admin"]+=$Tf["File access on server"];$Tf["Databases"]["Create routine"]=$Tf["Procedures"]["Create routine"];unset($Tf["Procedures"]["Create routine"]);$Tf["Columns"]=array();foreach(array("Select","Insert","Update","References")as$X)$Tf["Columns"][$X]=$Tf["Tables"][$X];unset($Tf["Server Admin"]["Usage"]);foreach($Tf["Tables"]as$y=>$X)unset($Tf["Databases"][$y]);$Ie=array();if($_POST){foreach($_POST["objects"]as$y=>$X)$Ie[$X]=(array)$Ie[$X]+(array)$_POST["grants"][$y];}$ed=array();$Ue="";if(isset($_GET["host"])&&($I=$g->query("SHOW GRANTS FOR ".q($ha)."@".q($_GET["host"])))){while($K=$I->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$K[0],$B)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$B[1],$ne,PREG_SET_ORDER)){foreach($ne
as$X){if($X[1]!="USAGE")$ed["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$ed["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ue=$B[1];}}if($_POST&&!$n){$Ve=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Ve",ME."privileges=",lang(229));else{$Ke=q($_POST["user"])."@".q($_POST["host"]);$Bf=$_POST["pass"];if($Bf!=''&&!$_POST["hashed"]){$Bf=$g->result("SELECT PASSWORD(".q($Bf).")");$n=!$Bf;}$Gb=false;if(!$n){if($Ve!=$Ke){$Gb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Ke IDENTIFIED BY PASSWORD ".q($Bf));$n=!$Gb;}elseif($Bf!=$Ue)queries("SET PASSWORD FOR $Ke = ".q($Bf));}if(!$n){$tg=array();foreach($Ie
as$Pe=>$dd){if(isset($_GET["grant"]))$dd=array_filter($dd);$dd=array_keys($dd);if(isset($_GET["grant"]))$tg=array_diff(array_keys(array_filter($Ie[$Pe],'strlen')),$dd);elseif($Ve==$Ke){$Se=array_keys((array)$ed[$Pe]);$tg=array_diff($Se,$dd);$dd=array_diff($dd,$Se);unset($ed[$Pe]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Pe,$B)&&(!grant("REVOKE",$tg,$B[2]," ON $B[1] FROM $Ke")||!grant("GRANT",$dd,$B[2]," ON $B[1] TO $Ke"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($Ve!=$Ke)queries("DROP USER $Ve");elseif(!isset($_GET["grant"])){foreach($ed
as$Pe=>$tg){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Pe,$B))grant("REVOKE",array_keys($tg),$B[2]," ON $B[1] FROM $Ke");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?lang(230):lang(231)),!$n);if($Gb)$g->query("DROP USER $Ke");}}page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(138)),$n,array("privileges"=>array('',lang(68))));if($_POST){$K=$_POST;$ed=$Ie;}else{$K=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Ue;if($Ue!="")$K["hashed"]=true;$ed[(DB==""||$ed?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(68).doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($ed
as$Pe=>$dd){echo'<th>'.($Pe!="*.*"?"<input name='objects[$s]' value='".h($Pe)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(123),"Columns"=>lang(45),"Procedures"=>lang(233),)as$Bb=>$Sb){foreach((array)$Tf[$Bb]as$Sf=>$vb){echo"<tr".odd()."><td".($Sb?">$Sb<td":" colspan='2'").' lang="en" title="'.h($vb).'">'.h($Sf);$s=0;foreach($ed
as$Pe=>$dd){$C="'grants[$s][".h(strtoupper($Sf))."]'";$Y=$dd[strtoupper($Sf)];if($Bb=="Server Admin"&&$Pe!=(isset($ed["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(234)."<option value='0'".($Y=="0"?" selected":"").">".lang(235)."</select>";else
echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Sf=="All privileges"?" id='grants-$s-all'":($Sf=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Lh,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Td=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$Td++;}queries_redirect(ME."processlist=",lang(236,$Td),$Td||!$_POST["kill"]);}page_header(lang(107),$n);echo'
<form action="" method="post">
...
';}elseif(isset($_GET["replication"])){page_header(lang(110));echo"<h3>".lang(240).doc_link(array("sql"=>"show-master-status.html"))."</h3>\n";$le=replication_status("MASTER");if(!$le)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($le[0]as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}$Tg=replication_status("SLAVE");if($Tg){echo"<h3>".lang(241).doc_link(array("sql"=>"show-slave-status.html"))."</h3>\n";foreach($Tg[0]as$Sg){echo"<table cellspacing='0'>\n";foreach($Sg
as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}}}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$p=fields($a);$Wc=column_foreign_keys($a);$Re="";if($S["Oid"]){$Re=($x=="sqlite"?"rowid":"oid");$w[]=array("type"=>"PRIMARY","columns"=>array($Re));}parse_str($_COOKIE["adminer_import"],$ya);$ug=array();$e=array();$Bh=null;foreach($p
as$y=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){if($o["is_virtual"])continue;$e[$y]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$Bh=$b->selectLengthProcess();}$ug+=$o["privileges"];}list($M,$fd)=$b->selectColumnsProcess($e,$w);$Hd=count($fd)<count($M);$Z=$b->selectSearchProcess($p,$w);$hf=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$bd=($M?implode(", ",$M):"*".($Re?", $Re":"")).convert_fields($e,$p,$M)."\nFROM ".table($a);$gd=($fd&&$Hd?"\nGROUP BY ".implode(", ",$fd):"").($hf?"\nORDER BY ".implode(", ",$hf):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$di=>$K){$Ha=convert_field($p[key($K)]);$M=array($Ha?$Ha:idf_escape(key($K)));$Z[]=where_check($di,$p);$J=$ac->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$Bi=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$gb=array();foreach($_POST["check"]as$db)$gb[]=where_check($db,$p);$Bi[]="((".implode(") OR (",$gb)."))";}$Bi=($Bi?"\nWHERE ".implode(" AND ",$Bi):"");$Of=$fi=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Of=array_flip($v["columns"]);$fi=($M?$Of:array());break;}}foreach((array)$fi
as$y=>$X){if(in_array(idf_escape($y),$M))unset($fi[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$fi===array())$H="SELECT $bd$Bi$gd";else{$bi=array();foreach($_POST["check"]as$X)$bi[]="(SELECT".limit($bd,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$gd,1).")";$H=implode(" UNION ALL ",$bi);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Wc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($e
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($fi===array()&&is_array($_POST["check"]))||$Hd){$I=($_POST["delete"]?$ac->delete($a,$Bi):($_POST["clone"]?queries("INSERT $H$Bi"):$ac->update($a,$O,$Bi)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Ai="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$ac->delete($a,$Ai,1):($_POST["clone"]?queries("INSERT".limit1($H,$Ai)):$ac->update($a,$O,$Ai)));if(!$I)break;$za+=$g->affected_rows;}}}$ve=lang(242,$za);if($_POST["clone"]&&$I&&$za==1){$Zd=last_id();if($Zd)$ve=lang(160," $Zd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$ve,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(243);else{$I=true;$za=0;foreach($_POST["val"]as$di=>$K){$O=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$I=$ac->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($di,$p),!($Hd||$fi===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(242,$za),$I);}}elseif(!is_string($Mc=get_file("csv_file",true)))$n=upload_error($Mc);elseif(!preg_match('~~u',$Mc))$n=lang(244);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$sb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Mc,$ne);$za=count($ne[0]);$ac->begin();$Ig=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($ne[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Ig]*)$Ig~",$X.$Ig,$oe);if(!$y&&!array_diff($oe[1],$sb)){$sb=$oe[1];$za--;}else{$O=array();foreach($oe[1]as$s=>$nb)$O[idf_escape($sb[$s])]=($nb==""&&$p[$sb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$nb))));$L[]=$O;}}$I=(!$L||$ac->insertUpdate($a,$L,$Of));if($I)$I=$ac->commit();queries_redirect(remove_from_uri("page"),lang(245,$za),$I);$ac->rollback();}}}$oh=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(50).": $oh",$n);$O=null;if(isset($ug["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Wc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e&&support("table"))echo"<p class='error'>".lang(246).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$w);$b->selectOrderPrint($hf,$e,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($Bh);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Zc=$g->result(count_rows($a,$Z,$Hd,$fd));$E=floor(max(0,$Zc-1)/$z);}$Fg=$M;if(!$Fg){$Fg[]="*";if($Re)$Fg[]=$Re;}$Cb=convert_fields($e,$p,$M);if($Cb)$Fg[]=substr($Cb,2);$I=$ac->select($a,$Fg,$Z,$fd,$hf,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$I->seek($z*$E);$pc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$fd&&$Hd&&$x=="sql")$Zc=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Qa=$b->backwardKeys($a,$oh);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$fd&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(247)."</a>");$He=array();$cd=array();reset($M);$dg=1;foreach($L[0]as$y=>$X){if($y!=$Re){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$y];$C=($o?$b->fieldName($o,$dg):($X["fun"]?"*":$y));if($C!=""){$dg++;$He[$y]=$C;$d=idf_escape($y);$qd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Sb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($qd.($hf[0]==$d||$hf[0]==$y||(!$hf&&$Hd&&$fd[0]==$d)?$Sb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($qd.$Sb)."' title='".lang(56)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($y)).'\'); return false;" title="'.lang(53).'" class="text jsonly"> =</a>';echo"</span>";}$cd[$y]=$X["fun"];next($M);}}$fe=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$y=>$X)$fe[$y]=max($fe[$y],min(40,strlen(utf8_decode($X))));}}echo($Qa?"<th>".lang(248):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Wc)as$Ge=>$K){$ci=unique_array($L[$Ge],$w);if(!$ci){$ci=array();foreach($L[$Ge]as$y=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$y))$ci[$y]=$X;}}$di="";foreach($ci
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x=='sql'&&preg_match("~^utf8_~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$di.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$fd&&$M?"":"<td>".checkbox("check[]",substr($di,1),in_array(substr($di,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Hd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$di)."'>".lang(249)."</a>"));foreach($K
as$y=>$X){if(isset($He[$y])){$o=$p[$y];if($X!=""&&(!isset($pc[$y])||$pc[$y]!=""))$pc[$y]=(is_mail($X)?$He[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$di;if(!$_&&$X!==null){foreach((array)$Wc[$y]as$q){if(count($Wc[$y])==1||end($q["source"])==$y){$_="";foreach($q["source"]as$s=>$Wg)$_.=where_link($s,$q["target"][$s],$L[$Ge][$Wg]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if($q["ns"])$_=preg_replace('~([?&]ns=)[^&]+~','\\1'.urlencode($q["ns"]),$_);if(count($q["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$ci))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($ci
as$Nd=>$W)$_.=where_link($s++,$Nd,$W);}$X=select_value($X,$_,$o,$Bh);$t=h("val[$di][".bracket_escape($y)."]");$Y=$_POST["val"][$di][bracket_escape($y)];$kc=!is_array($K[$y])&&is_utf8($X)&&$L[$Ge][$y]==$K[$y]&&!$cd[$y];$Ah=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$kc)||$Y!==null){$id=h($Y!==null?$Y:$K[$y]);echo"<td>".($Ah?"<textarea name='$t' cols='30' rows='".(substr_count($K[$y],"\n")+1)."'>$id</textarea>":"<input name='$t' value='$id' size='$fe[$y]'>");}else{$je=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($je?2:($Ah?1:0)).($kc?"":", '".h(lang(250))."'").");\">$X";}}}if($Qa)echo"<td>";$b->backwardKeysPrint($Qa,$L[$Ge]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$zc=true;if($_GET["page"]!="last"){if(!+$z)$Zc=count($L);elseif($x!="sql"||!$Hd){$Zc=($Hd?false:found_rows($S,$Z));if($Zc<max(1e4,2*($E+1)*$z))$Zc=reset(slow_query(count_rows($a,$Z,$Hd,$fd)));else$zc=false;}}if(+$z&&($Zc===false||$Zc>$z||$E)){echo"<p class='pages'>";$qe=($Zc===false?$E+(count($L)>=$z?2:1):floor(($Zc-1)/$z));if($x!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(251)."', '".($E+1)."'), event); return false;\">".lang(251)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($qe,$E+5);$s++)echo
pagination($s,$E);if($qe>0){echo($E+5<$qe?" ...":""),($zc&&$Zc!==false?pagination($qe,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$qe'>".lang(252)."</a>");}echo(($Zc===false?count($L)+1:$Zc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(253).'...\');" class="loadmore">'.lang(254).'</a>':'');}else{echo
lang(251).":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($qe>$E?pagination($E+1,$E).($qe>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Zc!==false?"(".($zc?"":"~ ").lang(142,$Zc).") ":"");$Xb=($zc?"":"~ ").$Zc;echo
checkbox("all",1,0,lang(255),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Xb' : checked); selectCount('selected2', this.checked || !checked ? '$Xb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>',lang(247),'</legend><div>
...
';}$Xc=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($Xc['sql']);break;}}if($Xc){print_fieldset("export",lang(70)." <span id='selected2'></span>");$sf=$b->dumpOutput();echo($sf?html_select("output",$sf,$ya["output"])." ":""),html_select("format",$Xc,$ya["format"])," <input type='submit' name='export' value='".lang(70)."'>\n","</div></fieldset>\n";}echo(!$fd&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",lang(69),!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ya["format"],1);echo" <input type='submit' name='import' value='".lang(69)."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($pc,'strlen'),$e);echo"<p><input type='hidden' name='token' value='$Lh'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$P=isset($_GET["status"]);page_header($P?lang(109):lang(108));$si=($P?show_status():show_variables());if(!$si)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($si
as$y=>$X){echo"<tr>","<th><code class='jush-".$x.($P?"status":"set")."'>".h($y)."</code>","<td>".nbsp($X);}echo"</table>\n";}}elseif(isset($_GET["script"])){header("Content-Type: text/javascript; charset=utf-8");if($_GET["script"]=="db"){$lh=array("Data_length"=>0,"Index_length"=>0,"Data_free"=>0);foreach(table_status()as$C=>$S){json_row("Comment-$C",nbsp($S["Comment"]));if(!is_view($S)){foreach(array("Engine","Collation")as$y)json_row("$y-$C",nbsp($S[$y]));foreach($lh+array("Auto_increment"=>0,"Rows"=>0)as$y=>$X){if($S[$y]!=""){$X=format_number($S[$y]);json_row("$y-$C",($y=="Rows"&&$X&&$S["Engine"]==($Zg=="pgsql"?"table":"InnoDB")?"~ $X":$X));if(isset($lh[$y]))$lh[$y]+=($S["Engine"]!="InnoDB"||$y!="Data_free"?$S[$y]:0);}elseif(array_key_exists($y,$S))json_row("$y-$C");}}}foreach($lh
as$y=>$X)json_row("sum-$y",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$m=>$X){json_row("tables-$m",$X);json_row("size-$m",db_size($m));}json_row("");}exit;}else{$uh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($uh&&!$n&&!$_POST["search"]){$I=true;$ve="";if($x=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ve=lang(256);}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ve=lang(257);}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ve=lang(258);}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ve=lang(259);}elseif($x!="sql"){$I=($x=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ve=lang(260);}elseif(!$_POST["tables"])$ve=lang(9);elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ve.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ve,$I);}page_header(($_GET["ns"]==""?lang(35).": ".h(DB):lang(73).": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".lang(261)."</h3>\n";$th=tables_list();if(!$th)echo"<p class='message'>".lang(9)."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".lang(262)." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".lang(53)."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}$Yb=doc_link(array('sql'=>'show-table-status.html'));echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);" class="jsonly">','<th>'.lang(123),'<td>'.lang(263).doc_link(array('sql'=>'storage-engines.html')),'<td>'.lang(114).doc_link(array('sql'=>'charset-mysql.html')),'<td>'.lang(264).$Yb,'<td>'.lang(265).$Yb,'<td>'.lang(266).$Yb,'<td>'.lang(48).doc_link(array('sql'=>'example-auto-increment.html')),'<td>'.lang(267).$Yb,(support("comment")?'<td>'.lang(47).$Yb:''),"</thead>\n";$T=0;foreach($th
as$C=>$U){$vi=($U!==null&&!preg_match('~table~i',$U));$t=h("Table-".$C);echo'<tr'.odd().'><td>'.checkbox(($vi?"views[]":"tables[]"),$C,in_array($C,$uh,true),"","formUncheck('check-all');","",$t),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($C)."' title='".lang(40)."' id='$t'>".h($C).'</a>':h($C));if($vi){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.lang(41).'">'.(preg_match('~materialized~i',$U)?lang(121):lang(122)).'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.lang(39).'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",lang(42)),"Index_length"=>array("indexes",lang(125)),"Data_free"=>array("edit",lang(43)),"Auto_increment"=>array("auto_increment=1&create",lang(42)),"Rows"=>array("select",lang(39)),)as$y=>$_){$t=" id='$y-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$y=="Rows"||(support("indexes")&&$y!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$t title='$_[1]'>?</a>":"<span$t>?</span>"):"<td id='$y-".h($C)."'>&nbsp;");}$T++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".lang(238,count($th)),"<td>".nbsp($x=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$y)echo"<td align='right' id='sum-$y'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$pi="<input type='submit' value='".lang(268)."'".on_help("'VACUUM'")."> ";$df="<input type='submit' name='optimize' value='".lang(269)."'".on_help($x=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".lang(118)." <span id='selected'></span></legend><div>".($x=="sqlite"?$pi:($x=="pgsql"?$pi.$df:($x=="sql"?"<input type='submit' value='".lang(270)."'".on_help("'ANALYZE TABLE'")."> ".$df."<input type='submit' name='check' value='".lang(271)."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".lang(272)."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".lang(273)."'".confirm().on_help($x=="sqlite"?"'DELETE'":"'TRUNCATE".($x=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".lang(119)."'".confirm().on_help("'DROP TABLE'").">\n";$l=(support("scheme")?$b->schemas():$b->databases());if(count($l)!=1&&$x!="sqlite"){$m=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".lang(274).": ",($l?html_select("target",$l,$m):'<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".lang(275)."'>",(support("copy")?" <input type='submit' name='copy' value='".lang(276)."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $T);":"")."\">\n";echo"<input type='hidden' name='token' value='$Lh'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.lang(71)."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.lang(195)."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".lang(135)."</h3>\n";$yg=routines();if($yg){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.lang(174).'<td>'.lang(46).'<td>'.lang(212)."<td>&nbsp;</thead>\n";odd('');foreach($yg
as$K){echo'<tr'.odd().'>','<th><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($K["ROUTINE_NAME"]).'">'.h($K["ROUTINE_NAME"]).'</a>','<td>'.h($K["ROUTINE_TYPE"]),'<td>'.h($K["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($K["ROUTINE_NAME"]).'">'.lang(128)."</a>";}echo"</table>\n";}echo'<p class="links">'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.lang(211).'</a>':'').'<a href="'.h(ME).'function=">'.lang(210)."</a>\n";}if(support("sequence")){echo"<h3 id='sequences'>".lang(277)."</h3>\n";$Kg=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema() ORDER BY sequence_name");if($Kg){echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(174)."</thead>\n";odd('');foreach($Kg
