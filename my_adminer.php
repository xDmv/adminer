...
explode("/",$J);}}function
editInput($Q,$n,$Ja,$Y){if($n["is_virtual"])return"<label $Ja>".h($Y).'</label>';else{if($n["type"]=="json")return"<textarea cols='50' rows='10' $Ja>".h($Y).'</textarea>';if($n["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ja value='-1' checked><i>".lang(8)."</i></label> ":"").($n["null"]?"<label><input type='radio'$Ja value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ja,$n,$Y,0);}}function
